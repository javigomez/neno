...
class LingoDatabaseParser
{
	...
	public static function getSqlQueryUsingShadowTable($sql)
	{
		...
		if (!empty($sqlElements['FROM']))
		{
			...
			if ($fromTable !== null)
			{
				...
				if ($languageCode !== '')
				{
					$fromTable[1]['table']              = self::generateShadowTableName(
						$fromTable['table'],
						$languageCode
					);
	 ...
	 *
	 * @param   array  $fromStatements  From statements array
	 *
	 ...
	 *
	 * @param   array  $fromStatement  From Statement array structure
	 *
	...
	{
		return str_replace(
			array( '#__', JFactory::getConfig()->get('dbprefix') ),
			'', $tableName
		);
	}
	...
	public static function getQueryType($sql)
	{
		...
		{
			if (!empty($parser->parsed['UPDATE']))
			{
				return self::UPDATE_QUERY;
			}
			else
			{
				if (!empty($parser->parsed['INSERT']))
				{
					return self::INSERT_QUERY;
				}
				else
				{
					if (!empty($parser->parsed['DELETE']))
					{
						return self::DELETE_QUERY;
					}
					else
					{
						if (!empty($parser->parsed['REPLACE']))
						{
