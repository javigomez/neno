...
class NenoController extends JControllerLegacy
{
	...
	public function display($cachable = false, $urlParams = array ())
	{
		...
		if (NenoSettings::get('installation_completed') != 1 && $view != 'installation')
		{
	...
	public function saveTranslationMethod()
	{
		...
		$db    = JFactory::getDbo();
		$query = 'REPLACE INTO #__neno_content_language_defaults (lang,translation_method_id, ordering) VALUES (' . $db->quote($language) . ',' . $translationMethod . ',' . $ordering . ')';
		$db->setQuery($query);
		$db->execute();
