...
class NenoDatabaseParser
{
	...
	public static function getSqlQueryUsingShadowTable($sql)
	{
		...
		if (!empty($sqlElements['FROM']))
		{
			...
			if ($fromTable !== null)
			{
				...
				if ($languageCode !== '')
				{
					$fromTable[1]['table']              = self::generateShadowTableName($fromTable[1]['table'], $languageCode);
	...
	{
		return '#__neno_sh_' . self::cleanLanguageTag($languageTag) . '_' . self::cleanTableName($tableName);
	}
