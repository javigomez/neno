-- --------------------------------------------------------

--
-- Table structure for table `#__neno_content_elements_translation_x_translation_method`
--

CREATE TABLE IF NOT EXISTS `#__neno_content_elements_translation_x_translation_method` (
  `translation_method_id` int(11) NOT NULL,
  `translation_id` int(11) NOT NULL,
  `state` tinyint(1) NOT NULL DEFAULT '0',
  `ordering` int(11) NOT NULL DEFAULT '0'
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- --------------------------------------------------------

--
-- Table structure for table `#__neno_content_element_fields`
--

CREATE TABLE IF NOT EXISTS `#__neno_content_element_fields` (
  `id` int(11) NOT NULL,
  `table_id` int(11) NOT NULL,
  `field_name` varchar(100) NOT NULL,
  `field_type` varchar(45) NOT NULL,
  `type` varchar(45) NOT NULL,
  `translate` tinyint(1) NOT NULL DEFAULT '1'
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- --------------------------------------------------------

--
-- Table structure for table `#__neno_content_element_fields_x_translations`
--

CREATE TABLE IF NOT EXISTS `#__neno_content_element_fields_x_translations` (
  `field_id` int(11) NOT NULL,
  `translation_id` int(11) NOT NULL,
  `value` text NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- --------------------------------------------------------

--
-- Table structure for table `#__neno_content_element_groups`
--

CREATE TABLE IF NOT EXISTS `#__neno_content_element_groups` (
  `id` int(11) NOT NULL,
  `group_name` varchar(150) NOT NULL,
  `extension_id` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- --------------------------------------------------------

--
-- Table structure for table `#__neno_content_element_langstrings`
--

CREATE TABLE IF NOT EXISTS `#__neno_content_element_langstrings` (
  `id` int(11) NOT NULL,
  `group_id` int(11) NOT NULL,
  `constant` varchar(255) NOT NULL,
  `string` text NOT NULL,
  `language` varchar(5) NOT NULL,
  `extension` varchar(150) NOT NULL,
  `time_added` datetime NOT NULL,
  `time_changed` datetime NOT NULL,
  `time_deleted` datetime NOT NULL,
  `state` tinyint(1) NOT NULL,
  `version` tinyint(4) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- --------------------------------------------------------

--
-- Table structure for table `#__neno_content_element_presets`
--

CREATE TABLE IF NOT EXISTS `#__neno_content_element_presets` (
  `id` int(11) NOT NULL,
  `group_id` int(11) NOT NULL,
  `table_id` int(11) NOT NULL,
  `lang` varchar(5) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- --------------------------------------------------------

--
-- Table structure for table `#__neno_content_element_preset_x_translation_methods`
--

CREATE TABLE IF NOT EXISTS `#__neno_content_element_preset_x_translation_methods` (
  `translator_id` int(11) NOT NULL,
  `content_element_preset_id` int(11) NOT NULL,
  `state` tinyint(1) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- --------------------------------------------------------

--
-- Table structure for table `#__neno_content_element_tables`
--

CREATE TABLE IF NOT EXISTS `#__neno_content_element_tables` (
  `id` int(11) NOT NULL,
  `group_id` int(11) NOT NULL,
  `table_name` varchar(255) NOT NULL,
  `primary_key` varchar(255) NOT NULL,
  `translate` tinyint(1) NOT NULL DEFAULT '1'
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- --------------------------------------------------------

--
-- Table structure for table `#__neno_content_element_translations`
--

CREATE TABLE IF NOT EXISTS `#__neno_content_element_translations` (
  `id` int(11) NOT NULL,
  `content_type` enum('lang_string','db_string') NOT NULL,
  `content_id` int(11) NOT NULL,
  `language` varchar(5) NOT NULL,
  `state` tinyint(1) NOT NULL,
  `string` text NOT NULL,
  `time_added` datetime NOT NULL,
  `time_requested` datetime NOT NULL,
  `time_completed` datetime NOT NULL,
  `translation_method` enum('machine','manual','pro','') NOT NULL,
  `version` int(11) NOT NULL DEFAULT '0'
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- --------------------------------------------------------

--
-- Table structure for table `#__neno_settings`
--

CREATE TABLE IF NOT EXISTS `#__neno_settings` (
  `id` int(11) NOT NULL,
  `setting_key` varchar(150) NOT NULL,
  `setting_value` varchar(255) NOT NULL,
  `read_only` tinyint(1) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- --------------------------------------------------------

--
-- Table structure for table `#__neno_translation_methods`
--

CREATE TABLE IF NOT EXISTS `#__neno_translation_methods` (
  `id` int(11) NOT NULL,
  `translator_name` varchar(45) NOT NULL,
  `translation_type` enum('machine','pro','manual') NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- --------------------------------------------------------

--
-- Table structure for table `#__neno_translation_methods_language_pairs`
--

CREATE TABLE IF NOT EXISTS `#__neno_translation_methods_language_pairs` (
  `id` int(11) NOT NULL,
  `translation_method_id` int(11) NOT NULL,
  `source_language` varchar(5) NOT NULL,
  `destination_language` varchar(5) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- --------------------------------------------------------

--
-- Table structure for table `#__neno_translators`
--

CREATE TABLE IF NOT EXISTS `#__neno_translators` (
  `id` int(11) NOT NULL,
  `translator_name` varchar(45) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

--
-- Indexes for dumped tables
--

--
-- Indexes for table `#__neno_content_elements_translation_x_translation_method`
--
ALTER TABLE `#__neno_content_elements_translation_x_translation_method`
ADD PRIMARY KEY (`translation_method_id`,`translation_id`), ADD KEY `fk_translation_x_translation_method_idx` (`translation_method_id`), ADD KEY `fk_translation_x_translation_method_idx1` (`translation_id`);

--
-- Indexes for table `#__neno_content_element_fields`
--
ALTER TABLE `#__neno_content_element_fields`
ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `table_id_x_field` (`table_id`,`field_name`), ADD KEY `content_elements_fields_table_idx` (`table_id`);

--
-- Indexes for table `#__neno_content_element_fields_x_translations`
--
ALTER TABLE `#__neno_content_element_fields_x_translations`
ADD PRIMARY KEY (`field_id`,`translation_id`), ADD KEY `fk_#__neno_content_element_fields_idx` (`translation_id`), ADD KEY `fk_#__neno_content_element_fields_idx1` (`field_id`);

--
-- Indexes for table `#__neno_content_element_groups`
--
ALTER TABLE `#__neno_content_element_groups`
ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `group_name_UNIQUE` (`group_name`), ADD UNIQUE KEY `extension_id_unique` (`extension_id`);

--
-- Indexes for table `#__neno_content_element_langstrings`
--
ALTER TABLE `#__neno_content_element_langstrings`
ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `constant` (`constant`,`language`), ADD KEY `state` (`state`), ADD KEY `extension` (`extension`), ADD KEY `language` (`language`), ADD KEY `group_id` (`group_id`);

--
-- Indexes for table `#__neno_content_element_presets`
--
ALTER TABLE `#__neno_content_element_presets`
ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `group_id_x_table_idx` (`group_id`,`table_id`,`lang`), ADD KEY `content_elements_preset_group_idx` (`group_id`);

--
-- Indexes for table `#__neno_content_element_preset_x_translation_methods`
--
ALTER TABLE `#__neno_content_element_preset_x_translation_methods`
ADD PRIMARY KEY (`translator_id`,`content_element_preset_id`), ADD KEY `fk_cep_x_translators_content_elements_preset_idx` (`content_element_preset_id`), ADD KEY `fk_cep_x_translators_translator_idx` (`translator_id`);

--
-- Indexes for table `#__neno_content_element_tables`
--
ALTER TABLE `#__neno_content_element_tables`
ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `group_id_x_table_name` (`group_id`,`table_name`), ADD UNIQUE KEY `table_name` (`table_name`), ADD KEY `content_elements_tables_group_idx` (`group_id`), ADD KEY `translate` (`translate`);

--
-- Indexes for table `#__neno_content_element_translations`
--
ALTER TABLE `#__neno_content_element_translations`
ADD PRIMARY KEY (`id`), ADD KEY `content_id` (`content_id`), ADD KEY `content_type` (`content_type`,`content_id`), ADD KEY `content_type_2` (`content_type`), ADD KEY `language` (`language`), ADD KEY `content_type_3` (`content_type`,`content_id`,`language`), ADD KEY `state` (`state`), ADD KEY `content_type_4` (`content_type`,`content_id`,`language`,`state`), ADD KEY `translation_method` (`translation_method`);

--
-- Indexes for table `#__neno_settings`
--
ALTER TABLE `#__neno_settings`
ADD PRIMARY KEY (`id`), ADD UNIQUE KEY `setting_key_UNIQUE` (`setting_key`);

--
-- Indexes for table `#__neno_translation_methods`
--
ALTER TABLE `#__neno_translation_methods`
ADD PRIMARY KEY (`id`);

--
-- Indexes for table `#__neno_translation_methods_language_pairs`
--
ALTER TABLE `#__neno_translation_methods_language_pairs`
ADD PRIMARY KEY (`id`), ADD KEY `translation_method_x_language_pairs_idx` (`translation_method_id`);

--
-- Indexes for table `#__neno_translators`
--
ALTER TABLE `#__neno_translators`
ADD PRIMARY KEY (`id`);

--
-- AUTO_INCREMENT for table `#__neno_content_element_fields`
--
ALTER TABLE `#__neno_content_element_fields`
MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
--
-- AUTO_INCREMENT for table `#__neno_content_element_groups`
--
ALTER TABLE `#__neno_content_element_groups`
MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
--
-- AUTO_INCREMENT for table `#__neno_content_element_langstrings`
--
ALTER TABLE `#__neno_content_element_langstrings`
MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
--
-- AUTO_INCREMENT for table `#__neno_content_element_presets`
--
ALTER TABLE `#__neno_content_element_presets`
MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
--
-- AUTO_INCREMENT for table `#__neno_content_element_tables`
--
ALTER TABLE `#__neno_content_element_tables`
MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
--
-- AUTO_INCREMENT for table `#__neno_content_element_translations`
--
ALTER TABLE `#__neno_content_element_translations`
MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
--
-- AUTO_INCREMENT for table `#__neno_settings`
--
ALTER TABLE `#__neno_settings`
MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
--
-- AUTO_INCREMENT for table `#__neno_translation_methods`
--
ALTER TABLE `#__neno_translation_methods`
MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
--
-- AUTO_INCREMENT for table `#__neno_translation_methods_language_pairs`
--
ALTER TABLE `#__neno_translation_methods_language_pairs`
MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
--
-- AUTO_INCREMENT for table `#__neno_translators`
--
ALTER TABLE `#__neno_translators`
MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
--
-- Constraints for dumped tables
--

--
-- Constraints for table `#__neno_content_elements_translation_x_translation_method`
--
ALTER TABLE `#__neno_content_elements_translation_x_translation_method`
ADD CONSTRAINT `fk_translation_x_translation_method_1` FOREIGN KEY (`translation_id`) REFERENCES `#__neno_content_element_translations` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
ADD CONSTRAINT `fk_translation_x_translation_method_2` FOREIGN KEY (`translation_method_id`) REFERENCES `#__neno_translation_methods` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Constraints for table `#__neno_content_element_fields`
--
ALTER TABLE `#__neno_content_element_fields`
ADD CONSTRAINT `fk_cef_table_idx` FOREIGN KEY (`table_id`) REFERENCES `#__neno_content_element_tables` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Constraints for table `#__neno_content_element_fields_x_translations`
--
ALTER TABLE `#__neno_content_element_fields_x_translations`
ADD CONSTRAINT `fk_#__neno_content_element_fields_has_#__neno_content_element1` FOREIGN KEY (`field_id`) REFERENCES `#__neno_content_element_fields` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
ADD CONSTRAINT `fk_#__neno_content_element_fields_has_#__neno_content_element2` FOREIGN KEY (`translation_id`) REFERENCES `#__neno_content_element_translations` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Constraints for table `#__neno_content_element_groups`
--
ALTER TABLE `#__neno_content_element_groups`
ADD CONSTRAINT `#__neno_content_element_groups_ibfk_1` FOREIGN KEY (`extension_id`) REFERENCES `#__extensions` (`extension_id`);

--
-- Constraints for table `#__neno_content_element_langstrings`
--
ALTER TABLE `#__neno_content_element_langstrings`
ADD CONSTRAINT `#__neno_content_element_langstrings_ibfk_1` FOREIGN KEY (`group_id`) REFERENCES `#__neno_content_element_groups` (`id`);

--
-- Constraints for table `#__neno_content_element_presets`
--
ALTER TABLE `#__neno_content_element_presets`
ADD CONSTRAINT `fk_cep_group_idx` FOREIGN KEY (`group_id`) REFERENCES `#__neno_content_element_groups` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Constraints for table `#__neno_content_element_preset_x_translation_methods`
--
ALTER TABLE `#__neno_content_element_preset_x_translation_methods`
ADD CONSTRAINT `fk_cep_x_translators_content_element_preset_idx` FOREIGN KEY (`content_element_preset_id`) REFERENCES `#__neno_content_element_presets` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
ADD CONSTRAINT `fk_cep_x_translators_translator_idx` FOREIGN KEY (`translator_id`) REFERENCES `#__neno_translators` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Constraints for table `#__neno_content_element_tables`
--
ALTER TABLE `#__neno_content_element_tables`
ADD CONSTRAINT `fk_cet_group_idx` FOREIGN KEY (`group_id`) REFERENCES `#__neno_content_element_groups` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Constraints for table `#__neno_translation_methods_language_pairs`
--
ALTER TABLE `#__neno_translation_methods_language_pairs`
ADD CONSTRAINT `translation_method_x_language_pairs_1` FOREIGN KEY (`translation_method_id`) REFERENCES `#__neno_translation_methods` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Dumping data for table `#__neno_translation_methods`
--

INSERT INTO `#__neno_translation_methods` (`id`, `translator_name`, `translation_type`) VALUES
  (1, 'Google Translate', 'machine');

--
-- Dumping data for table `#__neno_translation_methods_language_pairs`
--

INSERT INTO `#__neno_translation_methods_language_pairs` (`id`, `translation_method_id`, `source_language`, `destination_language`) VALUES
  (1, 1, 'af', 'ar'),
  (2, 1, 'af', 'az'),
  (3, 1, 'af', 'be'),
  (4, 1, 'af', 'bg'),
  (5, 1, 'af', 'bn'),
  (6, 1, 'af', 'bs'),
  (7, 1, 'af', 'ca'),
  (8, 1, 'af', 'ceb'),
  (9, 1, 'af', 'cs'),
  (10, 1, 'af', 'cy'),
  (11, 1, 'af', 'da'),
  (12, 1, 'af', 'de'),
  (13, 1, 'af', 'el'),
  (14, 1, 'af', 'en'),
  (15, 1, 'af', 'eo'),
  (16, 1, 'af', 'es'),
  (17, 1, 'af', 'et'),
  (18, 1, 'af', 'eu'),
  (19, 1, 'af', 'fa'),
  (20, 1, 'af', 'fi'),
  (21, 1, 'af', 'fr'),
  (22, 1, 'af', 'ga'),
  (23, 1, 'af', 'gl'),
  (24, 1, 'af', 'gu'),
  (25, 1, 'af', 'ha'),
  (26, 1, 'af', 'hi'),
  (27, 1, 'af', 'hmn'),
  (28, 1, 'af', 'hr'),
  (29, 1, 'af', 'ht'),
  (30, 1, 'af', 'hu'),
  (31, 1, 'af', 'hy'),
  (32, 1, 'af', 'id'),
  (33, 1, 'af', 'ig'),
  (34, 1, 'af', 'is'),
  (35, 1, 'af', 'it'),
  (36, 1, 'af', 'iw'),
  (37, 1, 'af', 'ja'),
  (38, 1, 'af', 'jw'),
  (39, 1, 'af', 'ka'),
  (40, 1, 'af', 'kk'),
  (41, 1, 'af', 'km'),
  (42, 1, 'af', 'kn'),
  (43, 1, 'af', 'ko'),
  (44, 1, 'af', 'la'),
  (45, 1, 'af', 'lo'),
  (46, 1, 'af', 'lt'),
  (47, 1, 'af', 'lv'),
  (48, 1, 'af', 'mg'),
  (49, 1, 'af', 'mi'),
  (50, 1, 'af', 'mk'),
  (51, 1, 'af', 'ml'),
  (52, 1, 'af', 'mn'),
  (53, 1, 'af', 'mr'),
  (54, 1, 'af', 'ms'),
  (55, 1, 'af', 'mt'),
  (56, 1, 'af', 'my'),
  (57, 1, 'af', 'ne'),
  (58, 1, 'af', 'nl'),
  (59, 1, 'af', 'no'),
  (60, 1, 'af', 'ny'),
  (61, 1, 'af', 'pa'),
  (62, 1, 'af', 'pl'),
  (63, 1, 'af', 'pt'),
  (64, 1, 'af', 'ro'),
  (65, 1, 'af', 'ru'),
  (66, 1, 'af', 'si'),
  (67, 1, 'af', 'sk'),
  (68, 1, 'af', 'sl'),
  (69, 1, 'af', 'so'),
  (70, 1, 'af', 'sq'),
  (71, 1, 'af', 'sr'),
  (72, 1, 'af', 'st'),
  (73, 1, 'af', 'su'),
  (74, 1, 'af', 'sv'),
  (75, 1, 'af', 'sw'),
  (76, 1, 'af', 'ta'),
  (77, 1, 'af', 'te'),
  (78, 1, 'af', 'tg'),
  (79, 1, 'af', 'th'),
  (80, 1, 'af', 'tl'),
  (81, 1, 'af', 'tr'),
  (82, 1, 'af', 'uk'),
  (83, 1, 'af', 'ur'),
  (84, 1, 'af', 'uz'),
  (85, 1, 'af', 'vi'),
  (86, 1, 'af', 'yi'),
  (87, 1, 'af', 'yo'),
  (88, 1, 'af', 'zh'),
  (89, 1, 'af', 'zh-TW'),
  (90, 1, 'af', 'zu'),
  (91, 1, 'ar', 'af'),
  (92, 1, 'ar', 'az'),
  (93, 1, 'ar', 'be'),
  (94, 1, 'ar', 'bg'),
  (95, 1, 'ar', 'bn'),
  (96, 1, 'ar', 'bs'),
  (97, 1, 'ar', 'ca'),
  (98, 1, 'ar', 'ceb'),
  (99, 1, 'ar', 'cs'),
  (100, 1, 'ar', 'cy'),
  (101, 1, 'ar', 'da'),
  (102, 1, 'ar', 'de'),
  (103, 1, 'ar', 'el'),
  (104, 1, 'ar', 'en'),
  (105, 1, 'ar', 'eo'),
  (106, 1, 'ar', 'es'),
  (107, 1, 'ar', 'et'),
  (108, 1, 'ar', 'eu'),
  (109, 1, 'ar', 'fa'),
  (110, 1, 'ar', 'fi'),
  (111, 1, 'ar', 'fr'),
  (112, 1, 'ar', 'ga'),
  (113, 1, 'ar', 'gl'),
  (114, 1, 'ar', 'gu'),
  (115, 1, 'ar', 'ha'),
  (116, 1, 'ar', 'hi'),
  (117, 1, 'ar', 'hmn'),
  (118, 1, 'ar', 'hr'),
  (119, 1, 'ar', 'ht'),
  (120, 1, 'ar', 'hu'),
  (121, 1, 'ar', 'hy'),
  (122, 1, 'ar', 'id'),
  (123, 1, 'ar', 'ig'),
  (124, 1, 'ar', 'is'),
  (125, 1, 'ar', 'it'),
  (126, 1, 'ar', 'iw'),
  (127, 1, 'ar', 'ja'),
  (128, 1, 'ar', 'jw'),
  (129, 1, 'ar', 'ka'),
  (130, 1, 'ar', 'kk'),
  (131, 1, 'ar', 'km'),
  (132, 1, 'ar', 'kn'),
  (133, 1, 'ar', 'ko'),
  (134, 1, 'ar', 'la'),
  (135, 1, 'ar', 'lo'),
  (136, 1, 'ar', 'lt'),
  (137, 1, 'ar', 'lv'),
  (138, 1, 'ar', 'mg'),
  (139, 1, 'ar', 'mi'),
  (140, 1, 'ar', 'mk'),
  (141, 1, 'ar', 'ml'),
  (142, 1, 'ar', 'mn'),
  (143, 1, 'ar', 'mr'),
  (144, 1, 'ar', 'ms'),
  (145, 1, 'ar', 'mt'),
  (146, 1, 'ar', 'my'),
  (147, 1, 'ar', 'ne'),
  (148, 1, 'ar', 'nl'),
  (149, 1, 'ar', 'no'),
  (150, 1, 'ar', 'ny'),
  (151, 1, 'ar', 'pa'),
  (152, 1, 'ar', 'pl'),
  (153, 1, 'ar', 'pt'),
  (154, 1, 'ar', 'ro'),
  (155, 1, 'ar', 'ru'),
  (156, 1, 'ar', 'si'),
  (157, 1, 'ar', 'sk'),
  (158, 1, 'ar', 'sl'),
  (159, 1, 'ar', 'so'),
  (160, 1, 'ar', 'sq'),
  (161, 1, 'ar', 'sr'),
  (162, 1, 'ar', 'st'),
  (163, 1, 'ar', 'su'),
  (164, 1, 'ar', 'sv'),
  (165, 1, 'ar', 'sw'),
  (166, 1, 'ar', 'ta'),
  (167, 1, 'ar', 'te'),
  (168, 1, 'ar', 'tg'),
  (169, 1, 'ar', 'th'),
  (170, 1, 'ar', 'tl'),
  (171, 1, 'ar', 'tr'),
  (172, 1, 'ar', 'uk'),
  (173, 1, 'ar', 'ur'),
  (174, 1, 'ar', 'uz'),
  (175, 1, 'ar', 'vi'),
  (176, 1, 'ar', 'yi'),
  (177, 1, 'ar', 'yo'),
  (178, 1, 'ar', 'zh'),
  (179, 1, 'ar', 'zh-TW'),
  (180, 1, 'ar', 'zu'),
  (181, 1, 'az', 'af'),
  (182, 1, 'az', 'ar'),
  (183, 1, 'az', 'be'),
  (184, 1, 'az', 'bg'),
  (185, 1, 'az', 'bn'),
  (186, 1, 'az', 'bs'),
  (187, 1, 'az', 'ca'),
  (188, 1, 'az', 'ceb'),
  (189, 1, 'az', 'cs'),
  (190, 1, 'az', 'cy'),
  (191, 1, 'az', 'da'),
  (192, 1, 'az', 'de'),
  (193, 1, 'az', 'el'),
  (194, 1, 'az', 'en'),
  (195, 1, 'az', 'eo'),
  (196, 1, 'az', 'es'),
  (197, 1, 'az', 'et'),
  (198, 1, 'az', 'eu'),
  (199, 1, 'az', 'fa'),
  (200, 1, 'az', 'fi'),
  (201, 1, 'az', 'fr'),
  (202, 1, 'az', 'ga'),
  (203, 1, 'az', 'gl'),
  (204, 1, 'az', 'gu'),
  (205, 1, 'az', 'ha'),
  (206, 1, 'az', 'hi'),
  (207, 1, 'az', 'hmn'),
  (208, 1, 'az', 'hr'),
  (209, 1, 'az', 'ht'),
  (210, 1, 'az', 'hu'),
  (211, 1, 'az', 'hy'),
  (212, 1, 'az', 'id'),
  (213, 1, 'az', 'ig'),
  (214, 1, 'az', 'is'),
  (215, 1, 'az', 'it'),
  (216, 1, 'az', 'iw'),
  (217, 1, 'az', 'ja'),
  (218, 1, 'az', 'jw'),
  (219, 1, 'az', 'ka'),
  (220, 1, 'az', 'kk'),
  (221, 1, 'az', 'km'),
  (222, 1, 'az', 'kn'),
  (223, 1, 'az', 'ko'),
  (224, 1, 'az', 'la'),
  (225, 1, 'az', 'lo'),
  (226, 1, 'az', 'lt'),
  (227, 1, 'az', 'lv'),
  (228, 1, 'az', 'mg'),
  (229, 1, 'az', 'mi'),
  (230, 1, 'az', 'mk'),
  (231, 1, 'az', 'ml'),
  (232, 1, 'az', 'mn'),
  (233, 1, 'az', 'mr'),
  (234, 1, 'az', 'ms'),
  (235, 1, 'az', 'mt'),
  (236, 1, 'az', 'my'),
  (237, 1, 'az', 'ne'),
  (238, 1, 'az', 'nl'),
  (239, 1, 'az', 'no'),
  (240, 1, 'az', 'ny'),
  (241, 1, 'az', 'pa'),
  (242, 1, 'az', 'pl'),
  (243, 1, 'az', 'pt'),
  (244, 1, 'az', 'ro'),
  (245, 1, 'az', 'ru'),
  (246, 1, 'az', 'si'),
  (247, 1, 'az', 'sk'),
  (248, 1, 'az', 'sl'),
  (249, 1, 'az', 'so'),
  (250, 1, 'az', 'sq'),
  (251, 1, 'az', 'sr'),
  (252, 1, 'az', 'st'),
  (253, 1, 'az', 'su'),
  (254, 1, 'az', 'sv'),
  (255, 1, 'az', 'sw'),
  (256, 1, 'az', 'ta'),
  (257, 1, 'az', 'te'),
  (258, 1, 'az', 'tg'),
  (259, 1, 'az', 'th'),
  (260, 1, 'az', 'tl'),
  (261, 1, 'az', 'tr'),
  (262, 1, 'az', 'uk'),
  (263, 1, 'az', 'ur'),
  (264, 1, 'az', 'uz'),
  (265, 1, 'az', 'vi'),
  (266, 1, 'az', 'yi'),
  (267, 1, 'az', 'yo'),
  (268, 1, 'az', 'zh'),
  (269, 1, 'az', 'zh-TW'),
  (270, 1, 'az', 'zu'),
  (271, 1, 'be', 'af'),
  (272, 1, 'be', 'ar'),
  (273, 1, 'be', 'az'),
  (274, 1, 'be', 'bg'),
  (275, 1, 'be', 'bn'),
  (276, 1, 'be', 'bs'),
  (277, 1, 'be', 'ca'),
  (278, 1, 'be', 'ceb'),
  (279, 1, 'be', 'cs'),
  (280, 1, 'be', 'cy'),
  (281, 1, 'be', 'da'),
  (282, 1, 'be', 'de'),
  (283, 1, 'be', 'el'),
  (284, 1, 'be', 'en'),
  (285, 1, 'be', 'eo'),
  (286, 1, 'be', 'es'),
  (287, 1, 'be', 'et'),
  (288, 1, 'be', 'eu'),
  (289, 1, 'be', 'fa'),
  (290, 1, 'be', 'fi'),
  (291, 1, 'be', 'fr'),
  (292, 1, 'be', 'ga'),
  (293, 1, 'be', 'gl'),
  (294, 1, 'be', 'gu'),
  (295, 1, 'be', 'ha'),
  (296, 1, 'be', 'hi'),
  (297, 1, 'be', 'hmn'),
  (298, 1, 'be', 'hr'),
  (299, 1, 'be', 'ht'),
  (300, 1, 'be', 'hu'),
  (301, 1, 'be', 'hy'),
  (302, 1, 'be', 'id'),
  (303, 1, 'be', 'ig'),
  (304, 1, 'be', 'is'),
  (305, 1, 'be', 'it'),
  (306, 1, 'be', 'iw'),
  (307, 1, 'be', 'ja'),
  (308, 1, 'be', 'jw'),
  (309, 1, 'be', 'ka'),
  (310, 1, 'be', 'kk'),
  (311, 1, 'be', 'km'),
  (312, 1, 'be', 'kn'),
  (313, 1, 'be', 'ko'),
  (314, 1, 'be', 'la'),
  (315, 1, 'be', 'lo'),
  (316, 1, 'be', 'lt'),
  (317, 1, 'be', 'lv'),
  (318, 1, 'be', 'mg'),
  (319, 1, 'be', 'mi'),
  (320, 1, 'be', 'mk'),
  (321, 1, 'be', 'ml'),
  (322, 1, 'be', 'mn'),
  (323, 1, 'be', 'mr'),
  (324, 1, 'be', 'ms'),
  (325, 1, 'be', 'mt'),
  (326, 1, 'be', 'my'),
  (327, 1, 'be', 'ne'),
  (328, 1, 'be', 'nl'),
  (329, 1, 'be', 'no'),
  (330, 1, 'be', 'ny'),
  (331, 1, 'be', 'pa'),
  (332, 1, 'be', 'pl'),
  (333, 1, 'be', 'pt'),
  (334, 1, 'be', 'ro'),
  (335, 1, 'be', 'ru'),
  (336, 1, 'be', 'si'),
  (337, 1, 'be', 'sk'),
  (338, 1, 'be', 'sl'),
  (339, 1, 'be', 'so'),
  (340, 1, 'be', 'sq'),
  (341, 1, 'be', 'sr'),
  (342, 1, 'be', 'st'),
  (343, 1, 'be', 'su'),
  (344, 1, 'be', 'sv'),
  (345, 1, 'be', 'sw'),
  (346, 1, 'be', 'ta'),
  (347, 1, 'be', 'te'),
  (348, 1, 'be', 'tg'),
  (349, 1, 'be', 'th'),
  (350, 1, 'be', 'tl'),
  (351, 1, 'be', 'tr'),
  (352, 1, 'be', 'uk'),
  (353, 1, 'be', 'ur'),
  (354, 1, 'be', 'uz'),
  (355, 1, 'be', 'vi'),
  (356, 1, 'be', 'yi'),
  (357, 1, 'be', 'yo'),
  (358, 1, 'be', 'zh'),
  (359, 1, 'be', 'zh-TW'),
  (360, 1, 'be', 'zu'),
  (361, 1, 'bg', 'af'),
  (362, 1, 'bg', 'ar'),
  (363, 1, 'bg', 'az'),
  (364, 1, 'bg', 'be'),
  (365, 1, 'bg', 'bn'),
  (366, 1, 'bg', 'bs'),
  (367, 1, 'bg', 'ca'),
  (368, 1, 'bg', 'ceb'),
  (369, 1, 'bg', 'cs'),
  (370, 1, 'bg', 'cy'),
  (371, 1, 'bg', 'da'),
  (372, 1, 'bg', 'de'),
  (373, 1, 'bg', 'el'),
  (374, 1, 'bg', 'en'),
  (375, 1, 'bg', 'eo'),
  (376, 1, 'bg', 'es'),
  (377, 1, 'bg', 'et'),
  (378, 1, 'bg', 'eu'),
  (379, 1, 'bg', 'fa'),
  (380, 1, 'bg', 'fi'),
  (381, 1, 'bg', 'fr'),
  (382, 1, 'bg', 'ga'),
  (383, 1, 'bg', 'gl'),
  (384, 1, 'bg', 'gu'),
  (385, 1, 'bg', 'ha'),
  (386, 1, 'bg', 'hi'),
  (387, 1, 'bg', 'hmn'),
  (388, 1, 'bg', 'hr'),
  (389, 1, 'bg', 'ht'),
  (390, 1, 'bg', 'hu'),
  (391, 1, 'bg', 'hy'),
  (392, 1, 'bg', 'id'),
  (393, 1, 'bg', 'ig'),
  (394, 1, 'bg', 'is'),
  (395, 1, 'bg', 'it'),
  (396, 1, 'bg', 'iw'),
  (397, 1, 'bg', 'ja'),
  (398, 1, 'bg', 'jw'),
  (399, 1, 'bg', 'ka'),
  (400, 1, 'bg', 'kk'),
  (401, 1, 'bg', 'km'),
  (402, 1, 'bg', 'kn'),
  (403, 1, 'bg', 'ko'),
  (404, 1, 'bg', 'la'),
  (405, 1, 'bg', 'lo'),
  (406, 1, 'bg', 'lt'),
  (407, 1, 'bg', 'lv'),
  (408, 1, 'bg', 'mg'),
  (409, 1, 'bg', 'mi'),
  (410, 1, 'bg', 'mk'),
  (411, 1, 'bg', 'ml'),
  (412, 1, 'bg', 'mn'),
  (413, 1, 'bg', 'mr'),
  (414, 1, 'bg', 'ms'),
  (415, 1, 'bg', 'mt'),
  (416, 1, 'bg', 'my'),
  (417, 1, 'bg', 'ne'),
  (418, 1, 'bg', 'nl'),
  (419, 1, 'bg', 'no'),
  (420, 1, 'bg', 'ny'),
  (421, 1, 'bg', 'pa'),
  (422, 1, 'bg', 'pl'),
  (423, 1, 'bg', 'pt'),
  (424, 1, 'bg', 'ro'),
  (425, 1, 'bg', 'ru'),
  (426, 1, 'bg', 'si'),
  (427, 1, 'bg', 'sk'),
  (428, 1, 'bg', 'sl'),
  (429, 1, 'bg', 'so'),
  (430, 1, 'bg', 'sq'),
  (431, 1, 'bg', 'sr'),
  (432, 1, 'bg', 'st'),
  (433, 1, 'bg', 'su'),
  (434, 1, 'bg', 'sv'),
  (435, 1, 'bg', 'sw'),
  (436, 1, 'bg', 'ta'),
  (437, 1, 'bg', 'te'),
  (438, 1, 'bg', 'tg'),
  (439, 1, 'bg', 'th'),
  (440, 1, 'bg', 'tl'),
  (441, 1, 'bg', 'tr'),
  (442, 1, 'bg', 'uk'),
  (443, 1, 'bg', 'ur'),
  (444, 1, 'bg', 'uz'),
  (445, 1, 'bg', 'vi'),
  (446, 1, 'bg', 'yi'),
  (447, 1, 'bg', 'yo'),
  (448, 1, 'bg', 'zh'),
  (449, 1, 'bg', 'zh-TW'),
  (450, 1, 'bg', 'zu'),
  (451, 1, 'bn', 'af'),
  (452, 1, 'bn', 'ar'),
  (453, 1, 'bn', 'az'),
  (454, 1, 'bn', 'be'),
  (455, 1, 'bn', 'bg'),
  (456, 1, 'bn', 'bs'),
  (457, 1, 'bn', 'ca'),
  (458, 1, 'bn', 'ceb'),
  (459, 1, 'bn', 'cs'),
  (460, 1, 'bn', 'cy'),
  (461, 1, 'bn', 'da'),
  (462, 1, 'bn', 'de'),
  (463, 1, 'bn', 'el'),
  (464, 1, 'bn', 'en'),
  (465, 1, 'bn', 'eo'),
  (466, 1, 'bn', 'es'),
  (467, 1, 'bn', 'et'),
  (468, 1, 'bn', 'eu'),
  (469, 1, 'bn', 'fa'),
  (470, 1, 'bn', 'fi'),
  (471, 1, 'bn', 'fr'),
  (472, 1, 'bn', 'ga'),
  (473, 1, 'bn', 'gl'),
  (474, 1, 'bn', 'gu'),
  (475, 1, 'bn', 'ha'),
  (476, 1, 'bn', 'hi'),
  (477, 1, 'bn', 'hmn'),
  (478, 1, 'bn', 'hr'),
  (479, 1, 'bn', 'ht'),
  (480, 1, 'bn', 'hu'),
  (481, 1, 'bn', 'hy'),
  (482, 1, 'bn', 'id'),
  (483, 1, 'bn', 'ig'),
  (484, 1, 'bn', 'is'),
  (485, 1, 'bn', 'it'),
  (486, 1, 'bn', 'iw'),
  (487, 1, 'bn', 'ja'),
  (488, 1, 'bn', 'jw'),
  (489, 1, 'bn', 'ka'),
  (490, 1, 'bn', 'kk'),
  (491, 1, 'bn', 'km'),
  (492, 1, 'bn', 'kn'),
  (493, 1, 'bn', 'ko'),
  (494, 1, 'bn', 'la'),
  (495, 1, 'bn', 'lo'),
  (496, 1, 'bn', 'lt'),
  (497, 1, 'bn', 'lv'),
  (498, 1, 'bn', 'mg'),
  (499, 1, 'bn', 'mi'),
  (500, 1, 'bn', 'mk'),
  (501, 1, 'bn', 'ml'),
  (502, 1, 'bn', 'mn'),
  (503, 1, 'bn', 'mr'),
  (504, 1, 'bn', 'ms'),
  (505, 1, 'bn', 'mt'),
  (506, 1, 'bn', 'my'),
  (507, 1, 'bn', 'ne'),
  (508, 1, 'bn', 'nl'),
  (509, 1, 'bn', 'no'),
  (510, 1, 'bn', 'ny'),
  (511, 1, 'bn', 'pa'),
  (512, 1, 'bn', 'pl'),
  (513, 1, 'bn', 'pt'),
  (514, 1, 'bn', 'ro'),
  (515, 1, 'bn', 'ru'),
  (516, 1, 'bn', 'si'),
  (517, 1, 'bn', 'sk'),
  (518, 1, 'bn', 'sl'),
  (519, 1, 'bn', 'so'),
  (520, 1, 'bn', 'sq'),
  (521, 1, 'bn', 'sr'),
  (522, 1, 'bn', 'st'),
  (523, 1, 'bn', 'su'),
  (524, 1, 'bn', 'sv'),
  (525, 1, 'bn', 'sw'),
  (526, 1, 'bn', 'ta'),
  (527, 1, 'bn', 'te'),
  (528, 1, 'bn', 'tg'),
  (529, 1, 'bn', 'th'),
  (530, 1, 'bn', 'tl'),
  (531, 1, 'bn', 'tr'),
  (532, 1, 'bn', 'uk'),
  (533, 1, 'bn', 'ur'),
  (534, 1, 'bn', 'uz'),
  (535, 1, 'bn', 'vi'),
  (536, 1, 'bn', 'yi'),
  (537, 1, 'bn', 'yo'),
  (538, 1, 'bn', 'zh'),
  (539, 1, 'bn', 'zh-TW'),
  (540, 1, 'bn', 'zu'),
  (541, 1, 'bs', 'af'),
  (542, 1, 'bs', 'ar'),
  (543, 1, 'bs', 'az'),
  (544, 1, 'bs', 'be'),
  (545, 1, 'bs', 'bg'),
  (546, 1, 'bs', 'bn'),
  (547, 1, 'bs', 'ca'),
  (548, 1, 'bs', 'ceb'),
  (549, 1, 'bs', 'cs'),
  (550, 1, 'bs', 'cy'),
  (551, 1, 'bs', 'da'),
  (552, 1, 'bs', 'de'),
  (553, 1, 'bs', 'el'),
  (554, 1, 'bs', 'en'),
  (555, 1, 'bs', 'eo'),
  (556, 1, 'bs', 'es'),
  (557, 1, 'bs', 'et'),
  (558, 1, 'bs', 'eu'),
  (559, 1, 'bs', 'fa'),
  (560, 1, 'bs', 'fi'),
  (561, 1, 'bs', 'fr'),
  (562, 1, 'bs', 'ga'),
  (563, 1, 'bs', 'gl'),
  (564, 1, 'bs', 'gu'),
  (565, 1, 'bs', 'ha'),
  (566, 1, 'bs', 'hi'),
  (567, 1, 'bs', 'hmn'),
  (568, 1, 'bs', 'hr'),
  (569, 1, 'bs', 'ht'),
  (570, 1, 'bs', 'hu'),
  (571, 1, 'bs', 'hy'),
  (572, 1, 'bs', 'id'),
  (573, 1, 'bs', 'ig'),
  (574, 1, 'bs', 'is'),
  (575, 1, 'bs', 'it'),
  (576, 1, 'bs', 'iw'),
  (577, 1, 'bs', 'ja'),
  (578, 1, 'bs', 'jw'),
  (579, 1, 'bs', 'ka'),
  (580, 1, 'bs', 'kk'),
  (581, 1, 'bs', 'km'),
  (582, 1, 'bs', 'kn'),
  (583, 1, 'bs', 'ko'),
  (584, 1, 'bs', 'la'),
  (585, 1, 'bs', 'lo'),
  (586, 1, 'bs', 'lt'),
  (587, 1, 'bs', 'lv'),
  (588, 1, 'bs', 'mg'),
  (589, 1, 'bs', 'mi'),
  (590, 1, 'bs', 'mk'),
  (591, 1, 'bs', 'ml'),
  (592, 1, 'bs', 'mn'),
  (593, 1, 'bs', 'mr'),
  (594, 1, 'bs', 'ms'),
  (595, 1, 'bs', 'mt'),
  (596, 1, 'bs', 'my'),
  (597, 1, 'bs', 'ne'),
  (598, 1, 'bs', 'nl'),
  (599, 1, 'bs', 'no'),
  (600, 1, 'bs', 'ny'),
  (601, 1, 'bs', 'pa'),
  (602, 1, 'bs', 'pl'),
  (603, 1, 'bs', 'pt'),
  (604, 1, 'bs', 'ro'),
  (605, 1, 'bs', 'ru'),
  (606, 1, 'bs', 'si'),
  (607, 1, 'bs', 'sk'),
  (608, 1, 'bs', 'sl'),
  (609, 1, 'bs', 'so'),
  (610, 1, 'bs', 'sq'),
  (611, 1, 'bs', 'sr'),
  (612, 1, 'bs', 'st'),
  (613, 1, 'bs', 'su'),
  (614, 1, 'bs', 'sv'),
  (615, 1, 'bs', 'sw'),
  (616, 1, 'bs', 'ta'),
  (617, 1, 'bs', 'te'),
  (618, 1, 'bs', 'tg'),
  (619, 1, 'bs', 'th'),
  (620, 1, 'bs', 'tl'),
  (621, 1, 'bs', 'tr'),
  (622, 1, 'bs', 'uk'),
  (623, 1, 'bs', 'ur'),
  (624, 1, 'bs', 'uz'),
  (625, 1, 'bs', 'vi'),
  (626, 1, 'bs', 'yi'),
  (627, 1, 'bs', 'yo'),
  (628, 1, 'bs', 'zh'),
  (629, 1, 'bs', 'zh-TW'),
  (630, 1, 'bs', 'zu'),
  (631, 1, 'ca', 'af'),
  (632, 1, 'ca', 'ar'),
  (633, 1, 'ca', 'az'),
  (634, 1, 'ca', 'be'),
  (635, 1, 'ca', 'bg'),
  (636, 1, 'ca', 'bn'),
  (637, 1, 'ca', 'bs'),
  (638, 1, 'ca', 'ceb'),
  (639, 1, 'ca', 'cs'),
  (640, 1, 'ca', 'cy'),
  (641, 1, 'ca', 'da'),
  (642, 1, 'ca', 'de'),
  (643, 1, 'ca', 'el'),
  (644, 1, 'ca', 'en'),
  (645, 1, 'ca', 'eo'),
  (646, 1, 'ca', 'es'),
  (647, 1, 'ca', 'et'),
  (648, 1, 'ca', 'eu'),
  (649, 1, 'ca', 'fa'),
  (650, 1, 'ca', 'fi'),
  (651, 1, 'ca', 'fr'),
  (652, 1, 'ca', 'ga'),
  (653, 1, 'ca', 'gl'),
  (654, 1, 'ca', 'gu'),
  (655, 1, 'ca', 'ha'),
  (656, 1, 'ca', 'hi'),
  (657, 1, 'ca', 'hmn'),
  (658, 1, 'ca', 'hr'),
  (659, 1, 'ca', 'ht'),
  (660, 1, 'ca', 'hu'),
  (661, 1, 'ca', 'hy'),
  (662, 1, 'ca', 'id'),
  (663, 1, 'ca', 'ig'),
  (664, 1, 'ca', 'is'),
  (665, 1, 'ca', 'it'),
  (666, 1, 'ca', 'iw'),
  (667, 1, 'ca', 'ja'),
  (668, 1, 'ca', 'jw'),
  (669, 1, 'ca', 'ka'),
  (670, 1, 'ca', 'kk'),
  (671, 1, 'ca', 'km'),
  (672, 1, 'ca', 'kn'),
  (673, 1, 'ca', 'ko'),
  (674, 1, 'ca', 'la'),
  (675, 1, 'ca', 'lo'),
  (676, 1, 'ca', 'lt'),
  (677, 1, 'ca', 'lv'),
  (678, 1, 'ca', 'mg'),
  (679, 1, 'ca', 'mi'),
  (680, 1, 'ca', 'mk'),
  (681, 1, 'ca', 'ml'),
  (682, 1, 'ca', 'mn'),
  (683, 1, 'ca', 'mr'),
  (684, 1, 'ca', 'ms'),
  (685, 1, 'ca', 'mt'),
  (686, 1, 'ca', 'my'),
  (687, 1, 'ca', 'ne'),
  (688, 1, 'ca', 'nl'),
  (689, 1, 'ca', 'no'),
  (690, 1, 'ca', 'ny'),
  (691, 1, 'ca', 'pa'),
  (692, 1, 'ca', 'pl'),
  (693, 1, 'ca', 'pt'),
  (694, 1, 'ca', 'ro'),
  (695, 1, 'ca', 'ru'),
  (696, 1, 'ca', 'si'),
  (697, 1, 'ca', 'sk'),
  (698, 1, 'ca', 'sl'),
  (699, 1, 'ca', 'so'),
  (700, 1, 'ca', 'sq'),
  (701, 1, 'ca', 'sr'),
  (702, 1, 'ca', 'st'),
  (703, 1, 'ca', 'su'),
  (704, 1, 'ca', 'sv'),
  (705, 1, 'ca', 'sw'),
  (706, 1, 'ca', 'ta'),
  (707, 1, 'ca', 'te'),
  (708, 1, 'ca', 'tg'),
  (709, 1, 'ca', 'th'),
  (710, 1, 'ca', 'tl'),
  (711, 1, 'ca', 'tr'),
  (712, 1, 'ca', 'uk'),
  (713, 1, 'ca', 'ur'),
  (714, 1, 'ca', 'uz'),
  (715, 1, 'ca', 'vi'),
  (716, 1, 'ca', 'yi'),
  (717, 1, 'ca', 'yo'),
  (718, 1, 'ca', 'zh'),
  (719, 1, 'ca', 'zh-TW'),
  (720, 1, 'ca', 'zu'),
  (721, 1, 'ceb', 'af'),
  (722, 1, 'ceb', 'ar'),
  (723, 1, 'ceb', 'az'),
  (724, 1, 'ceb', 'be'),
  (725, 1, 'ceb', 'bg'),
  (726, 1, 'ceb', 'bn'),
  (727, 1, 'ceb', 'bs'),
  (728, 1, 'ceb', 'ca'),
  (729, 1, 'ceb', 'cs'),
  (730, 1, 'ceb', 'cy'),
  (731, 1, 'ceb', 'da'),
  (732, 1, 'ceb', 'de'),
  (733, 1, 'ceb', 'el'),
  (734, 1, 'ceb', 'en'),
  (735, 1, 'ceb', 'eo'),
  (736, 1, 'ceb', 'es'),
  (737, 1, 'ceb', 'et'),
  (738, 1, 'ceb', 'eu'),
  (739, 1, 'ceb', 'fa'),
  (740, 1, 'ceb', 'fi'),
  (741, 1, 'ceb', 'fr'),
  (742, 1, 'ceb', 'ga'),
  (743, 1, 'ceb', 'gl'),
  (744, 1, 'ceb', 'gu'),
  (745, 1, 'ceb', 'ha'),
  (746, 1, 'ceb', 'hi'),
  (747, 1, 'ceb', 'hmn'),
  (748, 1, 'ceb', 'hr'),
  (749, 1, 'ceb', 'ht'),
  (750, 1, 'ceb', 'hu'),
  (751, 1, 'ceb', 'hy'),
  (752, 1, 'ceb', 'id'),
  (753, 1, 'ceb', 'ig'),
  (754, 1, 'ceb', 'is'),
  (755, 1, 'ceb', 'it'),
  (756, 1, 'ceb', 'iw'),
  (757, 1, 'ceb', 'ja'),
  (758, 1, 'ceb', 'jw'),
  (759, 1, 'ceb', 'ka'),
  (760, 1, 'ceb', 'kk'),
  (761, 1, 'ceb', 'km'),
  (762, 1, 'ceb', 'kn'),
  (763, 1, 'ceb', 'ko'),
  (764, 1, 'ceb', 'la'),
  (765, 1, 'ceb', 'lo'),
  (766, 1, 'ceb', 'lt'),
  (767, 1, 'ceb', 'lv'),
  (768, 1, 'ceb', 'mg'),
  (769, 1, 'ceb', 'mi'),
  (770, 1, 'ceb', 'mk'),
  (771, 1, 'ceb', 'ml'),
  (772, 1, 'ceb', 'mn'),
  (773, 1, 'ceb', 'mr'),
  (774, 1, 'ceb', 'ms'),
  (775, 1, 'ceb', 'mt'),
  (776, 1, 'ceb', 'my'),
  (777, 1, 'ceb', 'ne'),
  (778, 1, 'ceb', 'nl'),
  (779, 1, 'ceb', 'no'),
  (780, 1, 'ceb', 'ny'),
  (781, 1, 'ceb', 'pa'),
  (782, 1, 'ceb', 'pl'),
  (783, 1, 'ceb', 'pt'),
  (784, 1, 'ceb', 'ro'),
  (785, 1, 'ceb', 'ru'),
  (786, 1, 'ceb', 'si'),
  (787, 1, 'ceb', 'sk'),
  (788, 1, 'ceb', 'sl'),
  (789, 1, 'ceb', 'so'),
  (790, 1, 'ceb', 'sq'),
  (791, 1, 'ceb', 'sr'),
  (792, 1, 'ceb', 'st'),
  (793, 1, 'ceb', 'su'),
  (794, 1, 'ceb', 'sv'),
  (795, 1, 'ceb', 'sw'),
  (796, 1, 'ceb', 'ta'),
  (797, 1, 'ceb', 'te'),
  (798, 1, 'ceb', 'tg'),
  (799, 1, 'ceb', 'th'),
  (800, 1, 'ceb', 'tl'),
  (801, 1, 'ceb', 'tr'),
  (802, 1, 'ceb', 'uk'),
  (803, 1, 'ceb', 'ur'),
  (804, 1, 'ceb', 'uz'),
  (805, 1, 'ceb', 'vi'),
  (806, 1, 'ceb', 'yi'),
  (807, 1, 'ceb', 'yo'),
  (808, 1, 'ceb', 'zh'),
  (809, 1, 'ceb', 'zh-TW'),
  (810, 1, 'ceb', 'zu'),
  (811, 1, 'cs', 'af'),
  (812, 1, 'cs', 'ar'),
  (813, 1, 'cs', 'az'),
  (814, 1, 'cs', 'be'),
  (815, 1, 'cs', 'bg'),
  (816, 1, 'cs', 'bn'),
  (817, 1, 'cs', 'bs'),
  (818, 1, 'cs', 'ca'),
  (819, 1, 'cs', 'ceb'),
  (820, 1, 'cs', 'cy'),
  (821, 1, 'cs', 'da'),
  (822, 1, 'cs', 'de'),
  (823, 1, 'cs', 'el'),
  (824, 1, 'cs', 'en'),
  (825, 1, 'cs', 'eo'),
  (826, 1, 'cs', 'es'),
  (827, 1, 'cs', 'et'),
  (828, 1, 'cs', 'eu'),
  (829, 1, 'cs', 'fa'),
  (830, 1, 'cs', 'fi'),
  (831, 1, 'cs', 'fr'),
  (832, 1, 'cs', 'ga'),
  (833, 1, 'cs', 'gl'),
  (834, 1, 'cs', 'gu'),
  (835, 1, 'cs', 'ha'),
  (836, 1, 'cs', 'hi'),
  (837, 1, 'cs', 'hmn'),
  (838, 1, 'cs', 'hr'),
  (839, 1, 'cs', 'ht'),
  (840, 1, 'cs', 'hu'),
  (841, 1, 'cs', 'hy'),
  (842, 1, 'cs', 'id'),
  (843, 1, 'cs', 'ig'),
  (844, 1, 'cs', 'is'),
  (845, 1, 'cs', 'it'),
  (846, 1, 'cs', 'iw'),
  (847, 1, 'cs', 'ja'),
  (848, 1, 'cs', 'jw'),
  (849, 1, 'cs', 'ka'),
  (850, 1, 'cs', 'kk'),
  (851, 1, 'cs', 'km'),
  (852, 1, 'cs', 'kn'),
  (853, 1, 'cs', 'ko'),
  (854, 1, 'cs', 'la'),
  (855, 1, 'cs', 'lo'),
  (856, 1, 'cs', 'lt'),
  (857, 1, 'cs', 'lv'),
  (858, 1, 'cs', 'mg'),
  (859, 1, 'cs', 'mi'),
  (860, 1, 'cs', 'mk'),
  (861, 1, 'cs', 'ml'),
  (862, 1, 'cs', 'mn'),
  (863, 1, 'cs', 'mr'),
  (864, 1, 'cs', 'ms'),
  (865, 1, 'cs', 'mt'),
  (866, 1, 'cs', 'my'),
  (867, 1, 'cs', 'ne'),
  (868, 1, 'cs', 'nl'),
  (869, 1, 'cs', 'no'),
  (870, 1, 'cs', 'ny'),
  (871, 1, 'cs', 'pa'),
  (872, 1, 'cs', 'pl'),
  (873, 1, 'cs', 'pt'),
  (874, 1, 'cs', 'ro'),
  (875, 1, 'cs', 'ru'),
  (876, 1, 'cs', 'si'),
  (877, 1, 'cs', 'sk'),
  (878, 1, 'cs', 'sl'),
  (879, 1, 'cs', 'so'),
  (880, 1, 'cs', 'sq'),
  (881, 1, 'cs', 'sr'),
  (882, 1, 'cs', 'st'),
  (883, 1, 'cs', 'su'),
  (884, 1, 'cs', 'sv'),
  (885, 1, 'cs', 'sw'),
  (886, 1, 'cs', 'ta'),
  (887, 1, 'cs', 'te'),
  (888, 1, 'cs', 'tg'),
  (889, 1, 'cs', 'th'),
  (890, 1, 'cs', 'tl'),
  (891, 1, 'cs', 'tr'),
  (892, 1, 'cs', 'uk'),
  (893, 1, 'cs', 'ur'),
  (894, 1, 'cs', 'uz'),
  (895, 1, 'cs', 'vi'),
  (896, 1, 'cs', 'yi'),
  (897, 1, 'cs', 'yo'),
  (898, 1, 'cs', 'zh'),
  (899, 1, 'cs', 'zh-TW'),
  (900, 1, 'cs', 'zu'),
  (901, 1, 'cy', 'af'),
  (902, 1, 'cy', 'ar'),
  (903, 1, 'cy', 'az'),
  (904, 1, 'cy', 'be'),
  (905, 1, 'cy', 'bg'),
  (906, 1, 'cy', 'bn'),
  (907, 1, 'cy', 'bs'),
  (908, 1, 'cy', 'ca'),
  (909, 1, 'cy', 'ceb'),
  (910, 1, 'cy', 'cs'),
  (911, 1, 'cy', 'da'),
  (912, 1, 'cy', 'de'),
  (913, 1, 'cy', 'el'),
  (914, 1, 'cy', 'en'),
  (915, 1, 'cy', 'eo'),
  (916, 1, 'cy', 'es'),
  (917, 1, 'cy', 'et'),
  (918, 1, 'cy', 'eu'),
  (919, 1, 'cy', 'fa'),
  (920, 1, 'cy', 'fi'),
  (921, 1, 'cy', 'fr'),
  (922, 1, 'cy', 'ga'),
  (923, 1, 'cy', 'gl'),
  (924, 1, 'cy', 'gu'),
  (925, 1, 'cy', 'ha'),
  (926, 1, 'cy', 'hi'),
  (927, 1, 'cy', 'hmn'),
  (928, 1, 'cy', 'hr'),
  (929, 1, 'cy', 'ht'),
  (930, 1, 'cy', 'hu'),
  (931, 1, 'cy', 'hy'),
  (932, 1, 'cy', 'id'),
  (933, 1, 'cy', 'ig'),
  (934, 1, 'cy', 'is'),
  (935, 1, 'cy', 'it'),
  (936, 1, 'cy', 'iw'),
  (937, 1, 'cy', 'ja'),
  (938, 1, 'cy', 'jw'),
  (939, 1, 'cy', 'ka'),
  (940, 1, 'cy', 'kk'),
  (941, 1, 'cy', 'km'),
  (942, 1, 'cy', 'kn'),
  (943, 1, 'cy', 'ko'),
  (944, 1, 'cy', 'la'),
  (945, 1, 'cy', 'lo'),
  (946, 1, 'cy', 'lt'),
  (947, 1, 'cy', 'lv'),
  (948, 1, 'cy', 'mg'),
  (949, 1, 'cy', 'mi'),
  (950, 1, 'cy', 'mk'),
  (951, 1, 'cy', 'ml'),
  (952, 1, 'cy', 'mn'),
  (953, 1, 'cy', 'mr'),
  (954, 1, 'cy', 'ms'),
  (955, 1, 'cy', 'mt'),
  (956, 1, 'cy', 'my'),
  (957, 1, 'cy', 'ne'),
  (958, 1, 'cy', 'nl'),
  (959, 1, 'cy', 'no'),
  (960, 1, 'cy', 'ny'),
  (961, 1, 'cy', 'pa'),
  (962, 1, 'cy', 'pl'),
  (963, 1, 'cy', 'pt'),
  (964, 1, 'cy', 'ro'),
  (965, 1, 'cy', 'ru'),
  (966, 1, 'cy', 'si'),
  (967, 1, 'cy', 'sk'),
  (968, 1, 'cy', 'sl'),
  (969, 1, 'cy', 'so'),
  (970, 1, 'cy', 'sq'),
  (971, 1, 'cy', 'sr'),
  (972, 1, 'cy', 'st'),
  (973, 1, 'cy', 'su'),
  (974, 1, 'cy', 'sv'),
  (975, 1, 'cy', 'sw'),
  (976, 1, 'cy', 'ta'),
  (977, 1, 'cy', 'te'),
  (978, 1, 'cy', 'tg'),
  (979, 1, 'cy', 'th'),
  (980, 1, 'cy', 'tl'),
  (981, 1, 'cy', 'tr'),
  (982, 1, 'cy', 'uk'),
  (983, 1, 'cy', 'ur'),
  (984, 1, 'cy', 'uz'),
  (985, 1, 'cy', 'vi'),
  (986, 1, 'cy', 'yi'),
  (987, 1, 'cy', 'yo'),
  (988, 1, 'cy', 'zh'),
  (989, 1, 'cy', 'zh-TW'),
  (990, 1, 'cy', 'zu'),
  (991, 1, 'da', 'af'),
  (992, 1, 'da', 'ar'),
  (993, 1, 'da', 'az'),
  (994, 1, 'da', 'be'),
  (995, 1, 'da', 'bg'),
  (996, 1, 'da', 'bn'),
  (997, 1, 'da', 'bs'),
  (998, 1, 'da', 'ca'),
  (999, 1, 'da', 'ceb'),
  (1000, 1, 'da', 'cs'),
  (1001, 1, 'da', 'cy'),
  (1002, 1, 'da', 'de'),
  (1003, 1, 'da', 'el'),
  (1004, 1, 'da', 'en'),
  (1005, 1, 'da', 'eo'),
  (1006, 1, 'da', 'es'),
  (1007, 1, 'da', 'et'),
  (1008, 1, 'da', 'eu'),
  (1009, 1, 'da', 'fa'),
  (1010, 1, 'da', 'fi'),
  (1011, 1, 'da', 'fr'),
  (1012, 1, 'da', 'ga'),
  (1013, 1, 'da', 'gl'),
  (1014, 1, 'da', 'gu'),
  (1015, 1, 'da', 'ha'),
  (1016, 1, 'da', 'hi'),
  (1017, 1, 'da', 'hmn'),
  (1018, 1, 'da', 'hr'),
  (1019, 1, 'da', 'ht'),
  (1020, 1, 'da', 'hu'),
  (1021, 1, 'da', 'hy'),
  (1022, 1, 'da', 'id'),
  (1023, 1, 'da', 'ig'),
  (1024, 1, 'da', 'is'),
  (1025, 1, 'da', 'it'),
  (1026, 1, 'da', 'iw'),
  (1027, 1, 'da', 'ja'),
  (1028, 1, 'da', 'jw'),
  (1029, 1, 'da', 'ka'),
  (1030, 1, 'da', 'kk'),
  (1031, 1, 'da', 'km'),
  (1032, 1, 'da', 'kn'),
  (1033, 1, 'da', 'ko'),
  (1034, 1, 'da', 'la'),
  (1035, 1, 'da', 'lo'),
  (1036, 1, 'da', 'lt'),
  (1037, 1, 'da', 'lv'),
  (1038, 1, 'da', 'mg'),
  (1039, 1, 'da', 'mi'),
  (1040, 1, 'da', 'mk'),
  (1041, 1, 'da', 'ml'),
  (1042, 1, 'da', 'mn'),
  (1043, 1, 'da', 'mr'),
  (1044, 1, 'da', 'ms'),
  (1045, 1, 'da', 'mt'),
  (1046, 1, 'da', 'my'),
  (1047, 1, 'da', 'ne'),
  (1048, 1, 'da', 'nl'),
  (1049, 1, 'da', 'no'),
  (1050, 1, 'da', 'ny'),
  (1051, 1, 'da', 'pa'),
  (1052, 1, 'da', 'pl'),
  (1053, 1, 'da', 'pt'),
  (1054, 1, 'da', 'ro'),
  (1055, 1, 'da', 'ru'),
  (1056, 1, 'da', 'si'),
  (1057, 1, 'da', 'sk'),
  (1058, 1, 'da', 'sl'),
  (1059, 1, 'da', 'so'),
  (1060, 1, 'da', 'sq'),
  (1061, 1, 'da', 'sr'),
  (1062, 1, 'da', 'st'),
  (1063, 1, 'da', 'su'),
  (1064, 1, 'da', 'sv'),
  (1065, 1, 'da', 'sw'),
  (1066, 1, 'da', 'ta'),
  (1067, 1, 'da', 'te'),
  (1068, 1, 'da', 'tg'),
  (1069, 1, 'da', 'th'),
  (1070, 1, 'da', 'tl'),
  (1071, 1, 'da', 'tr'),
  (1072, 1, 'da', 'uk'),
  (1073, 1, 'da', 'ur'),
  (1074, 1, 'da', 'uz'),
  (1075, 1, 'da', 'vi'),
  (1076, 1, 'da', 'yi'),
  (1077, 1, 'da', 'yo'),
  (1078, 1, 'da', 'zh'),
  (1079, 1, 'da', 'zh-TW'),
  (1080, 1, 'da', 'zu'),
  (1081, 1, 'de', 'af'),
  (1082, 1, 'de', 'ar'),
  (1083, 1, 'de', 'az'),
  (1084, 1, 'de', 'be'),
  (1085, 1, 'de', 'bg'),
  (1086, 1, 'de', 'bn'),
  (1087, 1, 'de', 'bs'),
  (1088, 1, 'de', 'ca'),
  (1089, 1, 'de', 'ceb'),
  (1090, 1, 'de', 'cs'),
  (1091, 1, 'de', 'cy'),
  (1092, 1, 'de', 'da'),
  (1093, 1, 'de', 'el'),
  (1094, 1, 'de', 'en'),
  (1095, 1, 'de', 'eo'),
  (1096, 1, 'de', 'es'),
  (1097, 1, 'de', 'et'),
  (1098, 1, 'de', 'eu'),
  (1099, 1, 'de', 'fa'),
  (1100, 1, 'de', 'fi'),
  (1101, 1, 'de', 'fr'),
  (1102, 1, 'de', 'ga'),
  (1103, 1, 'de', 'gl'),
  (1104, 1, 'de', 'gu'),
  (1105, 1, 'de', 'ha'),
  (1106, 1, 'de', 'hi'),
  (1107, 1, 'de', 'hmn'),
  (1108, 1, 'de', 'hr'),
  (1109, 1, 'de', 'ht'),
  (1110, 1, 'de', 'hu'),
  (1111, 1, 'de', 'hy'),
  (1112, 1, 'de', 'id'),
  (1113, 1, 'de', 'ig'),
  (1114, 1, 'de', 'is'),
  (1115, 1, 'de', 'it'),
  (1116, 1, 'de', 'iw'),
  (1117, 1, 'de', 'ja'),
  (1118, 1, 'de', 'jw'),
  (1119, 1, 'de', 'ka'),
  (1120, 1, 'de', 'kk'),
  (1121, 1, 'de', 'km'),
  (1122, 1, 'de', 'kn'),
  (1123, 1, 'de', 'ko'),
  (1124, 1, 'de', 'la'),
  (1125, 1, 'de', 'lo'),
  (1126, 1, 'de', 'lt'),
  (1127, 1, 'de', 'lv'),
  (1128, 1, 'de', 'mg'),
  (1129, 1, 'de', 'mi'),
  (1130, 1, 'de', 'mk'),
  (1131, 1, 'de', 'ml'),
  (1132, 1, 'de', 'mn'),
  (1133, 1, 'de', 'mr'),
  (1134, 1, 'de', 'ms'),
  (1135, 1, 'de', 'mt'),
  (1136, 1, 'de', 'my'),
  (1137, 1, 'de', 'ne'),
  (1138, 1, 'de', 'nl'),
  (1139, 1, 'de', 'no'),
  (1140, 1, 'de', 'ny'),
  (1141, 1, 'de', 'pa'),
  (1142, 1, 'de', 'pl'),
  (1143, 1, 'de', 'pt'),
  (1144, 1, 'de', 'ro'),
  (1145, 1, 'de', 'ru'),
  (1146, 1, 'de', 'si'),
  (1147, 1, 'de', 'sk'),
  (1148, 1, 'de', 'sl'),
  (1149, 1, 'de', 'so'),
  (1150, 1, 'de', 'sq'),
  (1151, 1, 'de', 'sr'),
  (1152, 1, 'de', 'st'),
  (1153, 1, 'de', 'su'),
  (1154, 1, 'de', 'sv'),
  (1155, 1, 'de', 'sw'),
  (1156, 1, 'de', 'ta'),
  (1157, 1, 'de', 'te'),
  (1158, 1, 'de', 'tg'),
  (1159, 1, 'de', 'th'),
  (1160, 1, 'de', 'tl'),
  (1161, 1, 'de', 'tr'),
  (1162, 1, 'de', 'uk'),
  (1163, 1, 'de', 'ur'),
  (1164, 1, 'de', 'uz'),
  (1165, 1, 'de', 'vi'),
  (1166, 1, 'de', 'yi'),
  (1167, 1, 'de', 'yo'),
  (1168, 1, 'de', 'zh'),
  (1169, 1, 'de', 'zh-TW'),
  (1170, 1, 'de', 'zu'),
  (1171, 1, 'el', 'af'),
  (1172, 1, 'el', 'ar'),
  (1173, 1, 'el', 'az'),
  (1174, 1, 'el', 'be'),
  (1175, 1, 'el', 'bg'),
  (1176, 1, 'el', 'bn'),
  (1177, 1, 'el', 'bs'),
  (1178, 1, 'el', 'ca'),
  (1179, 1, 'el', 'ceb'),
  (1180, 1, 'el', 'cs'),
  (1181, 1, 'el', 'cy'),
  (1182, 1, 'el', 'da'),
  (1183, 1, 'el', 'de'),
  (1184, 1, 'el', 'en'),
  (1185, 1, 'el', 'eo'),
  (1186, 1, 'el', 'es'),
  (1187, 1, 'el', 'et'),
  (1188, 1, 'el', 'eu'),
  (1189, 1, 'el', 'fa'),
  (1190, 1, 'el', 'fi'),
  (1191, 1, 'el', 'fr'),
  (1192, 1, 'el', 'ga'),
  (1193, 1, 'el', 'gl'),
  (1194, 1, 'el', 'gu'),
  (1195, 1, 'el', 'ha'),
  (1196, 1, 'el', 'hi'),
  (1197, 1, 'el', 'hmn'),
  (1198, 1, 'el', 'hr'),
  (1199, 1, 'el', 'ht'),
  (1200, 1, 'el', 'hu'),
  (1201, 1, 'el', 'hy'),
  (1202, 1, 'el', 'id'),
  (1203, 1, 'el', 'ig'),
  (1204, 1, 'el', 'is'),
  (1205, 1, 'el', 'it'),
  (1206, 1, 'el', 'iw'),
  (1207, 1, 'el', 'ja'),
  (1208, 1, 'el', 'jw'),
  (1209, 1, 'el', 'ka'),
  (1210, 1, 'el', 'kk'),
  (1211, 1, 'el', 'km'),
  (1212, 1, 'el', 'kn'),
  (1213, 1, 'el', 'ko'),
  (1214, 1, 'el', 'la'),
  (1215, 1, 'el', 'lo'),
  (1216, 1, 'el', 'lt'),
  (1217, 1, 'el', 'lv'),
  (1218, 1, 'el', 'mg'),
  (1219, 1, 'el', 'mi'),
  (1220, 1, 'el', 'mk'),
  (1221, 1, 'el', 'ml'),
  (1222, 1, 'el', 'mn'),
  (1223, 1, 'el', 'mr'),
  (1224, 1, 'el', 'ms'),
  (1225, 1, 'el', 'mt'),
  (1226, 1, 'el', 'my'),
  (1227, 1, 'el', 'ne'),
  (1228, 1, 'el', 'nl'),
  (1229, 1, 'el', 'no'),
  (1230, 1, 'el', 'ny'),
  (1231, 1, 'el', 'pa'),
  (1232, 1, 'el', 'pl'),
  (1233, 1, 'el', 'pt'),
  (1234, 1, 'el', 'ro'),
  (1235, 1, 'el', 'ru'),
  (1236, 1, 'el', 'si'),
  (1237, 1, 'el', 'sk'),
  (1238, 1, 'el', 'sl'),
  (1239, 1, 'el', 'so'),
  (1240, 1, 'el', 'sq'),
  (1241, 1, 'el', 'sr'),
  (1242, 1, 'el', 'st'),
  (1243, 1, 'el', 'su'),
  (1244, 1, 'el', 'sv'),
  (1245, 1, 'el', 'sw'),
  (1246, 1, 'el', 'ta'),
  (1247, 1, 'el', 'te'),
  (1248, 1, 'el', 'tg'),
  (1249, 1, 'el', 'th'),
  (1250, 1, 'el', 'tl'),
  (1251, 1, 'el', 'tr'),
  (1252, 1, 'el', 'uk'),
  (1253, 1, 'el', 'ur'),
  (1254, 1, 'el', 'uz'),
  (1255, 1, 'el', 'vi'),
  (1256, 1, 'el', 'yi'),
  (1257, 1, 'el', 'yo'),
  (1258, 1, 'el', 'zh'),
  (1259, 1, 'el', 'zh-TW'),
  (1260, 1, 'el', 'zu'),
  (1261, 1, 'en', 'af'),
  (1262, 1, 'en', 'ar'),
  (1263, 1, 'en', 'az'),
  (1264, 1, 'en', 'be'),
  (1265, 1, 'en', 'bg'),
  (1266, 1, 'en', 'bn'),
  (1267, 1, 'en', 'bs'),
  (1268, 1, 'en', 'ca'),
  (1269, 1, 'en', 'ceb'),
  (1270, 1, 'en', 'cs'),
  (1271, 1, 'en', 'cy'),
  (1272, 1, 'en', 'da'),
  (1273, 1, 'en', 'de'),
  (1274, 1, 'en', 'el'),
  (1275, 1, 'en', 'eo'),
  (1276, 1, 'en', 'es'),
  (1277, 1, 'en', 'et'),
  (1278, 1, 'en', 'eu'),
  (1279, 1, 'en', 'fa'),
  (1280, 1, 'en', 'fi'),
  (1281, 1, 'en', 'fr'),
  (1282, 1, 'en', 'ga'),
  (1283, 1, 'en', 'gl'),
  (1284, 1, 'en', 'gu'),
  (1285, 1, 'en', 'ha'),
  (1286, 1, 'en', 'hi'),
  (1287, 1, 'en', 'hmn'),
  (1288, 1, 'en', 'hr'),
  (1289, 1, 'en', 'ht'),
  (1290, 1, 'en', 'hu'),
  (1291, 1, 'en', 'hy'),
  (1292, 1, 'en', 'id'),
  (1293, 1, 'en', 'ig'),
  (1294, 1, 'en', 'is'),
  (1295, 1, 'en', 'it'),
  (1296, 1, 'en', 'iw'),
  (1297, 1, 'en', 'ja'),
  (1298, 1, 'en', 'jw'),
  (1299, 1, 'en', 'ka'),
  (1300, 1, 'en', 'kk'),
  (1301, 1, 'en', 'km'),
  (1302, 1, 'en', 'kn'),
  (1303, 1, 'en', 'ko'),
  (1304, 1, 'en', 'la'),
  (1305, 1, 'en', 'lo'),
  (1306, 1, 'en', 'lt'),
  (1307, 1, 'en', 'lv'),
  (1308, 1, 'en', 'mg'),
  (1309, 1, 'en', 'mi'),
  (1310, 1, 'en', 'mk'),
  (1311, 1, 'en', 'ml'),
  (1312, 1, 'en', 'mn'),
  (1313, 1, 'en', 'mr'),
  (1314, 1, 'en', 'ms'),
  (1315, 1, 'en', 'mt'),
  (1316, 1, 'en', 'my'),
  (1317, 1, 'en', 'ne'),
  (1318, 1, 'en', 'nl'),
  (1319, 1, 'en', 'no'),
  (1320, 1, 'en', 'ny'),
  (1321, 1, 'en', 'pa'),
  (1322, 1, 'en', 'pl'),
  (1323, 1, 'en', 'pt'),
  (1324, 1, 'en', 'ro'),
  (1325, 1, 'en', 'ru'),
  (1326, 1, 'en', 'si'),
  (1327, 1, 'en', 'sk'),
  (1328, 1, 'en', 'sl'),
  (1329, 1, 'en', 'so'),
  (1330, 1, 'en', 'sq'),
  (1331, 1, 'en', 'sr'),
  (1332, 1, 'en', 'st'),
  (1333, 1, 'en', 'su'),
  (1334, 1, 'en', 'sv'),
  (1335, 1, 'en', 'sw'),
  (1336, 1, 'en', 'ta'),
  (1337, 1, 'en', 'te'),
  (1338, 1, 'en', 'tg'),
  (1339, 1, 'en', 'th'),
  (1340, 1, 'en', 'tl'),
  (1341, 1, 'en', 'tr'),
  (1342, 1, 'en', 'uk'),
  (1343, 1, 'en', 'ur'),
  (1344, 1, 'en', 'uz'),
  (1345, 1, 'en', 'vi'),
  (1346, 1, 'en', 'yi'),
  (1347, 1, 'en', 'yo'),
  (1348, 1, 'en', 'zh'),
  (1349, 1, 'en', 'zh-TW'),
  (1350, 1, 'en', 'zu'),
  (1351, 1, 'eo', 'af'),
  (1352, 1, 'eo', 'ar'),
  (1353, 1, 'eo', 'az'),
  (1354, 1, 'eo', 'be'),
  (1355, 1, 'eo', 'bg'),
  (1356, 1, 'eo', 'bn'),
  (1357, 1, 'eo', 'bs'),
  (1358, 1, 'eo', 'ca'),
  (1359, 1, 'eo', 'ceb'),
  (1360, 1, 'eo', 'cs'),
  (1361, 1, 'eo', 'cy'),
  (1362, 1, 'eo', 'da'),
  (1363, 1, 'eo', 'de'),
  (1364, 1, 'eo', 'el'),
  (1365, 1, 'eo', 'en'),
  (1366, 1, 'eo', 'es'),
  (1367, 1, 'eo', 'et'),
  (1368, 1, 'eo', 'eu'),
  (1369, 1, 'eo', 'fa'),
  (1370, 1, 'eo', 'fi'),
  (1371, 1, 'eo', 'fr'),
  (1372, 1, 'eo', 'ga'),
  (1373, 1, 'eo', 'gl'),
  (1374, 1, 'eo', 'gu'),
  (1375, 1, 'eo', 'ha'),
  (1376, 1, 'eo', 'hi'),
  (1377, 1, 'eo', 'hmn'),
  (1378, 1, 'eo', 'hr'),
  (1379, 1, 'eo', 'ht'),
  (1380, 1, 'eo', 'hu'),
  (1381, 1, 'eo', 'hy'),
  (1382, 1, 'eo', 'id'),
  (1383, 1, 'eo', 'ig'),
  (1384, 1, 'eo', 'is'),
  (1385, 1, 'eo', 'it'),
  (1386, 1, 'eo', 'iw'),
  (1387, 1, 'eo', 'ja'),
  (1388, 1, 'eo', 'jw'),
  (1389, 1, 'eo', 'ka'),
  (1390, 1, 'eo', 'kk'),
  (1391, 1, 'eo', 'km'),
  (1392, 1, 'eo', 'kn'),
  (1393, 1, 'eo', 'ko'),
  (1394, 1, 'eo', 'la'),
  (1395, 1, 'eo', 'lo'),
  (1396, 1, 'eo', 'lt'),
  (1397, 1, 'eo', 'lv'),
  (1398, 1, 'eo', 'mg'),
  (1399, 1, 'eo', 'mi'),
  (1400, 1, 'eo', 'mk'),
  (1401, 1, 'eo', 'ml'),
  (1402, 1, 'eo', 'mn'),
  (1403, 1, 'eo', 'mr'),
  (1404, 1, 'eo', 'ms'),
  (1405, 1, 'eo', 'mt'),
  (1406, 1, 'eo', 'my'),
  (1407, 1, 'eo', 'ne'),
  (1408, 1, 'eo', 'nl'),
  (1409, 1, 'eo', 'no'),
  (1410, 1, 'eo', 'ny'),
  (1411, 1, 'eo', 'pa'),
  (1412, 1, 'eo', 'pl'),
  (1413, 1, 'eo', 'pt'),
  (1414, 1, 'eo', 'ro'),
  (1415, 1, 'eo', 'ru'),
  (1416, 1, 'eo', 'si'),
  (1417, 1, 'eo', 'sk'),
  (1418, 1, 'eo', 'sl'),
  (1419, 1, 'eo', 'so'),
  (1420, 1, 'eo', 'sq'),
  (1421, 1, 'eo', 'sr'),
  (1422, 1, 'eo', 'st'),
  (1423, 1, 'eo', 'su'),
  (1424, 1, 'eo', 'sv'),
  (1425, 1, 'eo', 'sw'),
  (1426, 1, 'eo', 'ta'),
  (1427, 1, 'eo', 'te'),
  (1428, 1, 'eo', 'tg'),
  (1429, 1, 'eo', 'th'),
  (1430, 1, 'eo', 'tl'),
  (1431, 1, 'eo', 'tr'),
  (1432, 1, 'eo', 'uk'),
  (1433, 1, 'eo', 'ur'),
  (1434, 1, 'eo', 'uz'),
  (1435, 1, 'eo', 'vi'),
  (1436, 1, 'eo', 'yi'),
  (1437, 1, 'eo', 'yo'),
  (1438, 1, 'eo', 'zh'),
  (1439, 1, 'eo', 'zh-TW'),
  (1440, 1, 'eo', 'zu'),
  (1441, 1, 'es', 'af'),
  (1442, 1, 'es', 'ar'),
  (1443, 1, 'es', 'az'),
  (1444, 1, 'es', 'be'),
  (1445, 1, 'es', 'bg'),
  (1446, 1, 'es', 'bn'),
  (1447, 1, 'es', 'bs'),
  (1448, 1, 'es', 'ca'),
  (1449, 1, 'es', 'ceb'),
  (1450, 1, 'es', 'cs'),
  (1451, 1, 'es', 'cy'),
  (1452, 1, 'es', 'da'),
  (1453, 1, 'es', 'de'),
  (1454, 1, 'es', 'el'),
  (1455, 1, 'es', 'en'),
  (1456, 1, 'es', 'eo'),
  (1457, 1, 'es', 'et'),
  (1458, 1, 'es', 'eu'),
  (1459, 1, 'es', 'fa'),
  (1460, 1, 'es', 'fi'),
  (1461, 1, 'es', 'fr'),
  (1462, 1, 'es', 'ga'),
  (1463, 1, 'es', 'gl'),
  (1464, 1, 'es', 'gu'),
  (1465, 1, 'es', 'ha'),
  (1466, 1, 'es', 'hi'),
  (1467, 1, 'es', 'hmn'),
  (1468, 1, 'es', 'hr'),
  (1469, 1, 'es', 'ht'),
  (1470, 1, 'es', 'hu'),
  (1471, 1, 'es', 'hy'),
  (1472, 1, 'es', 'id'),
  (1473, 1, 'es', 'ig'),
  (1474, 1, 'es', 'is'),
  (1475, 1, 'es', 'it'),
  (1476, 1, 'es', 'iw'),
  (1477, 1, 'es', 'ja'),
  (1478, 1, 'es', 'jw'),
  (1479, 1, 'es', 'ka'),
  (1480, 1, 'es', 'kk'),
  (1481, 1, 'es', 'km'),
  (1482, 1, 'es', 'kn'),
  (1483, 1, 'es', 'ko'),
  (1484, 1, 'es', 'la'),
  (1485, 1, 'es', 'lo'),
  (1486, 1, 'es', 'lt'),
  (1487, 1, 'es', 'lv'),
  (1488, 1, 'es', 'mg'),
  (1489, 1, 'es', 'mi'),
  (1490, 1, 'es', 'mk'),
  (1491, 1, 'es', 'ml'),
  (1492, 1, 'es', 'mn'),
  (1493, 1, 'es', 'mr'),
  (1494, 1, 'es', 'ms'),
  (1495, 1, 'es', 'mt'),
  (1496, 1, 'es', 'my'),
  (1497, 1, 'es', 'ne'),
  (1498, 1, 'es', 'nl'),
  (1499, 1, 'es', 'no'),
  (1500, 1, 'es', 'ny'),
  (1501, 1, 'es', 'pa'),
  (1502, 1, 'es', 'pl'),
  (1503, 1, 'es', 'pt'),
  (1504, 1, 'es', 'ro'),
  (1505, 1, 'es', 'ru'),
  (1506, 1, 'es', 'si'),
  (1507, 1, 'es', 'sk'),
  (1508, 1, 'es', 'sl'),
  (1509, 1, 'es', 'so'),
  (1510, 1, 'es', 'sq'),
  (1511, 1, 'es', 'sr'),
  (1512, 1, 'es', 'st'),
  (1513, 1, 'es', 'su'),
  (1514, 1, 'es', 'sv'),
  (1515, 1, 'es', 'sw'),
  (1516, 1, 'es', 'ta'),
  (1517, 1, 'es', 'te'),
  (1518, 1, 'es', 'tg'),
  (1519, 1, 'es', 'th'),
  (1520, 1, 'es', 'tl'),
  (1521, 1, 'es', 'tr'),
  (1522, 1, 'es', 'uk'),
  (1523, 1, 'es', 'ur'),
  (1524, 1, 'es', 'uz'),
  (1525, 1, 'es', 'vi'),
  (1526, 1, 'es', 'yi'),
  (1527, 1, 'es', 'yo'),
  (1528, 1, 'es', 'zh'),
  (1529, 1, 'es', 'zh-TW'),
  (1530, 1, 'es', 'zu'),
  (1531, 1, 'et', 'af'),
  (1532, 1, 'et', 'ar'),
  (1533, 1, 'et', 'az'),
  (1534, 1, 'et', 'be'),
  (1535, 1, 'et', 'bg'),
  (1536, 1, 'et', 'bn'),
  (1537, 1, 'et', 'bs'),
  (1538, 1, 'et', 'ca'),
  (1539, 1, 'et', 'ceb'),
  (1540, 1, 'et', 'cs'),
  (1541, 1, 'et', 'cy'),
  (1542, 1, 'et', 'da'),
  (1543, 1, 'et', 'de'),
  (1544, 1, 'et', 'el'),
  (1545, 1, 'et', 'en'),
  (1546, 1, 'et', 'eo'),
  (1547, 1, 'et', 'es'),
  (1548, 1, 'et', 'eu'),
  (1549, 1, 'et', 'fa'),
  (1550, 1, 'et', 'fi'),
  (1551, 1, 'et', 'fr'),
  (1552, 1, 'et', 'ga'),
  (1553, 1, 'et', 'gl'),
  (1554, 1, 'et', 'gu'),
  (1555, 1, 'et', 'ha'),
  (1556, 1, 'et', 'hi'),
  (1557, 1, 'et', 'hmn'),
  (1558, 1, 'et', 'hr'),
  (1559, 1, 'et', 'ht'),
  (1560, 1, 'et', 'hu'),
  (1561, 1, 'et', 'hy'),
  (1562, 1, 'et', 'id'),
  (1563, 1, 'et', 'ig'),
  (1564, 1, 'et', 'is'),
  (1565, 1, 'et', 'it'),
  (1566, 1, 'et', 'iw'),
  (1567, 1, 'et', 'ja'),
  (1568, 1, 'et', 'jw'),
  (1569, 1, 'et', 'ka'),
  (1570, 1, 'et', 'kk'),
  (1571, 1, 'et', 'km'),
  (1572, 1, 'et', 'kn'),
  (1573, 1, 'et', 'ko'),
  (1574, 1, 'et', 'la'),
  (1575, 1, 'et', 'lo'),
  (1576, 1, 'et', 'lt'),
  (1577, 1, 'et', 'lv'),
  (1578, 1, 'et', 'mg'),
  (1579, 1, 'et', 'mi'),
  (1580, 1, 'et', 'mk'),
  (1581, 1, 'et', 'ml'),
  (1582, 1, 'et', 'mn'),
  (1583, 1, 'et', 'mr'),
  (1584, 1, 'et', 'ms'),
  (1585, 1, 'et', 'mt'),
  (1586, 1, 'et', 'my'),
  (1587, 1, 'et', 'ne'),
  (1588, 1, 'et', 'nl'),
  (1589, 1, 'et', 'no'),
  (1590, 1, 'et', 'ny'),
  (1591, 1, 'et', 'pa'),
  (1592, 1, 'et', 'pl'),
  (1593, 1, 'et', 'pt'),
  (1594, 1, 'et', 'ro'),
  (1595, 1, 'et', 'ru'),
  (1596, 1, 'et', 'si'),
  (1597, 1, 'et', 'sk'),
  (1598, 1, 'et', 'sl'),
  (1599, 1, 'et', 'so'),
  (1600, 1, 'et', 'sq'),
  (1601, 1, 'et', 'sr'),
  (1602, 1, 'et', 'st'),
  (1603, 1, 'et', 'su'),
  (1604, 1, 'et', 'sv'),
  (1605, 1, 'et', 'sw'),
  (1606, 1, 'et', 'ta'),
  (1607, 1, 'et', 'te'),
  (1608, 1, 'et', 'tg'),
  (1609, 1, 'et', 'th'),
  (1610, 1, 'et', 'tl'),
  (1611, 1, 'et', 'tr'),
  (1612, 1, 'et', 'uk'),
  (1613, 1, 'et', 'ur'),
  (1614, 1, 'et', 'uz'),
  (1615, 1, 'et', 'vi'),
  (1616, 1, 'et', 'yi'),
  (1617, 1, 'et', 'yo'),
  (1618, 1, 'et', 'zh'),
  (1619, 1, 'et', 'zh-TW'),
  (1620, 1, 'et', 'zu'),
  (1621, 1, 'eu', 'af'),
  (1622, 1, 'eu', 'ar'),
  (1623, 1, 'eu', 'az'),
  (1624, 1, 'eu', 'be'),
  (1625, 1, 'eu', 'bg'),
  (1626, 1, 'eu', 'bn'),
  (1627, 1, 'eu', 'bs'),
  (1628, 1, 'eu', 'ca'),
  (1629, 1, 'eu', 'ceb'),
  (1630, 1, 'eu', 'cs'),
  (1631, 1, 'eu', 'cy'),
  (1632, 1, 'eu', 'da'),
  (1633, 1, 'eu', 'de'),
  (1634, 1, 'eu', 'el'),
  (1635, 1, 'eu', 'en'),
  (1636, 1, 'eu', 'eo'),
  (1637, 1, 'eu', 'es'),
  (1638, 1, 'eu', 'et'),
  (1639, 1, 'eu', 'fa'),
  (1640, 1, 'eu', 'fi'),
  (1641, 1, 'eu', 'fr'),
  (1642, 1, 'eu', 'ga'),
  (1643, 1, 'eu', 'gl'),
  (1644, 1, 'eu', 'gu'),
  (1645, 1, 'eu', 'ha'),
  (1646, 1, 'eu', 'hi'),
  (1647, 1, 'eu', 'hmn'),
  (1648, 1, 'eu', 'hr'),
  (1649, 1, 'eu', 'ht'),
  (1650, 1, 'eu', 'hu'),
  (1651, 1, 'eu', 'hy'),
  (1652, 1, 'eu', 'id'),
  (1653, 1, 'eu', 'ig'),
  (1654, 1, 'eu', 'is'),
  (1655, 1, 'eu', 'it'),
  (1656, 1, 'eu', 'iw'),
  (1657, 1, 'eu', 'ja'),
  (1658, 1, 'eu', 'jw'),
  (1659, 1, 'eu', 'ka'),
  (1660, 1, 'eu', 'kk'),
  (1661, 1, 'eu', 'km'),
  (1662, 1, 'eu', 'kn'),
  (1663, 1, 'eu', 'ko'),
  (1664, 1, 'eu', 'la'),
  (1665, 1, 'eu', 'lo'),
  (1666, 1, 'eu', 'lt'),
  (1667, 1, 'eu', 'lv'),
  (1668, 1, 'eu', 'mg'),
  (1669, 1, 'eu', 'mi'),
  (1670, 1, 'eu', 'mk'),
  (1671, 1, 'eu', 'ml'),
  (1672, 1, 'eu', 'mn'),
  (1673, 1, 'eu', 'mr'),
  (1674, 1, 'eu', 'ms'),
  (1675, 1, 'eu', 'mt'),
  (1676, 1, 'eu', 'my'),
  (1677, 1, 'eu', 'ne'),
  (1678, 1, 'eu', 'nl'),
  (1679, 1, 'eu', 'no'),
  (1680, 1, 'eu', 'ny'),
  (1681, 1, 'eu', 'pa'),
  (1682, 1, 'eu', 'pl'),
  (1683, 1, 'eu', 'pt'),
  (1684, 1, 'eu', 'ro'),
  (1685, 1, 'eu', 'ru'),
  (1686, 1, 'eu', 'si'),
  (1687, 1, 'eu', 'sk'),
  (1688, 1, 'eu', 'sl'),
  (1689, 1, 'eu', 'so'),
  (1690, 1, 'eu', 'sq'),
  (1691, 1, 'eu', 'sr'),
  (1692, 1, 'eu', 'st'),
  (1693, 1, 'eu', 'su'),
  (1694, 1, 'eu', 'sv'),
  (1695, 1, 'eu', 'sw'),
  (1696, 1, 'eu', 'ta'),
  (1697, 1, 'eu', 'te'),
  (1698, 1, 'eu', 'tg'),
  (1699, 1, 'eu', 'th'),
  (1700, 1, 'eu', 'tl'),
  (1701, 1, 'eu', 'tr'),
  (1702, 1, 'eu', 'uk'),
  (1703, 1, 'eu', 'ur'),
  (1704, 1, 'eu', 'uz'),
  (1705, 1, 'eu', 'vi'),
  (1706, 1, 'eu', 'yi'),
  (1707, 1, 'eu', 'yo'),
  (1708, 1, 'eu', 'zh'),
  (1709, 1, 'eu', 'zh-TW'),
  (1710, 1, 'eu', 'zu'),
  (1711, 1, 'fa', 'af'),
  (1712, 1, 'fa', 'ar'),
  (1713, 1, 'fa', 'az'),
  (1714, 1, 'fa', 'be'),
  (1715, 1, 'fa', 'bg'),
  (1716, 1, 'fa', 'bn'),
  (1717, 1, 'fa', 'bs'),
  (1718, 1, 'fa', 'ca'),
  (1719, 1, 'fa', 'ceb'),
  (1720, 1, 'fa', 'cs'),
  (1721, 1, 'fa', 'cy'),
  (1722, 1, 'fa', 'da'),
  (1723, 1, 'fa', 'de'),
  (1724, 1, 'fa', 'el'),
  (1725, 1, 'fa', 'en'),
  (1726, 1, 'fa', 'eo'),
  (1727, 1, 'fa', 'es'),
  (1728, 1, 'fa', 'et'),
  (1729, 1, 'fa', 'eu'),
  (1730, 1, 'fa', 'fi'),
  (1731, 1, 'fa', 'fr'),
  (1732, 1, 'fa', 'ga'),
  (1733, 1, 'fa', 'gl'),
  (1734, 1, 'fa', 'gu'),
  (1735, 1, 'fa', 'ha'),
  (1736, 1, 'fa', 'hi'),
  (1737, 1, 'fa', 'hmn'),
  (1738, 1, 'fa', 'hr'),
  (1739, 1, 'fa', 'ht'),
  (1740, 1, 'fa', 'hu'),
  (1741, 1, 'fa', 'hy'),
  (1742, 1, 'fa', 'id'),
  (1743, 1, 'fa', 'ig'),
  (1744, 1, 'fa', 'is'),
  (1745, 1, 'fa', 'it'),
  (1746, 1, 'fa', 'iw'),
  (1747, 1, 'fa', 'ja'),
  (1748, 1, 'fa', 'jw'),
  (1749, 1, 'fa', 'ka'),
  (1750, 1, 'fa', 'kk'),
  (1751, 1, 'fa', 'km'),
  (1752, 1, 'fa', 'kn'),
  (1753, 1, 'fa', 'ko'),
  (1754, 1, 'fa', 'la'),
  (1755, 1, 'fa', 'lo'),
  (1756, 1, 'fa', 'lt'),
  (1757, 1, 'fa', 'lv'),
  (1758, 1, 'fa', 'mg'),
  (1759, 1, 'fa', 'mi'),
  (1760, 1, 'fa', 'mk'),
  (1761, 1, 'fa', 'ml'),
  (1762, 1, 'fa', 'mn'),
  (1763, 1, 'fa', 'mr'),
  (1764, 1, 'fa', 'ms'),
  (1765, 1, 'fa', 'mt'),
  (1766, 1, 'fa', 'my'),
  (1767, 1, 'fa', 'ne'),
  (1768, 1, 'fa', 'nl'),
  (1769, 1, 'fa', 'no'),
  (1770, 1, 'fa', 'ny'),
  (1771, 1, 'fa', 'pa'),
  (1772, 1, 'fa', 'pl'),
  (1773, 1, 'fa', 'pt'),
  (1774, 1, 'fa', 'ro'),
  (1775, 1, 'fa', 'ru'),
  (1776, 1, 'fa', 'si'),
  (1777, 1, 'fa', 'sk'),
  (1778, 1, 'fa', 'sl'),
  (1779, 1, 'fa', 'so'),
  (1780, 1, 'fa', 'sq'),
  (1781, 1, 'fa', 'sr'),
  (1782, 1, 'fa', 'st'),
  (1783, 1, 'fa', 'su'),
  (1784, 1, 'fa', 'sv'),
  (1785, 1, 'fa', 'sw'),
  (1786, 1, 'fa', 'ta'),
  (1787, 1, 'fa', 'te'),
  (1788, 1, 'fa', 'tg'),
  (1789, 1, 'fa', 'th'),
  (1790, 1, 'fa', 'tl'),
  (1791, 1, 'fa', 'tr'),
  (1792, 1, 'fa', 'uk'),
  (1793, 1, 'fa', 'ur'),
  (1794, 1, 'fa', 'uz'),
  (1795, 1, 'fa', 'vi'),
  (1796, 1, 'fa', 'yi'),
  (1797, 1, 'fa', 'yo'),
  (1798, 1, 'fa', 'zh'),
  (1799, 1, 'fa', 'zh-TW'),
  (1800, 1, 'fa', 'zu'),
  (1801, 1, 'fi', 'af'),
  (1802, 1, 'fi', 'ar'),
  (1803, 1, 'fi', 'az'),
  (1804, 1, 'fi', 'be'),
  (1805, 1, 'fi', 'bg'),
  (1806, 1, 'fi', 'bn'),
  (1807, 1, 'fi', 'bs'),
  (1808, 1, 'fi', 'ca'),
  (1809, 1, 'fi', 'ceb'),
  (1810, 1, 'fi', 'cs'),
  (1811, 1, 'fi', 'cy'),
  (1812, 1, 'fi', 'da'),
  (1813, 1, 'fi', 'de'),
  (1814, 1, 'fi', 'el'),
  (1815, 1, 'fi', 'en'),
  (1816, 1, 'fi', 'eo'),
  (1817, 1, 'fi', 'es'),
  (1818, 1, 'fi', 'et'),
  (1819, 1, 'fi', 'eu'),
  (1820, 1, 'fi', 'fa'),
  (1821, 1, 'fi', 'fr'),
  (1822, 1, 'fi', 'ga'),
  (1823, 1, 'fi', 'gl'),
  (1824, 1, 'fi', 'gu'),
  (1825, 1, 'fi', 'ha'),
  (1826, 1, 'fi', 'hi'),
  (1827, 1, 'fi', 'hmn'),
  (1828, 1, 'fi', 'hr'),
  (1829, 1, 'fi', 'ht'),
  (1830, 1, 'fi', 'hu'),
  (1831, 1, 'fi', 'hy'),
  (1832, 1, 'fi', 'id'),
  (1833, 1, 'fi', 'ig'),
  (1834, 1, 'fi', 'is'),
  (1835, 1, 'fi', 'it'),
  (1836, 1, 'fi', 'iw'),
  (1837, 1, 'fi', 'ja'),
  (1838, 1, 'fi', 'jw'),
  (1839, 1, 'fi', 'ka'),
  (1840, 1, 'fi', 'kk'),
  (1841, 1, 'fi', 'km'),
  (1842, 1, 'fi', 'kn'),
  (1843, 1, 'fi', 'ko'),
  (1844, 1, 'fi', 'la'),
  (1845, 1, 'fi', 'lo'),
  (1846, 1, 'fi', 'lt'),
  (1847, 1, 'fi', 'lv'),
  (1848, 1, 'fi', 'mg'),
  (1849, 1, 'fi', 'mi'),
  (1850, 1, 'fi', 'mk'),
  (1851, 1, 'fi', 'ml'),
  (1852, 1, 'fi', 'mn'),
  (1853, 1, 'fi', 'mr'),
  (1854, 1, 'fi', 'ms'),
  (1855, 1, 'fi', 'mt'),
  (1856, 1, 'fi', 'my'),
  (1857, 1, 'fi', 'ne'),
  (1858, 1, 'fi', 'nl'),
  (1859, 1, 'fi', 'no'),
  (1860, 1, 'fi', 'ny'),
  (1861, 1, 'fi', 'pa'),
  (1862, 1, 'fi', 'pl'),
  (1863, 1, 'fi', 'pt'),
  (1864, 1, 'fi', 'ro'),
  (1865, 1, 'fi', 'ru'),
  (1866, 1, 'fi', 'si'),
  (1867, 1, 'fi', 'sk'),
  (1868, 1, 'fi', 'sl'),
  (1869, 1, 'fi', 'so'),
  (1870, 1, 'fi', 'sq'),
  (1871, 1, 'fi', 'sr'),
  (1872, 1, 'fi', 'st'),
  (1873, 1, 'fi', 'su'),
  (1874, 1, 'fi', 'sv'),
  (1875, 1, 'fi', 'sw'),
  (1876, 1, 'fi', 'ta'),
  (1877, 1, 'fi', 'te'),
  (1878, 1, 'fi', 'tg'),
  (1879, 1, 'fi', 'th'),
  (1880, 1, 'fi', 'tl'),
  (1881, 1, 'fi', 'tr'),
  (1882, 1, 'fi', 'uk'),
  (1883, 1, 'fi', 'ur'),
  (1884, 1, 'fi', 'uz'),
  (1885, 1, 'fi', 'vi'),
  (1886, 1, 'fi', 'yi'),
  (1887, 1, 'fi', 'yo'),
  (1888, 1, 'fi', 'zh'),
  (1889, 1, 'fi', 'zh-TW'),
  (1890, 1, 'fi', 'zu'),
  (1891, 1, 'fr', 'af'),
  (1892, 1, 'fr', 'ar'),
  (1893, 1, 'fr', 'az'),
  (1894, 1, 'fr', 'be'),
  (1895, 1, 'fr', 'bg'),
  (1896, 1, 'fr', 'bn'),
  (1897, 1, 'fr', 'bs'),
  (1898, 1, 'fr', 'ca'),
  (1899, 1, 'fr', 'ceb'),
  (1900, 1, 'fr', 'cs'),
  (1901, 1, 'fr', 'cy'),
  (1902, 1, 'fr', 'da'),
  (1903, 1, 'fr', 'de'),
  (1904, 1, 'fr', 'el'),
  (1905, 1, 'fr', 'en'),
  (1906, 1, 'fr', 'eo'),
  (1907, 1, 'fr', 'es'),
  (1908, 1, 'fr', 'et'),
  (1909, 1, 'fr', 'eu'),
  (1910, 1, 'fr', 'fa'),
  (1911, 1, 'fr', 'fi'),
  (1912, 1, 'fr', 'ga'),
  (1913, 1, 'fr', 'gl'),
  (1914, 1, 'fr', 'gu'),
  (1915, 1, 'fr', 'ha'),
  (1916, 1, 'fr', 'hi'),
  (1917, 1, 'fr', 'hmn'),
  (1918, 1, 'fr', 'hr'),
  (1919, 1, 'fr', 'ht'),
  (1920, 1, 'fr', 'hu'),
  (1921, 1, 'fr', 'hy'),
  (1922, 1, 'fr', 'id'),
  (1923, 1, 'fr', 'ig'),
  (1924, 1, 'fr', 'is'),
  (1925, 1, 'fr', 'it'),
  (1926, 1, 'fr', 'iw'),
  (1927, 1, 'fr', 'ja'),
  (1928, 1, 'fr', 'jw'),
  (1929, 1, 'fr', 'ka'),
  (1930, 1, 'fr', 'kk'),
  (1931, 1, 'fr', 'km'),
  (1932, 1, 'fr', 'kn'),
  (1933, 1, 'fr', 'ko'),
  (1934, 1, 'fr', 'la'),
  (1935, 1, 'fr', 'lo'),
  (1936, 1, 'fr', 'lt'),
  (1937, 1, 'fr', 'lv'),
  (1938, 1, 'fr', 'mg'),
  (1939, 1, 'fr', 'mi'),
  (1940, 1, 'fr', 'mk'),
  (1941, 1, 'fr', 'ml'),
  (1942, 1, 'fr', 'mn'),
  (1943, 1, 'fr', 'mr'),
  (1944, 1, 'fr', 'ms'),
  (1945, 1, 'fr', 'mt'),
  (1946, 1, 'fr', 'my'),
  (1947, 1, 'fr', 'ne'),
  (1948, 1, 'fr', 'nl'),
  (1949, 1, 'fr', 'no'),
  (1950, 1, 'fr', 'ny'),
  (1951, 1, 'fr', 'pa'),
  (1952, 1, 'fr', 'pl'),
  (1953, 1, 'fr', 'pt'),
  (1954, 1, 'fr', 'ro'),
  (1955, 1, 'fr', 'ru'),
  (1956, 1, 'fr', 'si'),
  (1957, 1, 'fr', 'sk'),
  (1958, 1, 'fr', 'sl'),
  (1959, 1, 'fr', 'so'),
  (1960, 1, 'fr', 'sq'),
  (1961, 1, 'fr', 'sr'),
  (1962, 1, 'fr', 'st'),
  (1963, 1, 'fr', 'su'),
  (1964, 1, 'fr', 'sv'),
  (1965, 1, 'fr', 'sw'),
  (1966, 1, 'fr', 'ta'),
  (1967, 1, 'fr', 'te'),
  (1968, 1, 'fr', 'tg'),
  (1969, 1, 'fr', 'th'),
  (1970, 1, 'fr', 'tl'),
  (1971, 1, 'fr', 'tr'),
  (1972, 1, 'fr', 'uk'),
  (1973, 1, 'fr', 'ur'),
  (1974, 1, 'fr', 'uz'),
  (1975, 1, 'fr', 'vi'),
  (1976, 1, 'fr', 'yi'),
  (1977, 1, 'fr', 'yo'),
  (1978, 1, 'fr', 'zh'),
  (1979, 1, 'fr', 'zh-TW'),
  (1980, 1, 'fr', 'zu'),
  (1981, 1, 'ga', 'af'),
  (1982, 1, 'ga', 'ar'),
  (1983, 1, 'ga', 'az'),
  (1984, 1, 'ga', 'be'),
  (1985, 1, 'ga', 'bg'),
  (1986, 1, 'ga', 'bn'),
  (1987, 1, 'ga', 'bs'),
  (1988, 1, 'ga', 'ca'),
  (1989, 1, 'ga', 'ceb'),
  (1990, 1, 'ga', 'cs'),
  (1991, 1, 'ga', 'cy'),
  (1992, 1, 'ga', 'da'),
  (1993, 1, 'ga', 'de'),
  (1994, 1, 'ga', 'el'),
  (1995, 1, 'ga', 'en'),
  (1996, 1, 'ga', 'eo'),
  (1997, 1, 'ga', 'es'),
  (1998, 1, 'ga', 'et'),
  (1999, 1, 'ga', 'eu'),
  (2000, 1, 'ga', 'fa'),
  (2001, 1, 'ga', 'fi'),
  (2002, 1, 'ga', 'fr'),
  (2003, 1, 'ga', 'gl'),
  (2004, 1, 'ga', 'gu'),
  (2005, 1, 'ga', 'ha'),
  (2006, 1, 'ga', 'hi'),
  (2007, 1, 'ga', 'hmn'),
  (2008, 1, 'ga', 'hr'),
  (2009, 1, 'ga', 'ht'),
  (2010, 1, 'ga', 'hu'),
  (2011, 1, 'ga', 'hy'),
  (2012, 1, 'ga', 'id'),
  (2013, 1, 'ga', 'ig'),
  (2014, 1, 'ga', 'is'),
  (2015, 1, 'ga', 'it'),
  (2016, 1, 'ga', 'iw'),
  (2017, 1, 'ga', 'ja'),
  (2018, 1, 'ga', 'jw'),
  (2019, 1, 'ga', 'ka'),
  (2020, 1, 'ga', 'kk'),
  (2021, 1, 'ga', 'km'),
  (2022, 1, 'ga', 'kn'),
  (2023, 1, 'ga', 'ko'),
  (2024, 1, 'ga', 'la'),
  (2025, 1, 'ga', 'lo'),
  (2026, 1, 'ga', 'lt'),
  (2027, 1, 'ga', 'lv'),
  (2028, 1, 'ga', 'mg'),
  (2029, 1, 'ga', 'mi'),
  (2030, 1, 'ga', 'mk'),
  (2031, 1, 'ga', 'ml'),
  (2032, 1, 'ga', 'mn'),
  (2033, 1, 'ga', 'mr'),
  (2034, 1, 'ga', 'ms'),
  (2035, 1, 'ga', 'mt'),
  (2036, 1, 'ga', 'my'),
  (2037, 1, 'ga', 'ne'),
  (2038, 1, 'ga', 'nl'),
  (2039, 1, 'ga', 'no'),
  (2040, 1, 'ga', 'ny'),
  (2041, 1, 'ga', 'pa'),
  (2042, 1, 'ga', 'pl'),
  (2043, 1, 'ga', 'pt'),
  (2044, 1, 'ga', 'ro'),
  (2045, 1, 'ga', 'ru'),
  (2046, 1, 'ga', 'si'),
  (2047, 1, 'ga', 'sk'),
  (2048, 1, 'ga', 'sl'),
  (2049, 1, 'ga', 'so'),
  (2050, 1, 'ga', 'sq'),
  (2051, 1, 'ga', 'sr'),
  (2052, 1, 'ga', 'st'),
  (2053, 1, 'ga', 'su'),
  (2054, 1, 'ga', 'sv'),
  (2055, 1, 'ga', 'sw'),
  (2056, 1, 'ga', 'ta'),
  (2057, 1, 'ga', 'te'),
  (2058, 1, 'ga', 'tg'),
  (2059, 1, 'ga', 'th'),
  (2060, 1, 'ga', 'tl'),
  (2061, 1, 'ga', 'tr'),
  (2062, 1, 'ga', 'uk'),
  (2063, 1, 'ga', 'ur'),
  (2064, 1, 'ga', 'uz'),
  (2065, 1, 'ga', 'vi'),
  (2066, 1, 'ga', 'yi'),
  (2067, 1, 'ga', 'yo'),
  (2068, 1, 'ga', 'zh'),
  (2069, 1, 'ga', 'zh-TW'),
  (2070, 1, 'ga', 'zu'),
  (2071, 1, 'gl', 'af'),
  (2072, 1, 'gl', 'ar'),
  (2073, 1, 'gl', 'az'),
  (2074, 1, 'gl', 'be'),
  (2075, 1, 'gl', 'bg'),
  (2076, 1, 'gl', 'bn'),
  (2077, 1, 'gl', 'bs'),
  (2078, 1, 'gl', 'ca'),
  (2079, 1, 'gl', 'ceb'),
  (2080, 1, 'gl', 'cs'),
  (2081, 1, 'gl', 'cy'),
  (2082, 1, 'gl', 'da'),
  (2083, 1, 'gl', 'de'),
  (2084, 1, 'gl', 'el'),
  (2085, 1, 'gl', 'en'),
  (2086, 1, 'gl', 'eo'),
  (2087, 1, 'gl', 'es'),
  (2088, 1, 'gl', 'et'),
  (2089, 1, 'gl', 'eu'),
  (2090, 1, 'gl', 'fa'),
  (2091, 1, 'gl', 'fi'),
  (2092, 1, 'gl', 'fr'),
  (2093, 1, 'gl', 'ga'),
  (2094, 1, 'gl', 'gu'),
  (2095, 1, 'gl', 'ha'),
  (2096, 1, 'gl', 'hi'),
  (2097, 1, 'gl', 'hmn'),
  (2098, 1, 'gl', 'hr'),
  (2099, 1, 'gl', 'ht'),
  (2100, 1, 'gl', 'hu'),
  (2101, 1, 'gl', 'hy'),
  (2102, 1, 'gl', 'id'),
  (2103, 1, 'gl', 'ig'),
  (2104, 1, 'gl', 'is'),
  (2105, 1, 'gl', 'it'),
  (2106, 1, 'gl', 'iw'),
  (2107, 1, 'gl', 'ja'),
  (2108, 1, 'gl', 'jw'),
  (2109, 1, 'gl', 'ka'),
  (2110, 1, 'gl', 'kk'),
  (2111, 1, 'gl', 'km'),
  (2112, 1, 'gl', 'kn'),
  (2113, 1, 'gl', 'ko'),
  (2114, 1, 'gl', 'la'),
  (2115, 1, 'gl', 'lo'),
  (2116, 1, 'gl', 'lt'),
  (2117, 1, 'gl', 'lv'),
  (2118, 1, 'gl', 'mg'),
  (2119, 1, 'gl', 'mi'),
  (2120, 1, 'gl', 'mk'),
  (2121, 1, 'gl', 'ml'),
  (2122, 1, 'gl', 'mn'),
  (2123, 1, 'gl', 'mr'),
  (2124, 1, 'gl', 'ms'),
  (2125, 1, 'gl', 'mt'),
  (2126, 1, 'gl', 'my'),
  (2127, 1, 'gl', 'ne'),
  (2128, 1, 'gl', 'nl'),
  (2129, 1, 'gl', 'no'),
  (2130, 1, 'gl', 'ny'),
  (2131, 1, 'gl', 'pa'),
  (2132, 1, 'gl', 'pl'),
  (2133, 1, 'gl', 'pt'),
  (2134, 1, 'gl', 'ro'),
  (2135, 1, 'gl', 'ru'),
  (2136, 1, 'gl', 'si'),
  (2137, 1, 'gl', 'sk'),
  (2138, 1, 'gl', 'sl'),
  (2139, 1, 'gl', 'so'),
  (2140, 1, 'gl', 'sq'),
  (2141, 1, 'gl', 'sr'),
  (2142, 1, 'gl', 'st'),
  (2143, 1, 'gl', 'su'),
  (2144, 1, 'gl', 'sv'),
  (2145, 1, 'gl', 'sw'),
  (2146, 1, 'gl', 'ta'),
  (2147, 1, 'gl', 'te'),
  (2148, 1, 'gl', 'tg'),
  (2149, 1, 'gl', 'th'),
  (2150, 1, 'gl', 'tl'),
  (2151, 1, 'gl', 'tr'),
  (2152, 1, 'gl', 'uk'),
  (2153, 1, 'gl', 'ur'),
  (2154, 1, 'gl', 'uz'),
  (2155, 1, 'gl', 'vi'),
  (2156, 1, 'gl', 'yi'),
  (2157, 1, 'gl', 'yo'),
  (2158, 1, 'gl', 'zh'),
  (2159, 1, 'gl', 'zh-TW'),
  (2160, 1, 'gl', 'zu'),
  (2161, 1, 'gu', 'af'),
  (2162, 1, 'gu', 'ar'),
  (2163, 1, 'gu', 'az'),
  (2164, 1, 'gu', 'be'),
  (2165, 1, 'gu', 'bg'),
  (2166, 1, 'gu', 'bn'),
  (2167, 1, 'gu', 'bs'),
  (2168, 1, 'gu', 'ca'),
  (2169, 1, 'gu', 'ceb'),
  (2170, 1, 'gu', 'cs'),
  (2171, 1, 'gu', 'cy'),
  (2172, 1, 'gu', 'da'),
  (2173, 1, 'gu', 'de'),
  (2174, 1, 'gu', 'el'),
  (2175, 1, 'gu', 'en'),
  (2176, 1, 'gu', 'eo'),
  (2177, 1, 'gu', 'es'),
  (2178, 1, 'gu', 'et'),
  (2179, 1, 'gu', 'eu'),
  (2180, 1, 'gu', 'fa'),
  (2181, 1, 'gu', 'fi'),
  (2182, 1, 'gu', 'fr'),
  (2183, 1, 'gu', 'ga'),
  (2184, 1, 'gu', 'gl'),
  (2185, 1, 'gu', 'ha'),
  (2186, 1, 'gu', 'hi'),
  (2187, 1, 'gu', 'hmn'),
  (2188, 1, 'gu', 'hr'),
  (2189, 1, 'gu', 'ht'),
  (2190, 1, 'gu', 'hu'),
  (2191, 1, 'gu', 'hy'),
  (2192, 1, 'gu', 'id'),
  (2193, 1, 'gu', 'ig'),
  (2194, 1, 'gu', 'is'),
  (2195, 1, 'gu', 'it'),
  (2196, 1, 'gu', 'iw'),
  (2197, 1, 'gu', 'ja'),
  (2198, 1, 'gu', 'jw'),
  (2199, 1, 'gu', 'ka'),
  (2200, 1, 'gu', 'kk'),
  (2201, 1, 'gu', 'km'),
  (2202, 1, 'gu', 'kn'),
  (2203, 1, 'gu', 'ko'),
  (2204, 1, 'gu', 'la'),
  (2205, 1, 'gu', 'lo'),
  (2206, 1, 'gu', 'lt'),
  (2207, 1, 'gu', 'lv'),
  (2208, 1, 'gu', 'mg'),
  (2209, 1, 'gu', 'mi'),
  (2210, 1, 'gu', 'mk'),
  (2211, 1, 'gu', 'ml'),
  (2212, 1, 'gu', 'mn'),
  (2213, 1, 'gu', 'mr'),
  (2214, 1, 'gu', 'ms'),
  (2215, 1, 'gu', 'mt'),
  (2216, 1, 'gu', 'my'),
  (2217, 1, 'gu', 'ne'),
  (2218, 1, 'gu', 'nl'),
  (2219, 1, 'gu', 'no'),
  (2220, 1, 'gu', 'ny'),
  (2221, 1, 'gu', 'pa'),
  (2222, 1, 'gu', 'pl'),
  (2223, 1, 'gu', 'pt'),
  (2224, 1, 'gu', 'ro'),
  (2225, 1, 'gu', 'ru'),
  (2226, 1, 'gu', 'si'),
  (2227, 1, 'gu', 'sk'),
  (2228, 1, 'gu', 'sl'),
  (2229, 1, 'gu', 'so'),
  (2230, 1, 'gu', 'sq'),
  (2231, 1, 'gu', 'sr'),
  (2232, 1, 'gu', 'st'),
  (2233, 1, 'gu', 'su'),
  (2234, 1, 'gu', 'sv'),
  (2235, 1, 'gu', 'sw'),
  (2236, 1, 'gu', 'ta'),
  (2237, 1, 'gu', 'te'),
  (2238, 1, 'gu', 'tg'),
  (2239, 1, 'gu', 'th'),
  (2240, 1, 'gu', 'tl'),
  (2241, 1, 'gu', 'tr'),
  (2242, 1, 'gu', 'uk'),
  (2243, 1, 'gu', 'ur'),
  (2244, 1, 'gu', 'uz'),
  (2245, 1, 'gu', 'vi'),
  (2246, 1, 'gu', 'yi'),
  (2247, 1, 'gu', 'yo'),
  (2248, 1, 'gu', 'zh'),
  (2249, 1, 'gu', 'zh-TW'),
  (2250, 1, 'gu', 'zu'),
  (2251, 1, 'ha', 'af'),
  (2252, 1, 'ha', 'ar'),
  (2253, 1, 'ha', 'az'),
  (2254, 1, 'ha', 'be'),
  (2255, 1, 'ha', 'bg'),
  (2256, 1, 'ha', 'bn'),
  (2257, 1, 'ha', 'bs'),
  (2258, 1, 'ha', 'ca'),
  (2259, 1, 'ha', 'ceb'),
  (2260, 1, 'ha', 'cs'),
  (2261, 1, 'ha', 'cy'),
  (2262, 1, 'ha', 'da'),
  (2263, 1, 'ha', 'de'),
  (2264, 1, 'ha', 'el'),
  (2265, 1, 'ha', 'en'),
  (2266, 1, 'ha', 'eo'),
  (2267, 1, 'ha', 'es'),
  (2268, 1, 'ha', 'et'),
  (2269, 1, 'ha', 'eu'),
  (2270, 1, 'ha', 'fa'),
  (2271, 1, 'ha', 'fi'),
  (2272, 1, 'ha', 'fr'),
  (2273, 1, 'ha', 'ga'),
  (2274, 1, 'ha', 'gl'),
  (2275, 1, 'ha', 'gu'),
  (2276, 1, 'ha', 'hi'),
  (2277, 1, 'ha', 'hmn'),
  (2278, 1, 'ha', 'hr'),
  (2279, 1, 'ha', 'ht'),
  (2280, 1, 'ha', 'hu'),
  (2281, 1, 'ha', 'hy'),
  (2282, 1, 'ha', 'id'),
  (2283, 1, 'ha', 'ig'),
  (2284, 1, 'ha', 'is'),
  (2285, 1, 'ha', 'it'),
  (2286, 1, 'ha', 'iw'),
  (2287, 1, 'ha', 'ja'),
  (2288, 1, 'ha', 'jw'),
  (2289, 1, 'ha', 'ka'),
  (2290, 1, 'ha', 'kk'),
  (2291, 1, 'ha', 'km'),
  (2292, 1, 'ha', 'kn'),
  (2293, 1, 'ha', 'ko'),
  (2294, 1, 'ha', 'la'),
  (2295, 1, 'ha', 'lo'),
  (2296, 1, 'ha', 'lt'),
  (2297, 1, 'ha', 'lv'),
  (2298, 1, 'ha', 'mg'),
  (2299, 1, 'ha', 'mi'),
  (2300, 1, 'ha', 'mk'),
  (2301, 1, 'ha', 'ml'),
  (2302, 1, 'ha', 'mn'),
  (2303, 1, 'ha', 'mr'),
  (2304, 1, 'ha', 'ms'),
  (2305, 1, 'ha', 'mt'),
  (2306, 1, 'ha', 'my'),
  (2307, 1, 'ha', 'ne'),
  (2308, 1, 'ha', 'nl'),
  (2309, 1, 'ha', 'no'),
  (2310, 1, 'ha', 'ny'),
  (2311, 1, 'ha', 'pa'),
  (2312, 1, 'ha', 'pl'),
  (2313, 1, 'ha', 'pt'),
  (2314, 1, 'ha', 'ro'),
  (2315, 1, 'ha', 'ru'),
  (2316, 1, 'ha', 'si'),
  (2317, 1, 'ha', 'sk'),
  (2318, 1, 'ha', 'sl'),
  (2319, 1, 'ha', 'so'),
  (2320, 1, 'ha', 'sq'),
  (2321, 1, 'ha', 'sr'),
  (2322, 1, 'ha', 'st'),
  (2323, 1, 'ha', 'su'),
  (2324, 1, 'ha', 'sv'),
  (2325, 1, 'ha', 'sw'),
  (2326, 1, 'ha', 'ta'),
  (2327, 1, 'ha', 'te'),
  (2328, 1, 'ha', 'tg'),
  (2329, 1, 'ha', 'th'),
  (2330, 1, 'ha', 'tl'),
  (2331, 1, 'ha', 'tr'),
  (2332, 1, 'ha', 'uk'),
  (2333, 1, 'ha', 'ur'),
  (2334, 1, 'ha', 'uz'),
  (2335, 1, 'ha', 'vi'),
  (2336, 1, 'ha', 'yi'),
  (2337, 1, 'ha', 'yo'),
  (2338, 1, 'ha', 'zh'),
  (2339, 1, 'ha', 'zh-TW'),
  (2340, 1, 'ha', 'zu'),
  (2341, 1, 'hi', 'af'),
  (2342, 1, 'hi', 'ar'),
  (2343, 1, 'hi', 'az'),
  (2344, 1, 'hi', 'be'),
  (2345, 1, 'hi', 'bg'),
  (2346, 1, 'hi', 'bn'),
  (2347, 1, 'hi', 'bs'),
  (2348, 1, 'hi', 'ca'),
  (2349, 1, 'hi', 'ceb'),
  (2350, 1, 'hi', 'cs'),
  (2351, 1, 'hi', 'cy'),
  (2352, 1, 'hi', 'da'),
  (2353, 1, 'hi', 'de'),
  (2354, 1, 'hi', 'el'),
  (2355, 1, 'hi', 'en'),
  (2356, 1, 'hi', 'eo'),
  (2357, 1, 'hi', 'es'),
  (2358, 1, 'hi', 'et'),
  (2359, 1, 'hi', 'eu'),
  (2360, 1, 'hi', 'fa'),
  (2361, 1, 'hi', 'fi'),
  (2362, 1, 'hi', 'fr'),
  (2363, 1, 'hi', 'ga'),
  (2364, 1, 'hi', 'gl'),
  (2365, 1, 'hi', 'gu'),
  (2366, 1, 'hi', 'ha'),
  (2367, 1, 'hi', 'hmn'),
  (2368, 1, 'hi', 'hr'),
  (2369, 1, 'hi', 'ht'),
  (2370, 1, 'hi', 'hu'),
  (2371, 1, 'hi', 'hy'),
  (2372, 1, 'hi', 'id'),
  (2373, 1, 'hi', 'ig'),
  (2374, 1, 'hi', 'is'),
  (2375, 1, 'hi', 'it'),
  (2376, 1, 'hi', 'iw'),
  (2377, 1, 'hi', 'ja'),
  (2378, 1, 'hi', 'jw'),
  (2379, 1, 'hi', 'ka'),
  (2380, 1, 'hi', 'kk'),
  (2381, 1, 'hi', 'km'),
  (2382, 1, 'hi', 'kn'),
  (2383, 1, 'hi', 'ko'),
  (2384, 1, 'hi', 'la'),
  (2385, 1, 'hi', 'lo'),
  (2386, 1, 'hi', 'lt'),
  (2387, 1, 'hi', 'lv'),
  (2388, 1, 'hi', 'mg'),
  (2389, 1, 'hi', 'mi'),
  (2390, 1, 'hi', 'mk'),
  (2391, 1, 'hi', 'ml'),
  (2392, 1, 'hi', 'mn'),
  (2393, 1, 'hi', 'mr'),
  (2394, 1, 'hi', 'ms'),
  (2395, 1, 'hi', 'mt'),
  (2396, 1, 'hi', 'my'),
  (2397, 1, 'hi', 'ne'),
  (2398, 1, 'hi', 'nl'),
  (2399, 1, 'hi', 'no'),
  (2400, 1, 'hi', 'ny'),
  (2401, 1, 'hi', 'pa'),
  (2402, 1, 'hi', 'pl'),
  (2403, 1, 'hi', 'pt'),
  (2404, 1, 'hi', 'ro'),
  (2405, 1, 'hi', 'ru'),
  (2406, 1, 'hi', 'si'),
  (2407, 1, 'hi', 'sk'),
  (2408, 1, 'hi', 'sl'),
  (2409, 1, 'hi', 'so'),
  (2410, 1, 'hi', 'sq'),
  (2411, 1, 'hi', 'sr'),
  (2412, 1, 'hi', 'st'),
  (2413, 1, 'hi', 'su'),
  (2414, 1, 'hi', 'sv'),
  (2415, 1, 'hi', 'sw'),
  (2416, 1, 'hi', 'ta');
INSERT INTO `#__neno_translation_methods_language_pairs` (`id`, `translation_method_id`, `source_language`, `destination_language`) VALUES
  (2417, 1, 'hi', 'te'),
  (2418, 1, 'hi', 'tg'),
  (2419, 1, 'hi', 'th'),
  (2420, 1, 'hi', 'tl'),
  (2421, 1, 'hi', 'tr'),
  (2422, 1, 'hi', 'uk'),
  (2423, 1, 'hi', 'ur'),
  (2424, 1, 'hi', 'uz'),
  (2425, 1, 'hi', 'vi'),
  (2426, 1, 'hi', 'yi'),
  (2427, 1, 'hi', 'yo'),
  (2428, 1, 'hi', 'zh'),
  (2429, 1, 'hi', 'zh-TW'),
  (2430, 1, 'hi', 'zu'),
  (2431, 1, 'hmn', 'af'),
  (2432, 1, 'hmn', 'ar'),
  (2433, 1, 'hmn', 'az'),
  (2434, 1, 'hmn', 'be'),
  (2435, 1, 'hmn', 'bg'),
  (2436, 1, 'hmn', 'bn'),
  (2437, 1, 'hmn', 'bs'),
  (2438, 1, 'hmn', 'ca'),
  (2439, 1, 'hmn', 'ceb'),
  (2440, 1, 'hmn', 'cs'),
  (2441, 1, 'hmn', 'cy'),
  (2442, 1, 'hmn', 'da'),
  (2443, 1, 'hmn', 'de'),
  (2444, 1, 'hmn', 'el'),
  (2445, 1, 'hmn', 'en'),
  (2446, 1, 'hmn', 'eo'),
  (2447, 1, 'hmn', 'es'),
  (2448, 1, 'hmn', 'et'),
  (2449, 1, 'hmn', 'eu'),
  (2450, 1, 'hmn', 'fa'),
  (2451, 1, 'hmn', 'fi'),
  (2452, 1, 'hmn', 'fr'),
  (2453, 1, 'hmn', 'ga'),
  (2454, 1, 'hmn', 'gl'),
  (2455, 1, 'hmn', 'gu'),
  (2456, 1, 'hmn', 'ha'),
  (2457, 1, 'hmn', 'hi'),
  (2458, 1, 'hmn', 'hr'),
  (2459, 1, 'hmn', 'ht'),
  (2460, 1, 'hmn', 'hu'),
  (2461, 1, 'hmn', 'hy'),
  (2462, 1, 'hmn', 'id'),
  (2463, 1, 'hmn', 'ig'),
  (2464, 1, 'hmn', 'is'),
  (2465, 1, 'hmn', 'it'),
  (2466, 1, 'hmn', 'iw'),
  (2467, 1, 'hmn', 'ja'),
  (2468, 1, 'hmn', 'jw'),
  (2469, 1, 'hmn', 'ka'),
  (2470, 1, 'hmn', 'kk'),
  (2471, 1, 'hmn', 'km'),
  (2472, 1, 'hmn', 'kn'),
  (2473, 1, 'hmn', 'ko'),
  (2474, 1, 'hmn', 'la'),
  (2475, 1, 'hmn', 'lo'),
  (2476, 1, 'hmn', 'lt'),
  (2477, 1, 'hmn', 'lv'),
  (2478, 1, 'hmn', 'mg'),
  (2479, 1, 'hmn', 'mi'),
  (2480, 1, 'hmn', 'mk'),
  (2481, 1, 'hmn', 'ml'),
  (2482, 1, 'hmn', 'mn'),
  (2483, 1, 'hmn', 'mr'),
  (2484, 1, 'hmn', 'ms'),
  (2485, 1, 'hmn', 'mt'),
  (2486, 1, 'hmn', 'my'),
  (2487, 1, 'hmn', 'ne'),
  (2488, 1, 'hmn', 'nl'),
  (2489, 1, 'hmn', 'no'),
  (2490, 1, 'hmn', 'ny'),
  (2491, 1, 'hmn', 'pa'),
  (2492, 1, 'hmn', 'pl'),
  (2493, 1, 'hmn', 'pt'),
  (2494, 1, 'hmn', 'ro'),
  (2495, 1, 'hmn', 'ru'),
  (2496, 1, 'hmn', 'si'),
  (2497, 1, 'hmn', 'sk'),
  (2498, 1, 'hmn', 'sl'),
  (2499, 1, 'hmn', 'so'),
  (2500, 1, 'hmn', 'sq'),
  (2501, 1, 'hmn', 'sr'),
  (2502, 1, 'hmn', 'st'),
  (2503, 1, 'hmn', 'su'),
  (2504, 1, 'hmn', 'sv'),
  (2505, 1, 'hmn', 'sw'),
  (2506, 1, 'hmn', 'ta'),
  (2507, 1, 'hmn', 'te'),
  (2508, 1, 'hmn', 'tg'),
  (2509, 1, 'hmn', 'th'),
  (2510, 1, 'hmn', 'tl'),
  (2511, 1, 'hmn', 'tr'),
  (2512, 1, 'hmn', 'uk'),
  (2513, 1, 'hmn', 'ur'),
  (2514, 1, 'hmn', 'uz'),
  (2515, 1, 'hmn', 'vi'),
  (2516, 1, 'hmn', 'yi'),
  (2517, 1, 'hmn', 'yo'),
  (2518, 1, 'hmn', 'zh'),
  (2519, 1, 'hmn', 'zh-TW'),
  (2520, 1, 'hmn', 'zu'),
  (2521, 1, 'hr', 'af'),
  (2522, 1, 'hr', 'ar'),
  (2523, 1, 'hr', 'az'),
  (2524, 1, 'hr', 'be'),
  (2525, 1, 'hr', 'bg'),
  (2526, 1, 'hr', 'bn'),
  (2527, 1, 'hr', 'bs'),
  (2528, 1, 'hr', 'ca'),
  (2529, 1, 'hr', 'ceb'),
  (2530, 1, 'hr', 'cs'),
  (2531, 1, 'hr', 'cy'),
  (2532, 1, 'hr', 'da'),
  (2533, 1, 'hr', 'de'),
  (2534, 1, 'hr', 'el'),
  (2535, 1, 'hr', 'en'),
  (2536, 1, 'hr', 'eo'),
  (2537, 1, 'hr', 'es'),
  (2538, 1, 'hr', 'et'),
  (2539, 1, 'hr', 'eu'),
  (2540, 1, 'hr', 'fa'),
  (2541, 1, 'hr', 'fi'),
  (2542, 1, 'hr', 'fr'),
  (2543, 1, 'hr', 'ga'),
  (2544, 1, 'hr', 'gl'),
  (2545, 1, 'hr', 'gu'),
  (2546, 1, 'hr', 'ha'),
  (2547, 1, 'hr', 'hi'),
  (2548, 1, 'hr', 'hmn'),
  (2549, 1, 'hr', 'ht'),
  (2550, 1, 'hr', 'hu'),
  (2551, 1, 'hr', 'hy'),
  (2552, 1, 'hr', 'id'),
  (2553, 1, 'hr', 'ig'),
  (2554, 1, 'hr', 'is'),
  (2555, 1, 'hr', 'it'),
  (2556, 1, 'hr', 'iw'),
  (2557, 1, 'hr', 'ja'),
  (2558, 1, 'hr', 'jw'),
  (2559, 1, 'hr', 'ka'),
  (2560, 1, 'hr', 'kk'),
  (2561, 1, 'hr', 'km'),
  (2562, 1, 'hr', 'kn'),
  (2563, 1, 'hr', 'ko'),
  (2564, 1, 'hr', 'la'),
  (2565, 1, 'hr', 'lo'),
  (2566, 1, 'hr', 'lt'),
  (2567, 1, 'hr', 'lv'),
  (2568, 1, 'hr', 'mg'),
  (2569, 1, 'hr', 'mi'),
  (2570, 1, 'hr', 'mk'),
  (2571, 1, 'hr', 'ml'),
  (2572, 1, 'hr', 'mn'),
  (2573, 1, 'hr', 'mr'),
  (2574, 1, 'hr', 'ms'),
  (2575, 1, 'hr', 'mt'),
  (2576, 1, 'hr', 'my'),
  (2577, 1, 'hr', 'ne'),
  (2578, 1, 'hr', 'nl'),
  (2579, 1, 'hr', 'no'),
  (2580, 1, 'hr', 'ny'),
  (2581, 1, 'hr', 'pa'),
  (2582, 1, 'hr', 'pl'),
  (2583, 1, 'hr', 'pt'),
  (2584, 1, 'hr', 'ro'),
  (2585, 1, 'hr', 'ru'),
  (2586, 1, 'hr', 'si'),
  (2587, 1, 'hr', 'sk'),
  (2588, 1, 'hr', 'sl'),
  (2589, 1, 'hr', 'so'),
  (2590, 1, 'hr', 'sq'),
  (2591, 1, 'hr', 'sr'),
  (2592, 1, 'hr', 'st'),
  (2593, 1, 'hr', 'su'),
  (2594, 1, 'hr', 'sv'),
  (2595, 1, 'hr', 'sw'),
  (2596, 1, 'hr', 'ta'),
  (2597, 1, 'hr', 'te'),
  (2598, 1, 'hr', 'tg'),
  (2599, 1, 'hr', 'th'),
  (2600, 1, 'hr', 'tl'),
  (2601, 1, 'hr', 'tr'),
  (2602, 1, 'hr', 'uk'),
  (2603, 1, 'hr', 'ur'),
  (2604, 1, 'hr', 'uz'),
  (2605, 1, 'hr', 'vi'),
  (2606, 1, 'hr', 'yi'),
  (2607, 1, 'hr', 'yo'),
  (2608, 1, 'hr', 'zh'),
  (2609, 1, 'hr', 'zh-TW'),
  (2610, 1, 'hr', 'zu'),
  (2611, 1, 'ht', 'af'),
  (2612, 1, 'ht', 'ar'),
  (2613, 1, 'ht', 'az'),
  (2614, 1, 'ht', 'be'),
  (2615, 1, 'ht', 'bg'),
  (2616, 1, 'ht', 'bn'),
  (2617, 1, 'ht', 'bs'),
  (2618, 1, 'ht', 'ca'),
  (2619, 1, 'ht', 'ceb'),
  (2620, 1, 'ht', 'cs'),
  (2621, 1, 'ht', 'cy'),
  (2622, 1, 'ht', 'da'),
  (2623, 1, 'ht', 'de'),
  (2624, 1, 'ht', 'el'),
  (2625, 1, 'ht', 'en'),
  (2626, 1, 'ht', 'eo'),
  (2627, 1, 'ht', 'es'),
  (2628, 1, 'ht', 'et'),
  (2629, 1, 'ht', 'eu'),
  (2630, 1, 'ht', 'fa'),
  (2631, 1, 'ht', 'fi'),
  (2632, 1, 'ht', 'fr'),
  (2633, 1, 'ht', 'ga'),
  (2634, 1, 'ht', 'gl'),
  (2635, 1, 'ht', 'gu'),
  (2636, 1, 'ht', 'ha'),
  (2637, 1, 'ht', 'hi'),
  (2638, 1, 'ht', 'hmn'),
  (2639, 1, 'ht', 'hr'),
  (2640, 1, 'ht', 'hu'),
  (2641, 1, 'ht', 'hy'),
  (2642, 1, 'ht', 'id'),
  (2643, 1, 'ht', 'ig'),
  (2644, 1, 'ht', 'is'),
  (2645, 1, 'ht', 'it'),
  (2646, 1, 'ht', 'iw'),
  (2647, 1, 'ht', 'ja'),
  (2648, 1, 'ht', 'jw'),
  (2649, 1, 'ht', 'ka'),
  (2650, 1, 'ht', 'kk'),
  (2651, 1, 'ht', 'km'),
  (2652, 1, 'ht', 'kn'),
  (2653, 1, 'ht', 'ko'),
  (2654, 1, 'ht', 'la'),
  (2655, 1, 'ht', 'lo'),
  (2656, 1, 'ht', 'lt'),
  (2657, 1, 'ht', 'lv'),
  (2658, 1, 'ht', 'mg'),
  (2659, 1, 'ht', 'mi'),
  (2660, 1, 'ht', 'mk'),
  (2661, 1, 'ht', 'ml'),
  (2662, 1, 'ht', 'mn'),
  (2663, 1, 'ht', 'mr'),
  (2664, 1, 'ht', 'ms'),
  (2665, 1, 'ht', 'mt'),
  (2666, 1, 'ht', 'my'),
  (2667, 1, 'ht', 'ne'),
  (2668, 1, 'ht', 'nl'),
  (2669, 1, 'ht', 'no'),
  (2670, 1, 'ht', 'ny'),
  (2671, 1, 'ht', 'pa'),
  (2672, 1, 'ht', 'pl'),
  (2673, 1, 'ht', 'pt'),
  (2674, 1, 'ht', 'ro'),
  (2675, 1, 'ht', 'ru'),
  (2676, 1, 'ht', 'si'),
  (2677, 1, 'ht', 'sk'),
  (2678, 1, 'ht', 'sl'),
  (2679, 1, 'ht', 'so'),
  (2680, 1, 'ht', 'sq'),
  (2681, 1, 'ht', 'sr'),
  (2682, 1, 'ht', 'st'),
  (2683, 1, 'ht', 'su'),
  (2684, 1, 'ht', 'sv'),
  (2685, 1, 'ht', 'sw'),
  (2686, 1, 'ht', 'ta'),
  (2687, 1, 'ht', 'te'),
  (2688, 1, 'ht', 'tg'),
  (2689, 1, 'ht', 'th'),
  (2690, 1, 'ht', 'tl'),
  (2691, 1, 'ht', 'tr'),
  (2692, 1, 'ht', 'uk'),
  (2693, 1, 'ht', 'ur'),
  (2694, 1, 'ht', 'uz'),
  (2695, 1, 'ht', 'vi'),
  (2696, 1, 'ht', 'yi'),
  (2697, 1, 'ht', 'yo'),
  (2698, 1, 'ht', 'zh'),
  (2699, 1, 'ht', 'zh-TW'),
  (2700, 1, 'ht', 'zu'),
  (2701, 1, 'hu', 'af'),
  (2702, 1, 'hu', 'ar'),
  (2703, 1, 'hu', 'az'),
  (2704, 1, 'hu', 'be'),
  (2705, 1, 'hu', 'bg'),
  (2706, 1, 'hu', 'bn'),
  (2707, 1, 'hu', 'bs'),
  (2708, 1, 'hu', 'ca'),
  (2709, 1, 'hu', 'ceb'),
  (2710, 1, 'hu', 'cs'),
  (2711, 1, 'hu', 'cy'),
  (2712, 1, 'hu', 'da'),
  (2713, 1, 'hu', 'de'),
  (2714, 1, 'hu', 'el'),
  (2715, 1, 'hu', 'en'),
  (2716, 1, 'hu', 'eo'),
  (2717, 1, 'hu', 'es'),
  (2718, 1, 'hu', 'et'),
  (2719, 1, 'hu', 'eu'),
  (2720, 1, 'hu', 'fa'),
  (2721, 1, 'hu', 'fi'),
  (2722, 1, 'hu', 'fr'),
  (2723, 1, 'hu', 'ga'),
  (2724, 1, 'hu', 'gl'),
  (2725, 1, 'hu', 'gu'),
  (2726, 1, 'hu', 'ha'),
  (2727, 1, 'hu', 'hi'),
  (2728, 1, 'hu', 'hmn'),
  (2729, 1, 'hu', 'hr'),
  (2730, 1, 'hu', 'ht'),
  (2731, 1, 'hu', 'hy'),
  (2732, 1, 'hu', 'id'),
  (2733, 1, 'hu', 'ig'),
  (2734, 1, 'hu', 'is'),
  (2735, 1, 'hu', 'it'),
  (2736, 1, 'hu', 'iw'),
  (2737, 1, 'hu', 'ja'),
  (2738, 1, 'hu', 'jw'),
  (2739, 1, 'hu', 'ka'),
  (2740, 1, 'hu', 'kk'),
  (2741, 1, 'hu', 'km'),
  (2742, 1, 'hu', 'kn'),
  (2743, 1, 'hu', 'ko'),
  (2744, 1, 'hu', 'la'),
  (2745, 1, 'hu', 'lo'),
  (2746, 1, 'hu', 'lt'),
  (2747, 1, 'hu', 'lv'),
  (2748, 1, 'hu', 'mg'),
  (2749, 1, 'hu', 'mi'),
  (2750, 1, 'hu', 'mk'),
  (2751, 1, 'hu', 'ml'),
  (2752, 1, 'hu', 'mn'),
  (2753, 1, 'hu', 'mr'),
  (2754, 1, 'hu', 'ms'),
  (2755, 1, 'hu', 'mt'),
  (2756, 1, 'hu', 'my'),
  (2757, 1, 'hu', 'ne'),
  (2758, 1, 'hu', 'nl'),
  (2759, 1, 'hu', 'no'),
  (2760, 1, 'hu', 'ny'),
  (2761, 1, 'hu', 'pa'),
  (2762, 1, 'hu', 'pl'),
  (2763, 1, 'hu', 'pt'),
  (2764, 1, 'hu', 'ro'),
  (2765, 1, 'hu', 'ru'),
  (2766, 1, 'hu', 'si'),
  (2767, 1, 'hu', 'sk'),
  (2768, 1, 'hu', 'sl'),
  (2769, 1, 'hu', 'so'),
  (2770, 1, 'hu', 'sq'),
  (2771, 1, 'hu', 'sr'),
  (2772, 1, 'hu', 'st'),
  (2773, 1, 'hu', 'su'),
  (2774, 1, 'hu', 'sv'),
  (2775, 1, 'hu', 'sw'),
  (2776, 1, 'hu', 'ta'),
  (2777, 1, 'hu', 'te'),
  (2778, 1, 'hu', 'tg'),
  (2779, 1, 'hu', 'th'),
  (2780, 1, 'hu', 'tl'),
  (2781, 1, 'hu', 'tr'),
  (2782, 1, 'hu', 'uk'),
  (2783, 1, 'hu', 'ur'),
  (2784, 1, 'hu', 'uz'),
  (2785, 1, 'hu', 'vi'),
  (2786, 1, 'hu', 'yi'),
  (2787, 1, 'hu', 'yo'),
  (2788, 1, 'hu', 'zh'),
  (2789, 1, 'hu', 'zh-TW'),
  (2790, 1, 'hu', 'zu'),
  (2791, 1, 'hy', 'af'),
  (2792, 1, 'hy', 'ar'),
  (2793, 1, 'hy', 'az'),
  (2794, 1, 'hy', 'be'),
  (2795, 1, 'hy', 'bg'),
  (2796, 1, 'hy', 'bn'),
  (2797, 1, 'hy', 'bs'),
  (2798, 1, 'hy', 'ca'),
  (2799, 1, 'hy', 'ceb'),
  (2800, 1, 'hy', 'cs'),
  (2801, 1, 'hy', 'cy'),
  (2802, 1, 'hy', 'da'),
  (2803, 1, 'hy', 'de'),
  (2804, 1, 'hy', 'el'),
  (2805, 1, 'hy', 'en'),
  (2806, 1, 'hy', 'eo'),
  (2807, 1, 'hy', 'es'),
  (2808, 1, 'hy', 'et'),
  (2809, 1, 'hy', 'eu'),
  (2810, 1, 'hy', 'fa'),
  (2811, 1, 'hy', 'fi'),
  (2812, 1, 'hy', 'fr'),
  (2813, 1, 'hy', 'ga'),
  (2814, 1, 'hy', 'gl'),
  (2815, 1, 'hy', 'gu'),
  (2816, 1, 'hy', 'ha'),
  (2817, 1, 'hy', 'hi'),
  (2818, 1, 'hy', 'hmn'),
  (2819, 1, 'hy', 'hr'),
  (2820, 1, 'hy', 'ht'),
  (2821, 1, 'hy', 'hu'),
  (2822, 1, 'hy', 'id'),
  (2823, 1, 'hy', 'ig'),
  (2824, 1, 'hy', 'is'),
  (2825, 1, 'hy', 'it'),
  (2826, 1, 'hy', 'iw'),
  (2827, 1, 'hy', 'ja'),
  (2828, 1, 'hy', 'jw'),
  (2829, 1, 'hy', 'ka'),
  (2830, 1, 'hy', 'kk'),
  (2831, 1, 'hy', 'km'),
  (2832, 1, 'hy', 'kn'),
  (2833, 1, 'hy', 'ko'),
  (2834, 1, 'hy', 'la'),
  (2835, 1, 'hy', 'lo'),
  (2836, 1, 'hy', 'lt'),
  (2837, 1, 'hy', 'lv'),
  (2838, 1, 'hy', 'mg'),
  (2839, 1, 'hy', 'mi'),
  (2840, 1, 'hy', 'mk'),
  (2841, 1, 'hy', 'ml'),
  (2842, 1, 'hy', 'mn'),
  (2843, 1, 'hy', 'mr'),
  (2844, 1, 'hy', 'ms'),
  (2845, 1, 'hy', 'mt'),
  (2846, 1, 'hy', 'my'),
  (2847, 1, 'hy', 'ne'),
  (2848, 1, 'hy', 'nl'),
  (2849, 1, 'hy', 'no'),
  (2850, 1, 'hy', 'ny'),
  (2851, 1, 'hy', 'pa'),
  (2852, 1, 'hy', 'pl'),
  (2853, 1, 'hy', 'pt'),
  (2854, 1, 'hy', 'ro'),
  (2855, 1, 'hy', 'ru'),
  (2856, 1, 'hy', 'si'),
  (2857, 1, 'hy', 'sk'),
  (2858, 1, 'hy', 'sl'),
  (2859, 1, 'hy', 'so'),
  (2860, 1, 'hy', 'sq'),
  (2861, 1, 'hy', 'sr'),
  (2862, 1, 'hy', 'st'),
  (2863, 1, 'hy', 'su'),
  (2864, 1, 'hy', 'sv'),
  (2865, 1, 'hy', 'sw'),
  (2866, 1, 'hy', 'ta'),
  (2867, 1, 'hy', 'te'),
  (2868, 1, 'hy', 'tg'),
  (2869, 1, 'hy', 'th'),
  (2870, 1, 'hy', 'tl'),
  (2871, 1, 'hy', 'tr'),
  (2872, 1, 'hy', 'uk'),
  (2873, 1, 'hy', 'ur'),
  (2874, 1, 'hy', 'uz'),
  (2875, 1, 'hy', 'vi'),
  (2876, 1, 'hy', 'yi'),
  (2877, 1, 'hy', 'yo'),
  (2878, 1, 'hy', 'zh'),
  (2879, 1, 'hy', 'zh-TW'),
  (2880, 1, 'hy', 'zu'),
  (2881, 1, 'id', 'af'),
  (2882, 1, 'id', 'ar'),
  (2883, 1, 'id', 'az'),
  (2884, 1, 'id', 'be'),
  (2885, 1, 'id', 'bg'),
  (2886, 1, 'id', 'bn'),
  (2887, 1, 'id', 'bs'),
  (2888, 1, 'id', 'ca'),
  (2889, 1, 'id', 'ceb'),
  (2890, 1, 'id', 'cs'),
  (2891, 1, 'id', 'cy'),
  (2892, 1, 'id', 'da'),
  (2893, 1, 'id', 'de'),
  (2894, 1, 'id', 'el'),
  (2895, 1, 'id', 'en'),
  (2896, 1, 'id', 'eo'),
  (2897, 1, 'id', 'es'),
  (2898, 1, 'id', 'et'),
  (2899, 1, 'id', 'eu'),
  (2900, 1, 'id', 'fa'),
  (2901, 1, 'id', 'fi'),
  (2902, 1, 'id', 'fr'),
  (2903, 1, 'id', 'ga'),
  (2904, 1, 'id', 'gl'),
  (2905, 1, 'id', 'gu'),
  (2906, 1, 'id', 'ha'),
  (2907, 1, 'id', 'hi'),
  (2908, 1, 'id', 'hmn'),
  (2909, 1, 'id', 'hr'),
  (2910, 1, 'id', 'ht'),
  (2911, 1, 'id', 'hu'),
  (2912, 1, 'id', 'hy'),
  (2913, 1, 'id', 'ig'),
  (2914, 1, 'id', 'is'),
  (2915, 1, 'id', 'it'),
  (2916, 1, 'id', 'iw'),
  (2917, 1, 'id', 'ja'),
  (2918, 1, 'id', 'jw'),
  (2919, 1, 'id', 'ka'),
  (2920, 1, 'id', 'kk'),
  (2921, 1, 'id', 'km'),
  (2922, 1, 'id', 'kn'),
  (2923, 1, 'id', 'ko'),
  (2924, 1, 'id', 'la'),
  (2925, 1, 'id', 'lo'),
  (2926, 1, 'id', 'lt'),
  (2927, 1, 'id', 'lv'),
  (2928, 1, 'id', 'mg'),
  (2929, 1, 'id', 'mi'),
  (2930, 1, 'id', 'mk'),
  (2931, 1, 'id', 'ml'),
  (2932, 1, 'id', 'mn'),
  (2933, 1, 'id', 'mr'),
  (2934, 1, 'id', 'ms'),
  (2935, 1, 'id', 'mt'),
  (2936, 1, 'id', 'my'),
  (2937, 1, 'id', 'ne'),
  (2938, 1, 'id', 'nl'),
  (2939, 1, 'id', 'no'),
  (2940, 1, 'id', 'ny'),
  (2941, 1, 'id', 'pa'),
  (2942, 1, 'id', 'pl'),
  (2943, 1, 'id', 'pt'),
  (2944, 1, 'id', 'ro'),
  (2945, 1, 'id', 'ru'),
  (2946, 1, 'id', 'si'),
  (2947, 1, 'id', 'sk'),
  (2948, 1, 'id', 'sl'),
  (2949, 1, 'id', 'so'),
  (2950, 1, 'id', 'sq'),
  (2951, 1, 'id', 'sr'),
  (2952, 1, 'id', 'st'),
  (2953, 1, 'id', 'su'),
  (2954, 1, 'id', 'sv'),
  (2955, 1, 'id', 'sw'),
  (2956, 1, 'id', 'ta'),
  (2957, 1, 'id', 'te'),
  (2958, 1, 'id', 'tg'),
  (2959, 1, 'id', 'th'),
  (2960, 1, 'id', 'tl'),
  (2961, 1, 'id', 'tr'),
  (2962, 1, 'id', 'uk'),
  (2963, 1, 'id', 'ur'),
  (2964, 1, 'id', 'uz'),
  (2965, 1, 'id', 'vi'),
  (2966, 1, 'id', 'yi'),
  (2967, 1, 'id', 'yo'),
  (2968, 1, 'id', 'zh'),
  (2969, 1, 'id', 'zh-TW'),
  (2970, 1, 'id', 'zu'),
  (2971, 1, 'ig', 'af'),
  (2972, 1, 'ig', 'ar'),
  (2973, 1, 'ig', 'az'),
  (2974, 1, 'ig', 'be'),
  (2975, 1, 'ig', 'bg'),
  (2976, 1, 'ig', 'bn'),
  (2977, 1, 'ig', 'bs'),
  (2978, 1, 'ig', 'ca'),
  (2979, 1, 'ig', 'ceb'),
  (2980, 1, 'ig', 'cs'),
  (2981, 1, 'ig', 'cy'),
  (2982, 1, 'ig', 'da'),
  (2983, 1, 'ig', 'de'),
  (2984, 1, 'ig', 'el'),
  (2985, 1, 'ig', 'en'),
  (2986, 1, 'ig', 'eo'),
  (2987, 1, 'ig', 'es'),
  (2988, 1, 'ig', 'et'),
  (2989, 1, 'ig', 'eu'),
  (2990, 1, 'ig', 'fa'),
  (2991, 1, 'ig', 'fi'),
  (2992, 1, 'ig', 'fr'),
  (2993, 1, 'ig', 'ga'),
  (2994, 1, 'ig', 'gl'),
  (2995, 1, 'ig', 'gu'),
  (2996, 1, 'ig', 'ha'),
  (2997, 1, 'ig', 'hi'),
  (2998, 1, 'ig', 'hmn'),
  (2999, 1, 'ig', 'hr'),
  (3000, 1, 'ig', 'ht'),
  (3001, 1, 'ig', 'hu'),
  (3002, 1, 'ig', 'hy'),
  (3003, 1, 'ig', 'id'),
  (3004, 1, 'ig', 'is'),
  (3005, 1, 'ig', 'it'),
  (3006, 1, 'ig', 'iw'),
  (3007, 1, 'ig', 'ja'),
  (3008, 1, 'ig', 'jw'),
  (3009, 1, 'ig', 'ka'),
  (3010, 1, 'ig', 'kk'),
  (3011, 1, 'ig', 'km'),
  (3012, 1, 'ig', 'kn'),
  (3013, 1, 'ig', 'ko'),
  (3014, 1, 'ig', 'la'),
  (3015, 1, 'ig', 'lo'),
  (3016, 1, 'ig', 'lt'),
  (3017, 1, 'ig', 'lv'),
  (3018, 1, 'ig', 'mg'),
  (3019, 1, 'ig', 'mi'),
  (3020, 1, 'ig', 'mk'),
  (3021, 1, 'ig', 'ml'),
  (3022, 1, 'ig', 'mn'),
  (3023, 1, 'ig', 'mr'),
  (3024, 1, 'ig', 'ms'),
  (3025, 1, 'ig', 'mt'),
  (3026, 1, 'ig', 'my'),
  (3027, 1, 'ig', 'ne'),
  (3028, 1, 'ig', 'nl'),
  (3029, 1, 'ig', 'no'),
  (3030, 1, 'ig', 'ny'),
  (3031, 1, 'ig', 'pa'),
  (3032, 1, 'ig', 'pl'),
  (3033, 1, 'ig', 'pt'),
  (3034, 1, 'ig', 'ro'),
  (3035, 1, 'ig', 'ru'),
  (3036, 1, 'ig', 'si'),
  (3037, 1, 'ig', 'sk'),
  (3038, 1, 'ig', 'sl'),
  (3039, 1, 'ig', 'so'),
  (3040, 1, 'ig', 'sq'),
  (3041, 1, 'ig', 'sr'),
  (3042, 1, 'ig', 'st'),
  (3043, 1, 'ig', 'su'),
  (3044, 1, 'ig', 'sv'),
  (3045, 1, 'ig', 'sw'),
  (3046, 1, 'ig', 'ta'),
  (3047, 1, 'ig', 'te'),
  (3048, 1, 'ig', 'tg'),
  (3049, 1, 'ig', 'th'),
  (3050, 1, 'ig', 'tl'),
  (3051, 1, 'ig', 'tr'),
  (3052, 1, 'ig', 'uk'),
  (3053, 1, 'ig', 'ur'),
  (3054, 1, 'ig', 'uz'),
  (3055, 1, 'ig', 'vi'),
  (3056, 1, 'ig', 'yi'),
  (3057, 1, 'ig', 'yo'),
  (3058, 1, 'ig', 'zh'),
  (3059, 1, 'ig', 'zh-TW'),
  (3060, 1, 'ig', 'zu'),
  (3061, 1, 'is', 'af'),
  (3062, 1, 'is', 'ar'),
  (3063, 1, 'is', 'az'),
  (3064, 1, 'is', 'be'),
  (3065, 1, 'is', 'bg'),
  (3066, 1, 'is', 'bn'),
  (3067, 1, 'is', 'bs'),
  (3068, 1, 'is', 'ca'),
  (3069, 1, 'is', 'ceb'),
  (3070, 1, 'is', 'cs'),
  (3071, 1, 'is', 'cy'),
  (3072, 1, 'is', 'da'),
  (3073, 1, 'is', 'de'),
  (3074, 1, 'is', 'el'),
  (3075, 1, 'is', 'en'),
  (3076, 1, 'is', 'eo'),
  (3077, 1, 'is', 'es'),
  (3078, 1, 'is', 'et'),
  (3079, 1, 'is', 'eu'),
  (3080, 1, 'is', 'fa'),
  (3081, 1, 'is', 'fi'),
  (3082, 1, 'is', 'fr'),
  (3083, 1, 'is', 'ga'),
  (3084, 1, 'is', 'gl'),
  (3085, 1, 'is', 'gu'),
  (3086, 1, 'is', 'ha'),
  (3087, 1, 'is', 'hi'),
  (3088, 1, 'is', 'hmn'),
  (3089, 1, 'is', 'hr'),
  (3090, 1, 'is', 'ht'),
  (3091, 1, 'is', 'hu'),
  (3092, 1, 'is', 'hy'),
  (3093, 1, 'is', 'id'),
  (3094, 1, 'is', 'ig'),
  (3095, 1, 'is', 'it'),
  (3096, 1, 'is', 'iw'),
  (3097, 1, 'is', 'ja'),
  (3098, 1, 'is', 'jw'),
  (3099, 1, 'is', 'ka'),
  (3100, 1, 'is', 'kk'),
  (3101, 1, 'is', 'km'),
  (3102, 1, 'is', 'kn'),
  (3103, 1, 'is', 'ko'),
  (3104, 1, 'is', 'la'),
  (3105, 1, 'is', 'lo'),
  (3106, 1, 'is', 'lt'),
  (3107, 1, 'is', 'lv'),
  (3108, 1, 'is', 'mg'),
  (3109, 1, 'is', 'mi'),
  (3110, 1, 'is', 'mk'),
  (3111, 1, 'is', 'ml'),
  (3112, 1, 'is', 'mn'),
  (3113, 1, 'is', 'mr'),
  (3114, 1, 'is', 'ms'),
  (3115, 1, 'is', 'mt'),
  (3116, 1, 'is', 'my'),
  (3117, 1, 'is', 'ne'),
  (3118, 1, 'is', 'nl'),
  (3119, 1, 'is', 'no'),
  (3120, 1, 'is', 'ny'),
  (3121, 1, 'is', 'pa'),
  (3122, 1, 'is', 'pl'),
  (3123, 1, 'is', 'pt'),
  (3124, 1, 'is', 'ro'),
  (3125, 1, 'is', 'ru'),
  (3126, 1, 'is', 'si'),
  (3127, 1, 'is', 'sk'),
  (3128, 1, 'is', 'sl'),
  (3129, 1, 'is', 'so'),
  (3130, 1, 'is', 'sq'),
  (3131, 1, 'is', 'sr'),
  (3132, 1, 'is', 'st'),
  (3133, 1, 'is', 'su'),
  (3134, 1, 'is', 'sv'),
  (3135, 1, 'is', 'sw'),
  (3136, 1, 'is', 'ta'),
  (3137, 1, 'is', 'te'),
  (3138, 1, 'is', 'tg'),
  (3139, 1, 'is', 'th'),
  (3140, 1, 'is', 'tl'),
  (3141, 1, 'is', 'tr'),
  (3142, 1, 'is', 'uk'),
  (3143, 1, 'is', 'ur'),
  (3144, 1, 'is', 'uz'),
  (3145, 1, 'is', 'vi'),
  (3146, 1, 'is', 'yi'),
  (3147, 1, 'is', 'yo'),
  (3148, 1, 'is', 'zh'),
  (3149, 1, 'is', 'zh-TW'),
  (3150, 1, 'is', 'zu'),
  (3151, 1, 'it', 'af'),
  (3152, 1, 'it', 'ar'),
  (3153, 1, 'it', 'az'),
  (3154, 1, 'it', 'be'),
  (3155, 1, 'it', 'bg'),
  (3156, 1, 'it', 'bn'),
  (3157, 1, 'it', 'bs'),
  (3158, 1, 'it', 'ca'),
  (3159, 1, 'it', 'ceb'),
  (3160, 1, 'it', 'cs'),
  (3161, 1, 'it', 'cy'),
  (3162, 1, 'it', 'da'),
  (3163, 1, 'it', 'de'),
  (3164, 1, 'it', 'el'),
  (3165, 1, 'it', 'en'),
  (3166, 1, 'it', 'eo'),
  (3167, 1, 'it', 'es'),
  (3168, 1, 'it', 'et'),
  (3169, 1, 'it', 'eu'),
  (3170, 1, 'it', 'fa'),
  (3171, 1, 'it', 'fi'),
  (3172, 1, 'it', 'fr'),
  (3173, 1, 'it', 'ga'),
  (3174, 1, 'it', 'gl'),
  (3175, 1, 'it', 'gu'),
  (3176, 1, 'it', 'ha'),
  (3177, 1, 'it', 'hi'),
  (3178, 1, 'it', 'hmn'),
  (3179, 1, 'it', 'hr'),
  (3180, 1, 'it', 'ht'),
  (3181, 1, 'it', 'hu'),
  (3182, 1, 'it', 'hy'),
  (3183, 1, 'it', 'id'),
  (3184, 1, 'it', 'ig'),
  (3185, 1, 'it', 'is'),
  (3186, 1, 'it', 'iw'),
  (3187, 1, 'it', 'ja'),
  (3188, 1, 'it', 'jw'),
  (3189, 1, 'it', 'ka'),
  (3190, 1, 'it', 'kk'),
  (3191, 1, 'it', 'km'),
  (3192, 1, 'it', 'kn'),
  (3193, 1, 'it', 'ko'),
  (3194, 1, 'it', 'la'),
  (3195, 1, 'it', 'lo'),
  (3196, 1, 'it', 'lt'),
  (3197, 1, 'it', 'lv'),
  (3198, 1, 'it', 'mg'),
  (3199, 1, 'it', 'mi'),
  (3200, 1, 'it', 'mk'),
  (3201, 1, 'it', 'ml'),
  (3202, 1, 'it', 'mn'),
  (3203, 1, 'it', 'mr'),
  (3204, 1, 'it', 'ms'),
  (3205, 1, 'it', 'mt'),
  (3206, 1, 'it', 'my'),
  (3207, 1, 'it', 'ne'),
  (3208, 1, 'it', 'nl'),
  (3209, 1, 'it', 'no'),
  (3210, 1, 'it', 'ny'),
  (3211, 1, 'it', 'pa'),
  (3212, 1, 'it', 'pl'),
  (3213, 1, 'it', 'pt'),
  (3214, 1, 'it', 'ro'),
  (3215, 1, 'it', 'ru'),
  (3216, 1, 'it', 'si'),
  (3217, 1, 'it', 'sk'),
  (3218, 1, 'it', 'sl'),
  (3219, 1, 'it', 'so'),
  (3220, 1, 'it', 'sq'),
  (3221, 1, 'it', 'sr'),
  (3222, 1, 'it', 'st'),
  (3223, 1, 'it', 'su'),
  (3224, 1, 'it', 'sv'),
  (3225, 1, 'it', 'sw'),
  (3226, 1, 'it', 'ta'),
  (3227, 1, 'it', 'te'),
  (3228, 1, 'it', 'tg'),
  (3229, 1, 'it', 'th'),
  (3230, 1, 'it', 'tl'),
  (3231, 1, 'it', 'tr'),
  (3232, 1, 'it', 'uk'),
  (3233, 1, 'it', 'ur'),
  (3234, 1, 'it', 'uz'),
  (3235, 1, 'it', 'vi'),
  (3236, 1, 'it', 'yi'),
  (3237, 1, 'it', 'yo'),
  (3238, 1, 'it', 'zh'),
  (3239, 1, 'it', 'zh-TW'),
  (3240, 1, 'it', 'zu'),
  (3241, 1, 'iw', 'af'),
  (3242, 1, 'iw', 'ar'),
  (3243, 1, 'iw', 'az'),
  (3244, 1, 'iw', 'be'),
  (3245, 1, 'iw', 'bg'),
  (3246, 1, 'iw', 'bn'),
  (3247, 1, 'iw', 'bs'),
  (3248, 1, 'iw', 'ca'),
  (3249, 1, 'iw', 'ceb'),
  (3250, 1, 'iw', 'cs'),
  (3251, 1, 'iw', 'cy'),
  (3252, 1, 'iw', 'da'),
  (3253, 1, 'iw', 'de'),
  (3254, 1, 'iw', 'el'),
  (3255, 1, 'iw', 'en'),
  (3256, 1, 'iw', 'eo'),
  (3257, 1, 'iw', 'es'),
  (3258, 1, 'iw', 'et'),
  (3259, 1, 'iw', 'eu'),
  (3260, 1, 'iw', 'fa'),
  (3261, 1, 'iw', 'fi'),
  (3262, 1, 'iw', 'fr'),
  (3263, 1, 'iw', 'ga'),
  (3264, 1, 'iw', 'gl'),
  (3265, 1, 'iw', 'gu'),
  (3266, 1, 'iw', 'ha'),
  (3267, 1, 'iw', 'hi'),
  (3268, 1, 'iw', 'hmn'),
  (3269, 1, 'iw', 'hr'),
  (3270, 1, 'iw', 'ht'),
  (3271, 1, 'iw', 'hu'),
  (3272, 1, 'iw', 'hy'),
  (3273, 1, 'iw', 'id'),
  (3274, 1, 'iw', 'ig'),
  (3275, 1, 'iw', 'is'),
  (3276, 1, 'iw', 'it'),
  (3277, 1, 'iw', 'ja'),
  (3278, 1, 'iw', 'jw'),
  (3279, 1, 'iw', 'ka'),
  (3280, 1, 'iw', 'kk'),
  (3281, 1, 'iw', 'km'),
  (3282, 1, 'iw', 'kn'),
  (3283, 1, 'iw', 'ko'),
  (3284, 1, 'iw', 'la'),
  (3285, 1, 'iw', 'lo'),
  (3286, 1, 'iw', 'lt'),
  (3287, 1, 'iw', 'lv'),
  (3288, 1, 'iw', 'mg'),
  (3289, 1, 'iw', 'mi'),
  (3290, 1, 'iw', 'mk'),
  (3291, 1, 'iw', 'ml'),
  (3292, 1, 'iw', 'mn'),
  (3293, 1, 'iw', 'mr'),
  (3294, 1, 'iw', 'ms'),
  (3295, 1, 'iw', 'mt'),
  (3296, 1, 'iw', 'my'),
  (3297, 1, 'iw', 'ne'),
  (3298, 1, 'iw', 'nl'),
  (3299, 1, 'iw', 'no'),
  (3300, 1, 'iw', 'ny'),
  (3301, 1, 'iw', 'pa'),
  (3302, 1, 'iw', 'pl'),
  (3303, 1, 'iw', 'pt'),
  (3304, 1, 'iw', 'ro'),
  (3305, 1, 'iw', 'ru'),
  (3306, 1, 'iw', 'si'),
  (3307, 1, 'iw', 'sk'),
  (3308, 1, 'iw', 'sl'),
  (3309, 1, 'iw', 'so'),
  (3310, 1, 'iw', 'sq'),
  (3311, 1, 'iw', 'sr'),
  (3312, 1, 'iw', 'st'),
  (3313, 1, 'iw', 'su'),
  (3314, 1, 'iw', 'sv'),
  (3315, 1, 'iw', 'sw'),
  (3316, 1, 'iw', 'ta'),
  (3317, 1, 'iw', 'te'),
  (3318, 1, 'iw', 'tg'),
  (3319, 1, 'iw', 'th'),
  (3320, 1, 'iw', 'tl'),
  (3321, 1, 'iw', 'tr'),
  (3322, 1, 'iw', 'uk'),
  (3323, 1, 'iw', 'ur'),
  (3324, 1, 'iw', 'uz'),
  (3325, 1, 'iw', 'vi'),
  (3326, 1, 'iw', 'yi'),
  (3327, 1, 'iw', 'yo'),
  (3328, 1, 'iw', 'zh'),
  (3329, 1, 'iw', 'zh-TW'),
  (3330, 1, 'iw', 'zu'),
  (3331, 1, 'ja', 'af'),
  (3332, 1, 'ja', 'ar'),
  (3333, 1, 'ja', 'az'),
  (3334, 1, 'ja', 'be'),
  (3335, 1, 'ja', 'bg'),
  (3336, 1, 'ja', 'bn'),
  (3337, 1, 'ja', 'bs'),
  (3338, 1, 'ja', 'ca'),
  (3339, 1, 'ja', 'ceb'),
  (3340, 1, 'ja', 'cs'),
  (3341, 1, 'ja', 'cy'),
  (3342, 1, 'ja', 'da'),
  (3343, 1, 'ja', 'de'),
  (3344, 1, 'ja', 'el'),
  (3345, 1, 'ja', 'en'),
  (3346, 1, 'ja', 'eo'),
  (3347, 1, 'ja', 'es'),
  (3348, 1, 'ja', 'et'),
  (3349, 1, 'ja', 'eu'),
  (3350, 1, 'ja', 'fa'),
  (3351, 1, 'ja', 'fi'),
  (3352, 1, 'ja', 'fr'),
  (3353, 1, 'ja', 'ga'),
  (3354, 1, 'ja', 'gl'),
  (3355, 1, 'ja', 'gu'),
  (3356, 1, 'ja', 'ha'),
  (3357, 1, 'ja', 'hi'),
  (3358, 1, 'ja', 'hmn'),
  (3359, 1, 'ja', 'hr'),
  (3360, 1, 'ja', 'ht'),
  (3361, 1, 'ja', 'hu'),
  (3362, 1, 'ja', 'hy'),
  (3363, 1, 'ja', 'id'),
  (3364, 1, 'ja', 'ig'),
  (3365, 1, 'ja', 'is'),
  (3366, 1, 'ja', 'it'),
  (3367, 1, 'ja', 'iw'),
  (3368, 1, 'ja', 'jw'),
  (3369, 1, 'ja', 'ka'),
  (3370, 1, 'ja', 'kk'),
  (3371, 1, 'ja', 'km'),
  (3372, 1, 'ja', 'kn'),
  (3373, 1, 'ja', 'ko'),
  (3374, 1, 'ja', 'la'),
  (3375, 1, 'ja', 'lo'),
  (3376, 1, 'ja', 'lt'),
  (3377, 1, 'ja', 'lv'),
  (3378, 1, 'ja', 'mg'),
  (3379, 1, 'ja', 'mi'),
  (3380, 1, 'ja', 'mk'),
  (3381, 1, 'ja', 'ml'),
  (3382, 1, 'ja', 'mn'),
  (3383, 1, 'ja', 'mr'),
  (3384, 1, 'ja', 'ms'),
  (3385, 1, 'ja', 'mt'),
  (3386, 1, 'ja', 'my'),
  (3387, 1, 'ja', 'ne'),
  (3388, 1, 'ja', 'nl'),
  (3389, 1, 'ja', 'no'),
  (3390, 1, 'ja', 'ny'),
  (3391, 1, 'ja', 'pa'),
  (3392, 1, 'ja', 'pl'),
  (3393, 1, 'ja', 'pt'),
  (3394, 1, 'ja', 'ro'),
  (3395, 1, 'ja', 'ru'),
  (3396, 1, 'ja', 'si'),
  (3397, 1, 'ja', 'sk'),
  (3398, 1, 'ja', 'sl'),
  (3399, 1, 'ja', 'so'),
  (3400, 1, 'ja', 'sq'),
  (3401, 1, 'ja', 'sr'),
  (3402, 1, 'ja', 'st'),
  (3403, 1, 'ja', 'su'),
  (3404, 1, 'ja', 'sv'),
  (3405, 1, 'ja', 'sw'),
  (3406, 1, 'ja', 'ta'),
  (3407, 1, 'ja', 'te'),
  (3408, 1, 'ja', 'tg'),
  (3409, 1, 'ja', 'th'),
  (3410, 1, 'ja', 'tl'),
  (3411, 1, 'ja', 'tr'),
  (3412, 1, 'ja', 'uk'),
  (3413, 1, 'ja', 'ur'),
  (3414, 1, 'ja', 'uz'),
  (3415, 1, 'ja', 'vi'),
  (3416, 1, 'ja', 'yi'),
  (3417, 1, 'ja', 'yo'),
  (3418, 1, 'ja', 'zh'),
  (3419, 1, 'ja', 'zh-TW'),
  (3420, 1, 'ja', 'zu'),
  (3421, 1, 'jw', 'af'),
  (3422, 1, 'jw', 'ar'),
  (3423, 1, 'jw', 'az'),
  (3424, 1, 'jw', 'be'),
  (3425, 1, 'jw', 'bg'),
  (3426, 1, 'jw', 'bn'),
  (3427, 1, 'jw', 'bs'),
  (3428, 1, 'jw', 'ca'),
  (3429, 1, 'jw', 'ceb'),
  (3430, 1, 'jw', 'cs'),
  (3431, 1, 'jw', 'cy'),
  (3432, 1, 'jw', 'da'),
  (3433, 1, 'jw', 'de'),
  (3434, 1, 'jw', 'el'),
  (3435, 1, 'jw', 'en'),
  (3436, 1, 'jw', 'eo'),
  (3437, 1, 'jw', 'es'),
  (3438, 1, 'jw', 'et'),
  (3439, 1, 'jw', 'eu'),
  (3440, 1, 'jw', 'fa'),
  (3441, 1, 'jw', 'fi'),
  (3442, 1, 'jw', 'fr'),
  (3443, 1, 'jw', 'ga'),
  (3444, 1, 'jw', 'gl'),
  (3445, 1, 'jw', 'gu'),
  (3446, 1, 'jw', 'ha'),
  (3447, 1, 'jw', 'hi'),
  (3448, 1, 'jw', 'hmn'),
  (3449, 1, 'jw', 'hr'),
  (3450, 1, 'jw', 'ht'),
  (3451, 1, 'jw', 'hu'),
  (3452, 1, 'jw', 'hy'),
  (3453, 1, 'jw', 'id'),
  (3454, 1, 'jw', 'ig'),
  (3455, 1, 'jw', 'is'),
  (3456, 1, 'jw', 'it'),
  (3457, 1, 'jw', 'iw'),
  (3458, 1, 'jw', 'ja'),
  (3459, 1, 'jw', 'ka'),
  (3460, 1, 'jw', 'kk'),
  (3461, 1, 'jw', 'km'),
  (3462, 1, 'jw', 'kn'),
  (3463, 1, 'jw', 'ko'),
  (3464, 1, 'jw', 'la'),
  (3465, 1, 'jw', 'lo'),
  (3466, 1, 'jw', 'lt'),
  (3467, 1, 'jw', 'lv'),
  (3468, 1, 'jw', 'mg'),
  (3469, 1, 'jw', 'mi'),
  (3470, 1, 'jw', 'mk'),
  (3471, 1, 'jw', 'ml'),
  (3472, 1, 'jw', 'mn'),
  (3473, 1, 'jw', 'mr'),
  (3474, 1, 'jw', 'ms'),
  (3475, 1, 'jw', 'mt'),
  (3476, 1, 'jw', 'my'),
  (3477, 1, 'jw', 'ne'),
  (3478, 1, 'jw', 'nl'),
  (3479, 1, 'jw', 'no'),
  (3480, 1, 'jw', 'ny'),
  (3481, 1, 'jw', 'pa'),
  (3482, 1, 'jw', 'pl'),
  (3483, 1, 'jw', 'pt'),
  (3484, 1, 'jw', 'ro'),
  (3485, 1, 'jw', 'ru'),
  (3486, 1, 'jw', 'si'),
  (3487, 1, 'jw', 'sk'),
  (3488, 1, 'jw', 'sl'),
  (3489, 1, 'jw', 'so'),
  (3490, 1, 'jw', 'sq'),
  (3491, 1, 'jw', 'sr'),
  (3492, 1, 'jw', 'st'),
  (3493, 1, 'jw', 'su'),
  (3494, 1, 'jw', 'sv'),
  (3495, 1, 'jw', 'sw'),
  (3496, 1, 'jw', 'ta'),
  (3497, 1, 'jw', 'te'),
  (3498, 1, 'jw', 'tg'),
  (3499, 1, 'jw', 'th'),
  (3500, 1, 'jw', 'tl'),
  (3501, 1, 'jw', 'tr'),
  (3502, 1, 'jw', 'uk'),
  (3503, 1, 'jw', 'ur'),
  (3504, 1, 'jw', 'uz'),
  (3505, 1, 'jw', 'vi'),
  (3506, 1, 'jw', 'yi'),
  (3507, 1, 'jw', 'yo'),
  (3508, 1, 'jw', 'zh'),
  (3509, 1, 'jw', 'zh-TW'),
  (3510, 1, 'jw', 'zu'),
  (3511, 1, 'ka', 'af'),
  (3512, 1, 'ka', 'ar'),
  (3513, 1, 'ka', 'az'),
  (3514, 1, 'ka', 'be'),
  (3515, 1, 'ka', 'bg'),
  (3516, 1, 'ka', 'bn'),
  (3517, 1, 'ka', 'bs'),
  (3518, 1, 'ka', 'ca'),
  (3519, 1, 'ka', 'ceb'),
  (3520, 1, 'ka', 'cs'),
  (3521, 1, 'ka', 'cy'),
  (3522, 1, 'ka', 'da'),
  (3523, 1, 'ka', 'de'),
  (3524, 1, 'ka', 'el'),
  (3525, 1, 'ka', 'en'),
  (3526, 1, 'ka', 'eo'),
  (3527, 1, 'ka', 'es'),
  (3528, 1, 'ka', 'et'),
  (3529, 1, 'ka', 'eu'),
  (3530, 1, 'ka', 'fa'),
  (3531, 1, 'ka', 'fi'),
  (3532, 1, 'ka', 'fr'),
  (3533, 1, 'ka', 'ga'),
  (3534, 1, 'ka', 'gl'),
  (3535, 1, 'ka', 'gu'),
  (3536, 1, 'ka', 'ha'),
  (3537, 1, 'ka', 'hi'),
  (3538, 1, 'ka', 'hmn'),
  (3539, 1, 'ka', 'hr'),
  (3540, 1, 'ka', 'ht'),
  (3541, 1, 'ka', 'hu'),
  (3542, 1, 'ka', 'hy'),
  (3543, 1, 'ka', 'id'),
  (3544, 1, 'ka', 'ig'),
  (3545, 1, 'ka', 'is'),
  (3546, 1, 'ka', 'it'),
  (3547, 1, 'ka', 'iw'),
  (3548, 1, 'ka', 'ja'),
  (3549, 1, 'ka', 'jw'),
  (3550, 1, 'ka', 'kk'),
  (3551, 1, 'ka', 'km'),
  (3552, 1, 'ka', 'kn'),
  (3553, 1, 'ka', 'ko'),
  (3554, 1, 'ka', 'la'),
  (3555, 1, 'ka', 'lo'),
  (3556, 1, 'ka', 'lt'),
  (3557, 1, 'ka', 'lv'),
  (3558, 1, 'ka', 'mg'),
  (3559, 1, 'ka', 'mi'),
  (3560, 1, 'ka', 'mk'),
  (3561, 1, 'ka', 'ml'),
  (3562, 1, 'ka', 'mn'),
  (3563, 1, 'ka', 'mr'),
  (3564, 1, 'ka', 'ms'),
  (3565, 1, 'ka', 'mt'),
  (3566, 1, 'ka', 'my'),
  (3567, 1, 'ka', 'ne'),
  (3568, 1, 'ka', 'nl'),
  (3569, 1, 'ka', 'no'),
  (3570, 1, 'ka', 'ny'),
  (3571, 1, 'ka', 'pa'),
  (3572, 1, 'ka', 'pl'),
  (3573, 1, 'ka', 'pt'),
  (3574, 1, 'ka', 'ro'),
  (3575, 1, 'ka', 'ru'),
  (3576, 1, 'ka', 'si'),
  (3577, 1, 'ka', 'sk'),
  (3578, 1, 'ka', 'sl'),
  (3579, 1, 'ka', 'so'),
  (3580, 1, 'ka', 'sq'),
  (3581, 1, 'ka', 'sr'),
  (3582, 1, 'ka', 'st'),
  (3583, 1, 'ka', 'su'),
  (3584, 1, 'ka', 'sv'),
  (3585, 1, 'ka', 'sw'),
  (3586, 1, 'ka', 'ta'),
  (3587, 1, 'ka', 'te'),
  (3588, 1, 'ka', 'tg'),
  (3589, 1, 'ka', 'th'),
  (3590, 1, 'ka', 'tl'),
  (3591, 1, 'ka', 'tr'),
  (3592, 1, 'ka', 'uk'),
  (3593, 1, 'ka', 'ur'),
  (3594, 1, 'ka', 'uz'),
  (3595, 1, 'ka', 'vi'),
  (3596, 1, 'ka', 'yi'),
  (3597, 1, 'ka', 'yo'),
  (3598, 1, 'ka', 'zh'),
  (3599, 1, 'ka', 'zh-TW'),
  (3600, 1, 'ka', 'zu'),
  (3601, 1, 'kk', 'af'),
  (3602, 1, 'kk', 'ar'),
  (3603, 1, 'kk', 'az'),
  (3604, 1, 'kk', 'be'),
  (3605, 1, 'kk', 'bg'),
  (3606, 1, 'kk', 'bn'),
  (3607, 1, 'kk', 'bs'),
  (3608, 1, 'kk', 'ca'),
  (3609, 1, 'kk', 'ceb'),
  (3610, 1, 'kk', 'cs'),
  (3611, 1, 'kk', 'cy'),
  (3612, 1, 'kk', 'da'),
  (3613, 1, 'kk', 'de'),
  (3614, 1, 'kk', 'el'),
  (3615, 1, 'kk', 'en'),
  (3616, 1, 'kk', 'eo'),
  (3617, 1, 'kk', 'es'),
  (3618, 1, 'kk', 'et'),
  (3619, 1, 'kk', 'eu'),
  (3620, 1, 'kk', 'fa'),
  (3621, 1, 'kk', 'fi'),
  (3622, 1, 'kk', 'fr'),
  (3623, 1, 'kk', 'ga'),
  (3624, 1, 'kk', 'gl'),
  (3625, 1, 'kk', 'gu'),
  (3626, 1, 'kk', 'ha'),
  (3627, 1, 'kk', 'hi'),
  (3628, 1, 'kk', 'hmn'),
  (3629, 1, 'kk', 'hr'),
  (3630, 1, 'kk', 'ht'),
  (3631, 1, 'kk', 'hu'),
  (3632, 1, 'kk', 'hy'),
  (3633, 1, 'kk', 'id'),
  (3634, 1, 'kk', 'ig'),
  (3635, 1, 'kk', 'is'),
  (3636, 1, 'kk', 'it'),
  (3637, 1, 'kk', 'iw'),
  (3638, 1, 'kk', 'ja'),
  (3639, 1, 'kk', 'jw'),
  (3640, 1, 'kk', 'ka'),
  (3641, 1, 'kk', 'km'),
  (3642, 1, 'kk', 'kn'),
  (3643, 1, 'kk', 'ko'),
  (3644, 1, 'kk', 'la'),
  (3645, 1, 'kk', 'lo'),
  (3646, 1, 'kk', 'lt'),
  (3647, 1, 'kk', 'lv'),
  (3648, 1, 'kk', 'mg'),
  (3649, 1, 'kk', 'mi'),
  (3650, 1, 'kk', 'mk'),
  (3651, 1, 'kk', 'ml'),
  (3652, 1, 'kk', 'mn'),
  (3653, 1, 'kk', 'mr'),
  (3654, 1, 'kk', 'ms'),
  (3655, 1, 'kk', 'mt'),
  (3656, 1, 'kk', 'my'),
  (3657, 1, 'kk', 'ne'),
  (3658, 1, 'kk', 'nl'),
  (3659, 1, 'kk', 'no'),
  (3660, 1, 'kk', 'ny'),
  (3661, 1, 'kk', 'pa'),
  (3662, 1, 'kk', 'pl'),
  (3663, 1, 'kk', 'pt'),
  (3664, 1, 'kk', 'ro'),
  (3665, 1, 'kk', 'ru'),
  (3666, 1, 'kk', 'si'),
  (3667, 1, 'kk', 'sk'),
  (3668, 1, 'kk', 'sl'),
  (3669, 1, 'kk', 'so'),
  (3670, 1, 'kk', 'sq'),
  (3671, 1, 'kk', 'sr'),
  (3672, 1, 'kk', 'st'),
  (3673, 1, 'kk', 'su'),
  (3674, 1, 'kk', 'sv'),
  (3675, 1, 'kk', 'sw'),
  (3676, 1, 'kk', 'ta'),
  (3677, 1, 'kk', 'te'),
  (3678, 1, 'kk', 'tg'),
  (3679, 1, 'kk', 'th'),
  (3680, 1, 'kk', 'tl'),
  (3681, 1, 'kk', 'tr'),
  (3682, 1, 'kk', 'uk'),
  (3683, 1, 'kk', 'ur'),
  (3684, 1, 'kk', 'uz'),
  (3685, 1, 'kk', 'vi'),
  (3686, 1, 'kk', 'yi'),
  (3687, 1, 'kk', 'yo'),
  (3688, 1, 'kk', 'zh'),
  (3689, 1, 'kk', 'zh-TW'),
  (3690, 1, 'kk', 'zu'),
  (3691, 1, 'km', 'af'),
  (3692, 1, 'km', 'ar'),
  (3693, 1, 'km', 'az'),
  (3694, 1, 'km', 'be'),
  (3695, 1, 'km', 'bg'),
  (3696, 1, 'km', 'bn'),
  (3697, 1, 'km', 'bs'),
  (3698, 1, 'km', 'ca'),
  (3699, 1, 'km', 'ceb'),
  (3700, 1, 'km', 'cs'),
  (3701, 1, 'km', 'cy'),
  (3702, 1, 'km', 'da'),
  (3703, 1, 'km', 'de'),
  (3704, 1, 'km', 'el'),
  (3705, 1, 'km', 'en'),
  (3706, 1, 'km', 'eo'),
  (3707, 1, 'km', 'es'),
  (3708, 1, 'km', 'et'),
  (3709, 1, 'km', 'eu'),
  (3710, 1, 'km', 'fa'),
  (3711, 1, 'km', 'fi'),
  (3712, 1, 'km', 'fr'),
  (3713, 1, 'km', 'ga'),
  (3714, 1, 'km', 'gl'),
  (3715, 1, 'km', 'gu'),
  (3716, 1, 'km', 'ha'),
  (3717, 1, 'km', 'hi'),
  (3718, 1, 'km', 'hmn'),
  (3719, 1, 'km', 'hr'),
  (3720, 1, 'km', 'ht'),
  (3721, 1, 'km', 'hu'),
  (3722, 1, 'km', 'hy'),
  (3723, 1, 'km', 'id'),
  (3724, 1, 'km', 'ig'),
  (3725, 1, 'km', 'is'),
  (3726, 1, 'km', 'it'),
  (3727, 1, 'km', 'iw'),
  (3728, 1, 'km', 'ja'),
  (3729, 1, 'km', 'jw'),
  (3730, 1, 'km', 'ka'),
  (3731, 1, 'km', 'kk'),
  (3732, 1, 'km', 'kn'),
  (3733, 1, 'km', 'ko'),
  (3734, 1, 'km', 'la'),
  (3735, 1, 'km', 'lo'),
  (3736, 1, 'km', 'lt'),
  (3737, 1, 'km', 'lv'),
  (3738, 1, 'km', 'mg'),
  (3739, 1, 'km', 'mi'),
  (3740, 1, 'km', 'mk'),
  (3741, 1, 'km', 'ml'),
  (3742, 1, 'km', 'mn'),
  (3743, 1, 'km', 'mr'),
  (3744, 1, 'km', 'ms'),
  (3745, 1, 'km', 'mt'),
  (3746, 1, 'km', 'my'),
  (3747, 1, 'km', 'ne'),
  (3748, 1, 'km', 'nl'),
  (3749, 1, 'km', 'no'),
  (3750, 1, 'km', 'ny'),
  (3751, 1, 'km', 'pa'),
  (3752, 1, 'km', 'pl'),
  (3753, 1, 'km', 'pt'),
  (3754, 1, 'km', 'ro'),
  (3755, 1, 'km', 'ru'),
  (3756, 1, 'km', 'si'),
  (3757, 1, 'km', 'sk'),
  (3758, 1, 'km', 'sl'),
  (3759, 1, 'km', 'so'),
  (3760, 1, 'km', 'sq'),
  (3761, 1, 'km', 'sr'),
  (3762, 1, 'km', 'st'),
  (3763, 1, 'km', 'su'),
  (3764, 1, 'km', 'sv'),
  (3765, 1, 'km', 'sw'),
  (3766, 1, 'km', 'ta'),
  (3767, 1, 'km', 'te'),
  (3768, 1, 'km', 'tg'),
  (3769, 1, 'km', 'th'),
  (3770, 1, 'km', 'tl'),
  (3771, 1, 'km', 'tr'),
  (3772, 1, 'km', 'uk'),
  (3773, 1, 'km', 'ur'),
  (3774, 1, 'km', 'uz'),
  (3775, 1, 'km', 'vi'),
  (3776, 1, 'km', 'yi'),
  (3777, 1, 'km', 'yo'),
  (3778, 1, 'km', 'zh'),
  (3779, 1, 'km', 'zh-TW'),
  (3780, 1, 'km', 'zu'),
  (3781, 1, 'kn', 'af'),
  (3782, 1, 'kn', 'ar'),
  (3783, 1, 'kn', 'az'),
  (3784, 1, 'kn', 'be'),
  (3785, 1, 'kn', 'bg'),
  (3786, 1, 'kn', 'bn'),
  (3787, 1, 'kn', 'bs'),
  (3788, 1, 'kn', 'ca'),
  (3789, 1, 'kn', 'ceb'),
  (3790, 1, 'kn', 'cs'),
  (3791, 1, 'kn', 'cy'),
  (3792, 1, 'kn', 'da'),
  (3793, 1, 'kn', 'de'),
  (3794, 1, 'kn', 'el'),
  (3795, 1, 'kn', 'en'),
  (3796, 1, 'kn', 'eo'),
  (3797, 1, 'kn', 'es'),
  (3798, 1, 'kn', 'et'),
  (3799, 1, 'kn', 'eu'),
  (3800, 1, 'kn', 'fa'),
  (3801, 1, 'kn', 'fi'),
  (3802, 1, 'kn', 'fr'),
  (3803, 1, 'kn', 'ga'),
  (3804, 1, 'kn', 'gl'),
  (3805, 1, 'kn', 'gu'),
  (3806, 1, 'kn', 'ha'),
  (3807, 1, 'kn', 'hi'),
  (3808, 1, 'kn', 'hmn'),
  (3809, 1, 'kn', 'hr'),
  (3810, 1, 'kn', 'ht'),
  (3811, 1, 'kn', 'hu'),
  (3812, 1, 'kn', 'hy'),
  (3813, 1, 'kn', 'id'),
  (3814, 1, 'kn', 'ig'),
  (3815, 1, 'kn', 'is'),
  (3816, 1, 'kn', 'it'),
  (3817, 1, 'kn', 'iw'),
  (3818, 1, 'kn', 'ja'),
  (3819, 1, 'kn', 'jw'),
  (3820, 1, 'kn', 'ka'),
  (3821, 1, 'kn', 'kk'),
  (3822, 1, 'kn', 'km'),
  (3823, 1, 'kn', 'ko'),
  (3824, 1, 'kn', 'la'),
  (3825, 1, 'kn', 'lo'),
  (3826, 1, 'kn', 'lt'),
  (3827, 1, 'kn', 'lv'),
  (3828, 1, 'kn', 'mg'),
  (3829, 1, 'kn', 'mi'),
  (3830, 1, 'kn', 'mk'),
  (3831, 1, 'kn', 'ml'),
  (3832, 1, 'kn', 'mn'),
  (3833, 1, 'kn', 'mr'),
  (3834, 1, 'kn', 'ms'),
  (3835, 1, 'kn', 'mt'),
  (3836, 1, 'kn', 'my'),
  (3837, 1, 'kn', 'ne'),
  (3838, 1, 'kn', 'nl'),
  (3839, 1, 'kn', 'no'),
  (3840, 1, 'kn', 'ny'),
  (3841, 1, 'kn', 'pa'),
  (3842, 1, 'kn', 'pl'),
  (3843, 1, 'kn', 'pt'),
  (3844, 1, 'kn', 'ro'),
  (3845, 1, 'kn', 'ru'),
  (3846, 1, 'kn', 'si'),
  (3847, 1, 'kn', 'sk'),
  (3848, 1, 'kn', 'sl'),
  (3849, 1, 'kn', 'so'),
  (3850, 1, 'kn', 'sq'),
  (3851, 1, 'kn', 'sr'),
  (3852, 1, 'kn', 'st'),
  (3853, 1, 'kn', 'su'),
  (3854, 1, 'kn', 'sv'),
  (3855, 1, 'kn', 'sw'),
  (3856, 1, 'kn', 'ta'),
  (3857, 1, 'kn', 'te'),
  (3858, 1, 'kn', 'tg'),
  (3859, 1, 'kn', 'th'),
  (3860, 1, 'kn', 'tl'),
  (3861, 1, 'kn', 'tr'),
  (3862, 1, 'kn', 'uk'),
  (3863, 1, 'kn', 'ur'),
  (3864, 1, 'kn', 'uz'),
  (3865, 1, 'kn', 'vi'),
  (3866, 1, 'kn', 'yi'),
  (3867, 1, 'kn', 'yo'),
  (3868, 1, 'kn', 'zh'),
  (3869, 1, 'kn', 'zh-TW'),
  (3870, 1, 'kn', 'zu'),
  (3871, 1, 'ko', 'af'),
  (3872, 1, 'ko', 'ar'),
  (3873, 1, 'ko', 'az'),
  (3874, 1, 'ko', 'be'),
  (3875, 1, 'ko', 'bg'),
  (3876, 1, 'ko', 'bn'),
  (3877, 1, 'ko', 'bs'),
  (3878, 1, 'ko', 'ca'),
  (3879, 1, 'ko', 'ceb'),
  (3880, 1, 'ko', 'cs'),
  (3881, 1, 'ko', 'cy'),
  (3882, 1, 'ko', 'da'),
  (3883, 1, 'ko', 'de'),
  (3884, 1, 'ko', 'el'),
  (3885, 1, 'ko', 'en'),
  (3886, 1, 'ko', 'eo'),
  (3887, 1, 'ko', 'es'),
  (3888, 1, 'ko', 'et'),
  (3889, 1, 'ko', 'eu'),
  (3890, 1, 'ko', 'fa'),
  (3891, 1, 'ko', 'fi'),
  (3892, 1, 'ko', 'fr'),
  (3893, 1, 'ko', 'ga'),
  (3894, 1, 'ko', 'gl'),
  (3895, 1, 'ko', 'gu'),
  (3896, 1, 'ko', 'ha'),
  (3897, 1, 'ko', 'hi'),
  (3898, 1, 'ko', 'hmn'),
  (3899, 1, 'ko', 'hr'),
  (3900, 1, 'ko', 'ht'),
  (3901, 1, 'ko', 'hu'),
  (3902, 1, 'ko', 'hy'),
  (3903, 1, 'ko', 'id'),
  (3904, 1, 'ko', 'ig'),
  (3905, 1, 'ko', 'is'),
  (3906, 1, 'ko', 'it'),
  (3907, 1, 'ko', 'iw'),
  (3908, 1, 'ko', 'ja'),
  (3909, 1, 'ko', 'jw'),
  (3910, 1, 'ko', 'ka'),
  (3911, 1, 'ko', 'kk'),
  (3912, 1, 'ko', 'km'),
  (3913, 1, 'ko', 'kn'),
  (3914, 1, 'ko', 'la'),
  (3915, 1, 'ko', 'lo'),
  (3916, 1, 'ko', 'lt'),
  (3917, 1, 'ko', 'lv'),
  (3918, 1, 'ko', 'mg'),
  (3919, 1, 'ko', 'mi'),
  (3920, 1, 'ko', 'mk'),
  (3921, 1, 'ko', 'ml'),
  (3922, 1, 'ko', 'mn'),
  (3923, 1, 'ko', 'mr'),
  (3924, 1, 'ko', 'ms'),
  (3925, 1, 'ko', 'mt'),
  (3926, 1, 'ko', 'my'),
  (3927, 1, 'ko', 'ne'),
  (3928, 1, 'ko', 'nl'),
  (3929, 1, 'ko', 'no'),
  (3930, 1, 'ko', 'ny'),
  (3931, 1, 'ko', 'pa'),
  (3932, 1, 'ko', 'pl'),
  (3933, 1, 'ko', 'pt'),
  (3934, 1, 'ko', 'ro'),
  (3935, 1, 'ko', 'ru'),
  (3936, 1, 'ko', 'si'),
  (3937, 1, 'ko', 'sk'),
  (3938, 1, 'ko', 'sl'),
  (3939, 1, 'ko', 'so'),
  (3940, 1, 'ko', 'sq'),
  (3941, 1, 'ko', 'sr'),
  (3942, 1, 'ko', 'st'),
  (3943, 1, 'ko', 'su'),
  (3944, 1, 'ko', 'sv'),
  (3945, 1, 'ko', 'sw'),
  (3946, 1, 'ko', 'ta'),
  (3947, 1, 'ko', 'te'),
  (3948, 1, 'ko', 'tg'),
  (3949, 1, 'ko', 'th'),
  (3950, 1, 'ko', 'tl'),
  (3951, 1, 'ko', 'tr'),
  (3952, 1, 'ko', 'uk'),
  (3953, 1, 'ko', 'ur'),
  (3954, 1, 'ko', 'uz'),
  (3955, 1, 'ko', 'vi'),
  (3956, 1, 'ko', 'yi'),
  (3957, 1, 'ko', 'yo'),
  (3958, 1, 'ko', 'zh'),
  (3959, 1, 'ko', 'zh-TW'),
  (3960, 1, 'ko', 'zu'),
  (3961, 1, 'la', 'af'),
  (3962, 1, 'la', 'ar'),
  (3963, 1, 'la', 'az'),
  (3964, 1, 'la', 'be'),
  (3965, 1, 'la', 'bg'),
  (3966, 1, 'la', 'bn'),
  (3967, 1, 'la', 'bs'),
  (3968, 1, 'la', 'ca'),
  (3969, 1, 'la', 'ceb'),
  (3970, 1, 'la', 'cs'),
  (3971, 1, 'la', 'cy'),
  (3972, 1, 'la', 'da'),
  (3973, 1, 'la', 'de'),
  (3974, 1, 'la', 'el'),
  (3975, 1, 'la', 'en'),
  (3976, 1, 'la', 'eo'),
  (3977, 1, 'la', 'es'),
  (3978, 1, 'la', 'et'),
  (3979, 1, 'la', 'eu'),
  (3980, 1, 'la', 'fa'),
  (3981, 1, 'la', 'fi'),
  (3982, 1, 'la', 'fr'),
  (3983, 1, 'la', 'ga'),
  (3984, 1, 'la', 'gl'),
  (3985, 1, 'la', 'gu'),
  (3986, 1, 'la', 'ha'),
  (3987, 1, 'la', 'hi'),
  (3988, 1, 'la', 'hmn'),
  (3989, 1, 'la', 'hr'),
  (3990, 1, 'la', 'ht'),
  (3991, 1, 'la', 'hu'),
  (3992, 1, 'la', 'hy'),
  (3993, 1, 'la', 'id'),
  (3994, 1, 'la', 'ig'),
  (3995, 1, 'la', 'is'),
  (3996, 1, 'la', 'it'),
  (3997, 1, 'la', 'iw'),
  (3998, 1, 'la', 'ja'),
  (3999, 1, 'la', 'jw'),
  (4000, 1, 'la', 'ka'),
  (4001, 1, 'la', 'kk'),
  (4002, 1, 'la', 'km'),
  (4003, 1, 'la', 'kn'),
  (4004, 1, 'la', 'ko'),
  (4005, 1, 'la', 'lo'),
  (4006, 1, 'la', 'lt'),
  (4007, 1, 'la', 'lv'),
  (4008, 1, 'la', 'mg'),
  (4009, 1, 'la', 'mi'),
  (4010, 1, 'la', 'mk'),
  (4011, 1, 'la', 'ml'),
  (4012, 1, 'la', 'mn'),
  (4013, 1, 'la', 'mr'),
  (4014, 1, 'la', 'ms'),
  (4015, 1, 'la', 'mt'),
  (4016, 1, 'la', 'my'),
  (4017, 1, 'la', 'ne'),
  (4018, 1, 'la', 'nl'),
  (4019, 1, 'la', 'no'),
  (4020, 1, 'la', 'ny'),
  (4021, 1, 'la', 'pa'),
  (4022, 1, 'la', 'pl'),
  (4023, 1, 'la', 'pt'),
  (4024, 1, 'la', 'ro'),
  (4025, 1, 'la', 'ru'),
  (4026, 1, 'la', 'si'),
  (4027, 1, 'la', 'sk'),
  (4028, 1, 'la', 'sl'),
  (4029, 1, 'la', 'so'),
  (4030, 1, 'la', 'sq'),
  (4031, 1, 'la', 'sr'),
  (4032, 1, 'la', 'st'),
  (4033, 1, 'la', 'su'),
  (4034, 1, 'la', 'sv'),
  (4035, 1, 'la', 'sw'),
  (4036, 1, 'la', 'ta'),
  (4037, 1, 'la', 'te'),
  (4038, 1, 'la', 'tg'),
  (4039, 1, 'la', 'th'),
  (4040, 1, 'la', 'tl'),
  (4041, 1, 'la', 'tr'),
  (4042, 1, 'la', 'uk'),
  (4043, 1, 'la', 'ur'),
  (4044, 1, 'la', 'uz'),
  (4045, 1, 'la', 'vi'),
  (4046, 1, 'la', 'yi'),
  (4047, 1, 'la', 'yo'),
  (4048, 1, 'la', 'zh'),
  (4049, 1, 'la', 'zh-TW'),
  (4050, 1, 'la', 'zu'),
  (4051, 1, 'lo', 'af'),
  (4052, 1, 'lo', 'ar'),
  (4053, 1, 'lo', 'az'),
  (4054, 1, 'lo', 'be'),
  (4055, 1, 'lo', 'bg'),
  (4056, 1, 'lo', 'bn'),
  (4057, 1, 'lo', 'bs'),
  (4058, 1, 'lo', 'ca'),
  (4059, 1, 'lo', 'ceb'),
  (4060, 1, 'lo', 'cs'),
  (4061, 1, 'lo', 'cy'),
  (4062, 1, 'lo', 'da'),
  (4063, 1, 'lo', 'de'),
  (4064, 1, 'lo', 'el'),
  (4065, 1, 'lo', 'en'),
  (4066, 1, 'lo', 'eo'),
  (4067, 1, 'lo', 'es'),
  (4068, 1, 'lo', 'et'),
  (4069, 1, 'lo', 'eu'),
  (4070, 1, 'lo', 'fa'),
  (4071, 1, 'lo', 'fi'),
  (4072, 1, 'lo', 'fr'),
  (4073, 1, 'lo', 'ga'),
  (4074, 1, 'lo', 'gl'),
  (4075, 1, 'lo', 'gu'),
  (4076, 1, 'lo', 'ha'),
  (4077, 1, 'lo', 'hi'),
  (4078, 1, 'lo', 'hmn'),
  (4079, 1, 'lo', 'hr'),
  (4080, 1, 'lo', 'ht'),
  (4081, 1, 'lo', 'hu'),
  (4082, 1, 'lo', 'hy'),
  (4083, 1, 'lo', 'id'),
  (4084, 1, 'lo', 'ig'),
  (4085, 1, 'lo', 'is'),
  (4086, 1, 'lo', 'it'),
  (4087, 1, 'lo', 'iw'),
  (4088, 1, 'lo', 'ja'),
  (4089, 1, 'lo', 'jw'),
  (4090, 1, 'lo', 'ka'),
  (4091, 1, 'lo', 'kk'),
  (4092, 1, 'lo', 'km'),
  (4093, 1, 'lo', 'kn'),
  (4094, 1, 'lo', 'ko'),
  (4095, 1, 'lo', 'la'),
  (4096, 1, 'lo', 'lt'),
  (4097, 1, 'lo', 'lv'),
  (4098, 1, 'lo', 'mg'),
  (4099, 1, 'lo', 'mi'),
  (4100, 1, 'lo', 'mk'),
  (4101, 1, 'lo', 'ml'),
  (4102, 1, 'lo', 'mn'),
  (4103, 1, 'lo', 'mr'),
  (4104, 1, 'lo', 'ms'),
  (4105, 1, 'lo', 'mt'),
  (4106, 1, 'lo', 'my'),
  (4107, 1, 'lo', 'ne'),
  (4108, 1, 'lo', 'nl'),
  (4109, 1, 'lo', 'no'),
  (4110, 1, 'lo', 'ny'),
  (4111, 1, 'lo', 'pa'),
  (4112, 1, 'lo', 'pl'),
  (4113, 1, 'lo', 'pt'),
  (4114, 1, 'lo', 'ro'),
  (4115, 1, 'lo', 'ru'),
  (4116, 1, 'lo', 'si'),
  (4117, 1, 'lo', 'sk'),
  (4118, 1, 'lo', 'sl'),
  (4119, 1, 'lo', 'so'),
  (4120, 1, 'lo', 'sq'),
  (4121, 1, 'lo', 'sr'),
  (4122, 1, 'lo', 'st'),
  (4123, 1, 'lo', 'su'),
  (4124, 1, 'lo', 'sv'),
  (4125, 1, 'lo', 'sw'),
  (4126, 1, 'lo', 'ta'),
  (4127, 1, 'lo', 'te'),
  (4128, 1, 'lo', 'tg'),
  (4129, 1, 'lo', 'th'),
  (4130, 1, 'lo', 'tl'),
  (4131, 1, 'lo', 'tr'),
  (4132, 1, 'lo', 'uk'),
  (4133, 1, 'lo', 'ur'),
  (4134, 1, 'lo', 'uz'),
  (4135, 1, 'lo', 'vi'),
  (4136, 1, 'lo', 'yi'),
  (4137, 1, 'lo', 'yo'),
  (4138, 1, 'lo', 'zh'),
  (4139, 1, 'lo', 'zh-TW'),
  (4140, 1, 'lo', 'zu'),
  (4141, 1, 'lt', 'af'),
  (4142, 1, 'lt', 'ar'),
  (4143, 1, 'lt', 'az'),
  (4144, 1, 'lt', 'be'),
  (4145, 1, 'lt', 'bg'),
  (4146, 1, 'lt', 'bn'),
  (4147, 1, 'lt', 'bs'),
  (4148, 1, 'lt', 'ca'),
  (4149, 1, 'lt', 'ceb'),
  (4150, 1, 'lt', 'cs'),
  (4151, 1, 'lt', 'cy'),
  (4152, 1, 'lt', 'da'),
  (4153, 1, 'lt', 'de'),
  (4154, 1, 'lt', 'el'),
  (4155, 1, 'lt', 'en'),
  (4156, 1, 'lt', 'eo'),
  (4157, 1, 'lt', 'es'),
  (4158, 1, 'lt', 'et'),
  (4159, 1, 'lt', 'eu'),
  (4160, 1, 'lt', 'fa'),
  (4161, 1, 'lt', 'fi'),
  (4162, 1, 'lt', 'fr'),
  (4163, 1, 'lt', 'ga'),
  (4164, 1, 'lt', 'gl'),
  (4165, 1, 'lt', 'gu'),
  (4166, 1, 'lt', 'ha'),
  (4167, 1, 'lt', 'hi'),
  (4168, 1, 'lt', 'hmn'),
  (4169, 1, 'lt', 'hr'),
  (4170, 1, 'lt', 'ht'),
  (4171, 1, 'lt', 'hu'),
  (4172, 1, 'lt', 'hy'),
  (4173, 1, 'lt', 'id'),
  (4174, 1, 'lt', 'ig'),
  (4175, 1, 'lt', 'is'),
  (4176, 1, 'lt', 'it'),
  (4177, 1, 'lt', 'iw'),
  (4178, 1, 'lt', 'ja'),
  (4179, 1, 'lt', 'jw'),
  (4180, 1, 'lt', 'ka'),
  (4181, 1, 'lt', 'kk'),
  (4182, 1, 'lt', 'km'),
  (4183, 1, 'lt', 'kn'),
  (4184, 1, 'lt', 'ko'),
  (4185, 1, 'lt', 'la'),
  (4186, 1, 'lt', 'lo'),
  (4187, 1, 'lt', 'lv'),
  (4188, 1, 'lt', 'mg'),
  (4189, 1, 'lt', 'mi'),
  (4190, 1, 'lt', 'mk'),
  (4191, 1, 'lt', 'ml'),
  (4192, 1, 'lt', 'mn'),
  (4193, 1, 'lt', 'mr'),
  (4194, 1, 'lt', 'ms'),
  (4195, 1, 'lt', 'mt'),
  (4196, 1, 'lt', 'my'),
  (4197, 1, 'lt', 'ne'),
  (4198, 1, 'lt', 'nl'),
  (4199, 1, 'lt', 'no'),
  (4200, 1, 'lt', 'ny'),
  (4201, 1, 'lt', 'pa'),
  (4202, 1, 'lt', 'pl'),
  (4203, 1, 'lt', 'pt'),
  (4204, 1, 'lt', 'ro'),
  (4205, 1, 'lt', 'ru'),
  (4206, 1, 'lt', 'si'),
  (4207, 1, 'lt', 'sk'),
  (4208, 1, 'lt', 'sl'),
  (4209, 1, 'lt', 'so'),
  (4210, 1, 'lt', 'sq'),
  (4211, 1, 'lt', 'sr'),
  (4212, 1, 'lt', 'st'),
  (4213, 1, 'lt', 'su'),
  (4214, 1, 'lt', 'sv'),
  (4215, 1, 'lt', 'sw'),
  (4216, 1, 'lt', 'ta'),
  (4217, 1, 'lt', 'te'),
  (4218, 1, 'lt', 'tg'),
  (4219, 1, 'lt', 'th'),
  (4220, 1, 'lt', 'tl'),
  (4221, 1, 'lt', 'tr'),
  (4222, 1, 'lt', 'uk'),
  (4223, 1, 'lt', 'ur'),
  (4224, 1, 'lt', 'uz'),
  (4225, 1, 'lt', 'vi'),
  (4226, 1, 'lt', 'yi'),
  (4227, 1, 'lt', 'yo'),
  (4228, 1, 'lt', 'zh'),
  (4229, 1, 'lt', 'zh-TW'),
  (4230, 1, 'lt', 'zu'),
  (4231, 1, 'lv', 'af'),
  (4232, 1, 'lv', 'ar'),
  (4233, 1, 'lv', 'az'),
  (4234, 1, 'lv', 'be'),
  (4235, 1, 'lv', 'bg'),
  (4236, 1, 'lv', 'bn'),
  (4237, 1, 'lv', 'bs'),
  (4238, 1, 'lv', 'ca'),
  (4239, 1, 'lv', 'ceb'),
  (4240, 1, 'lv', 'cs'),
  (4241, 1, 'lv', 'cy'),
  (4242, 1, 'lv', 'da'),
  (4243, 1, 'lv', 'de'),
  (4244, 1, 'lv', 'el'),
  (4245, 1, 'lv', 'en'),
  (4246, 1, 'lv', 'eo'),
  (4247, 1, 'lv', 'es'),
  (4248, 1, 'lv', 'et'),
  (4249, 1, 'lv', 'eu'),
  (4250, 1, 'lv', 'fa'),
  (4251, 1, 'lv', 'fi'),
  (4252, 1, 'lv', 'fr'),
  (4253, 1, 'lv', 'ga'),
  (4254, 1, 'lv', 'gl'),
  (4255, 1, 'lv', 'gu'),
  (4256, 1, 'lv', 'ha'),
  (4257, 1, 'lv', 'hi'),
  (4258, 1, 'lv', 'hmn'),
  (4259, 1, 'lv', 'hr'),
  (4260, 1, 'lv', 'ht'),
  (4261, 1, 'lv', 'hu'),
  (4262, 1, 'lv', 'hy'),
  (4263, 1, 'lv', 'id'),
  (4264, 1, 'lv', 'ig'),
  (4265, 1, 'lv', 'is'),
  (4266, 1, 'lv', 'it'),
  (4267, 1, 'lv', 'iw'),
  (4268, 1, 'lv', 'ja'),
  (4269, 1, 'lv', 'jw'),
  (4270, 1, 'lv', 'ka'),
  (4271, 1, 'lv', 'kk'),
  (4272, 1, 'lv', 'km'),
  (4273, 1, 'lv', 'kn'),
  (4274, 1, 'lv', 'ko'),
  (4275, 1, 'lv', 'la'),
  (4276, 1, 'lv', 'lo'),
  (4277, 1, 'lv', 'lt'),
  (4278, 1, 'lv', 'mg'),
  (4279, 1, 'lv', 'mi'),
  (4280, 1, 'lv', 'mk'),
  (4281, 1, 'lv', 'ml'),
  (4282, 1, 'lv', 'mn'),
  (4283, 1, 'lv', 'mr'),
  (4284, 1, 'lv', 'ms'),
  (4285, 1, 'lv', 'mt'),
  (4286, 1, 'lv', 'my'),
  (4287, 1, 'lv', 'ne'),
  (4288, 1, 'lv', 'nl'),
  (4289, 1, 'lv', 'no'),
  (4290, 1, 'lv', 'ny'),
  (4291, 1, 'lv', 'pa'),
  (4292, 1, 'lv', 'pl'),
  (4293, 1, 'lv', 'pt'),
  (4294, 1, 'lv', 'ro'),
  (4295, 1, 'lv', 'ru'),
  (4296, 1, 'lv', 'si'),
  (4297, 1, 'lv', 'sk'),
  (4298, 1, 'lv', 'sl'),
  (4299, 1, 'lv', 'so'),
  (4300, 1, 'lv', 'sq'),
  (4301, 1, 'lv', 'sr'),
  (4302, 1, 'lv', 'st'),
  (4303, 1, 'lv', 'su'),
  (4304, 1, 'lv', 'sv'),
  (4305, 1, 'lv', 'sw'),
  (4306, 1, 'lv', 'ta'),
  (4307, 1, 'lv', 'te'),
  (4308, 1, 'lv', 'tg'),
  (4309, 1, 'lv', 'th'),
  (4310, 1, 'lv', 'tl'),
  (4311, 1, 'lv', 'tr'),
  (4312, 1, 'lv', 'uk'),
  (4313, 1, 'lv', 'ur'),
  (4314, 1, 'lv', 'uz'),
  (4315, 1, 'lv', 'vi'),
  (4316, 1, 'lv', 'yi'),
  (4317, 1, 'lv', 'yo'),
  (4318, 1, 'lv', 'zh'),
  (4319, 1, 'lv', 'zh-TW'),
  (4320, 1, 'lv', 'zu'),
  (4321, 1, 'mg', 'af'),
  (4322, 1, 'mg', 'ar'),
  (4323, 1, 'mg', 'az'),
  (4324, 1, 'mg', 'be'),
  (4325, 1, 'mg', 'bg'),
  (4326, 1, 'mg', 'bn'),
  (4327, 1, 'mg', 'bs'),
  (4328, 1, 'mg', 'ca'),
  (4329, 1, 'mg', 'ceb'),
  (4330, 1, 'mg', 'cs'),
  (4331, 1, 'mg', 'cy'),
  (4332, 1, 'mg', 'da'),
  (4333, 1, 'mg', 'de'),
  (4334, 1, 'mg', 'el'),
  (4335, 1, 'mg', 'en'),
  (4336, 1, 'mg', 'eo'),
  (4337, 1, 'mg', 'es'),
  (4338, 1, 'mg', 'et'),
  (4339, 1, 'mg', 'eu'),
  (4340, 1, 'mg', 'fa'),
  (4341, 1, 'mg', 'fi'),
  (4342, 1, 'mg', 'fr'),
  (4343, 1, 'mg', 'ga'),
  (4344, 1, 'mg', 'gl'),
  (4345, 1, 'mg', 'gu'),
  (4346, 1, 'mg', 'ha'),
  (4347, 1, 'mg', 'hi'),
  (4348, 1, 'mg', 'hmn'),
  (4349, 1, 'mg', 'hr'),
  (4350, 1, 'mg', 'ht'),
  (4351, 1, 'mg', 'hu'),
  (4352, 1, 'mg', 'hy'),
  (4353, 1, 'mg', 'id'),
  (4354, 1, 'mg', 'ig'),
  (4355, 1, 'mg', 'is'),
  (4356, 1, 'mg', 'it'),
  (4357, 1, 'mg', 'iw'),
  (4358, 1, 'mg', 'ja'),
  (4359, 1, 'mg', 'jw'),
  (4360, 1, 'mg', 'ka'),
  (4361, 1, 'mg', 'kk'),
  (4362, 1, 'mg', 'km'),
  (4363, 1, 'mg', 'kn'),
  (4364, 1, 'mg', 'ko'),
  (4365, 1, 'mg', 'la'),
  (4366, 1, 'mg', 'lo'),
  (4367, 1, 'mg', 'lt'),
  (4368, 1, 'mg', 'lv'),
  (4369, 1, 'mg', 'mi'),
  (4370, 1, 'mg', 'mk'),
  (4371, 1, 'mg', 'ml'),
  (4372, 1, 'mg', 'mn'),
  (4373, 1, 'mg', 'mr'),
  (4374, 1, 'mg', 'ms'),
  (4375, 1, 'mg', 'mt'),
  (4376, 1, 'mg', 'my'),
  (4377, 1, 'mg', 'ne'),
  (4378, 1, 'mg', 'nl'),
  (4379, 1, 'mg', 'no'),
  (4380, 1, 'mg', 'ny'),
  (4381, 1, 'mg', 'pa'),
  (4382, 1, 'mg', 'pl'),
  (4383, 1, 'mg', 'pt'),
  (4384, 1, 'mg', 'ro'),
  (4385, 1, 'mg', 'ru'),
  (4386, 1, 'mg', 'si'),
  (4387, 1, 'mg', 'sk'),
  (4388, 1, 'mg', 'sl'),
  (4389, 1, 'mg', 'so'),
  (4390, 1, 'mg', 'sq'),
  (4391, 1, 'mg', 'sr'),
  (4392, 1, 'mg', 'st'),
  (4393, 1, 'mg', 'su'),
  (4394, 1, 'mg', 'sv'),
  (4395, 1, 'mg', 'sw'),
  (4396, 1, 'mg', 'ta'),
  (4397, 1, 'mg', 'te'),
  (4398, 1, 'mg', 'tg'),
  (4399, 1, 'mg', 'th'),
  (4400, 1, 'mg', 'tl'),
  (4401, 1, 'mg', 'tr'),
  (4402, 1, 'mg', 'uk'),
  (4403, 1, 'mg', 'ur'),
  (4404, 1, 'mg', 'uz'),
  (4405, 1, 'mg', 'vi'),
  (4406, 1, 'mg', 'yi'),
  (4407, 1, 'mg', 'yo'),
  (4408, 1, 'mg', 'zh'),
  (4409, 1, 'mg', 'zh-TW'),
  (4410, 1, 'mg', 'zu'),
  (4411, 1, 'mi', 'af'),
  (4412, 1, 'mi', 'ar'),
  (4413, 1, 'mi', 'az'),
  (4414, 1, 'mi', 'be'),
  (4415, 1, 'mi', 'bg'),
  (4416, 1, 'mi', 'bn'),
  (4417, 1, 'mi', 'bs'),
  (4418, 1, 'mi', 'ca'),
  (4419, 1, 'mi', 'ceb'),
  (4420, 1, 'mi', 'cs'),
  (4421, 1, 'mi', 'cy'),
  (4422, 1, 'mi', 'da'),
  (4423, 1, 'mi', 'de'),
  (4424, 1, 'mi', 'el'),
  (4425, 1, 'mi', 'en'),
  (4426, 1, 'mi', 'eo'),
  (4427, 1, 'mi', 'es'),
  (4428, 1, 'mi', 'et'),
  (4429, 1, 'mi', 'eu'),
  (4430, 1, 'mi', 'fa'),
  (4431, 1, 'mi', 'fi'),
  (4432, 1, 'mi', 'fr'),
  (4433, 1, 'mi', 'ga'),
  (4434, 1, 'mi', 'gl'),
  (4435, 1, 'mi', 'gu'),
  (4436, 1, 'mi', 'ha'),
  (4437, 1, 'mi', 'hi'),
  (4438, 1, 'mi', 'hmn'),
  (4439, 1, 'mi', 'hr'),
  (4440, 1, 'mi', 'ht'),
  (4441, 1, 'mi', 'hu'),
  (4442, 1, 'mi', 'hy'),
  (4443, 1, 'mi', 'id'),
  (4444, 1, 'mi', 'ig'),
  (4445, 1, 'mi', 'is'),
  (4446, 1, 'mi', 'it'),
  (4447, 1, 'mi', 'iw'),
  (4448, 1, 'mi', 'ja'),
  (4449, 1, 'mi', 'jw'),
  (4450, 1, 'mi', 'ka'),
  (4451, 1, 'mi', 'kk'),
  (4452, 1, 'mi', 'km'),
  (4453, 1, 'mi', 'kn'),
  (4454, 1, 'mi', 'ko'),
  (4455, 1, 'mi', 'la'),
  (4456, 1, 'mi', 'lo'),
  (4457, 1, 'mi', 'lt'),
  (4458, 1, 'mi', 'lv'),
  (4459, 1, 'mi', 'mg'),
  (4460, 1, 'mi', 'mk'),
  (4461, 1, 'mi', 'ml'),
  (4462, 1, 'mi', 'mn'),
  (4463, 1, 'mi', 'mr'),
  (4464, 1, 'mi', 'ms'),
  (4465, 1, 'mi', 'mt'),
  (4466, 1, 'mi', 'my'),
  (4467, 1, 'mi', 'ne'),
  (4468, 1, 'mi', 'nl'),
  (4469, 1, 'mi', 'no'),
  (4470, 1, 'mi', 'ny'),
  (4471, 1, 'mi', 'pa'),
  (4472, 1, 'mi', 'pl'),
  (4473, 1, 'mi', 'pt'),
  (4474, 1, 'mi', 'ro'),
  (4475, 1, 'mi', 'ru'),
  (4476, 1, 'mi', 'si'),
  (4477, 1, 'mi', 'sk'),
  (4478, 1, 'mi', 'sl'),
  (4479, 1, 'mi', 'so'),
  (4480, 1, 'mi', 'sq'),
  (4481, 1, 'mi', 'sr'),
  (4482, 1, 'mi', 'st'),
  (4483, 1, 'mi', 'su'),
  (4484, 1, 'mi', 'sv'),
  (4485, 1, 'mi', 'sw'),
  (4486, 1, 'mi', 'ta'),
  (4487, 1, 'mi', 'te'),
  (4488, 1, 'mi', 'tg'),
  (4489, 1, 'mi', 'th'),
  (4490, 1, 'mi', 'tl'),
  (4491, 1, 'mi', 'tr'),
  (4492, 1, 'mi', 'uk'),
  (4493, 1, 'mi', 'ur'),
  (4494, 1, 'mi', 'uz'),
  (4495, 1, 'mi', 'vi'),
  (4496, 1, 'mi', 'yi'),
  (4497, 1, 'mi', 'yo'),
  (4498, 1, 'mi', 'zh'),
  (4499, 1, 'mi', 'zh-TW'),
  (4500, 1, 'mi', 'zu'),
  (4501, 1, 'mk', 'af'),
  (4502, 1, 'mk', 'ar'),
  (4503, 1, 'mk', 'az'),
  (4504, 1, 'mk', 'be'),
  (4505, 1, 'mk', 'bg'),
  (4506, 1, 'mk', 'bn'),
  (4507, 1, 'mk', 'bs'),
  (4508, 1, 'mk', 'ca'),
  (4509, 1, 'mk', 'ceb'),
  (4510, 1, 'mk', 'cs'),
  (4511, 1, 'mk', 'cy'),
  (4512, 1, 'mk', 'da'),
  (4513, 1, 'mk', 'de'),
  (4514, 1, 'mk', 'el'),
  (4515, 1, 'mk', 'en'),
  (4516, 1, 'mk', 'eo'),
  (4517, 1, 'mk', 'es'),
  (4518, 1, 'mk', 'et'),
  (4519, 1, 'mk', 'eu'),
  (4520, 1, 'mk', 'fa'),
  (4521, 1, 'mk', 'fi'),
  (4522, 1, 'mk', 'fr'),
  (4523, 1, 'mk', 'ga'),
  (4524, 1, 'mk', 'gl'),
  (4525, 1, 'mk', 'gu'),
  (4526, 1, 'mk', 'ha'),
  (4527, 1, 'mk', 'hi'),
  (4528, 1, 'mk', 'hmn'),
  (4529, 1, 'mk', 'hr'),
  (4530, 1, 'mk', 'ht'),
  (4531, 1, 'mk', 'hu'),
  (4532, 1, 'mk', 'hy'),
  (4533, 1, 'mk', 'id'),
  (4534, 1, 'mk', 'ig'),
  (4535, 1, 'mk', 'is'),
  (4536, 1, 'mk', 'it'),
  (4537, 1, 'mk', 'iw'),
  (4538, 1, 'mk', 'ja'),
  (4539, 1, 'mk', 'jw'),
  (4540, 1, 'mk', 'ka'),
  (4541, 1, 'mk', 'kk'),
  (4542, 1, 'mk', 'km'),
  (4543, 1, 'mk', 'kn'),
  (4544, 1, 'mk', 'ko'),
  (4545, 1, 'mk', 'la'),
  (4546, 1, 'mk', 'lo'),
  (4547, 1, 'mk', 'lt'),
  (4548, 1, 'mk', 'lv'),
  (4549, 1, 'mk', 'mg'),
  (4550, 1, 'mk', 'mi'),
  (4551, 1, 'mk', 'ml'),
  (4552, 1, 'mk', 'mn'),
  (4553, 1, 'mk', 'mr'),
  (4554, 1, 'mk', 'ms'),
  (4555, 1, 'mk', 'mt'),
  (4556, 1, 'mk', 'my'),
  (4557, 1, 'mk', 'ne'),
  (4558, 1, 'mk', 'nl'),
  (4559, 1, 'mk', 'no'),
  (4560, 1, 'mk', 'ny'),
  (4561, 1, 'mk', 'pa'),
  (4562, 1, 'mk', 'pl'),
  (4563, 1, 'mk', 'pt'),
  (4564, 1, 'mk', 'ro'),
  (4565, 1, 'mk', 'ru'),
  (4566, 1, 'mk', 'si'),
  (4567, 1, 'mk', 'sk'),
  (4568, 1, 'mk', 'sl'),
  (4569, 1, 'mk', 'so'),
  (4570, 1, 'mk', 'sq'),
  (4571, 1, 'mk', 'sr'),
  (4572, 1, 'mk', 'st'),
  (4573, 1, 'mk', 'su'),
  (4574, 1, 'mk', 'sv'),
  (4575, 1, 'mk', 'sw'),
  (4576, 1, 'mk', 'ta'),
  (4577, 1, 'mk', 'te'),
  (4578, 1, 'mk', 'tg'),
  (4579, 1, 'mk', 'th'),
  (4580, 1, 'mk', 'tl'),
  (4581, 1, 'mk', 'tr'),
  (4582, 1, 'mk', 'uk'),
  (4583, 1, 'mk', 'ur'),
  (4584, 1, 'mk', 'uz'),
  (4585, 1, 'mk', 'vi'),
  (4586, 1, 'mk', 'yi'),
  (4587, 1, 'mk', 'yo'),
  (4588, 1, 'mk', 'zh'),
  (4589, 1, 'mk', 'zh-TW'),
  (4590, 1, 'mk', 'zu'),
  (4591, 1, 'ml', 'af'),
  (4592, 1, 'ml', 'ar'),
  (4593, 1, 'ml', 'az'),
  (4594, 1, 'ml', 'be'),
  (4595, 1, 'ml', 'bg'),
  (4596, 1, 'ml', 'bn'),
  (4597, 1, 'ml', 'bs'),
  (4598, 1, 'ml', 'ca'),
  (4599, 1, 'ml', 'ceb'),
  (4600, 1, 'ml', 'cs'),
  (4601, 1, 'ml', 'cy'),
  (4602, 1, 'ml', 'da'),
  (4603, 1, 'ml', 'de'),
  (4604, 1, 'ml', 'el'),
  (4605, 1, 'ml', 'en'),
  (4606, 1, 'ml', 'eo'),
  (4607, 1, 'ml', 'es'),
  (4608, 1, 'ml', 'et'),
  (4609, 1, 'ml', 'eu'),
  (4610, 1, 'ml', 'fa'),
  (4611, 1, 'ml', 'fi'),
  (4612, 1, 'ml', 'fr'),
  (4613, 1, 'ml', 'ga'),
  (4614, 1, 'ml', 'gl'),
  (4615, 1, 'ml', 'gu'),
  (4616, 1, 'ml', 'ha'),
  (4617, 1, 'ml', 'hi'),
  (4618, 1, 'ml', 'hmn'),
  (4619, 1, 'ml', 'hr'),
  (4620, 1, 'ml', 'ht'),
  (4621, 1, 'ml', 'hu'),
  (4622, 1, 'ml', 'hy'),
  (4623, 1, 'ml', 'id'),
  (4624, 1, 'ml', 'ig'),
  (4625, 1, 'ml', 'is'),
  (4626, 1, 'ml', 'it'),
  (4627, 1, 'ml', 'iw'),
  (4628, 1, 'ml', 'ja'),
  (4629, 1, 'ml', 'jw'),
  (4630, 1, 'ml', 'ka'),
  (4631, 1, 'ml', 'kk'),
  (4632, 1, 'ml', 'km'),
  (4633, 1, 'ml', 'kn'),
  (4634, 1, 'ml', 'ko'),
  (4635, 1, 'ml', 'la'),
  (4636, 1, 'ml', 'lo'),
  (4637, 1, 'ml', 'lt'),
  (4638, 1, 'ml', 'lv'),
  (4639, 1, 'ml', 'mg'),
  (4640, 1, 'ml', 'mi'),
  (4641, 1, 'ml', 'mk'),
  (4642, 1, 'ml', 'mn'),
  (4643, 1, 'ml', 'mr'),
  (4644, 1, 'ml', 'ms'),
  (4645, 1, 'ml', 'mt'),
  (4646, 1, 'ml', 'my'),
  (4647, 1, 'ml', 'ne'),
  (4648, 1, 'ml', 'nl'),
  (4649, 1, 'ml', 'no'),
  (4650, 1, 'ml', 'ny'),
  (4651, 1, 'ml', 'pa'),
  (4652, 1, 'ml', 'pl'),
  (4653, 1, 'ml', 'pt'),
  (4654, 1, 'ml', 'ro'),
  (4655, 1, 'ml', 'ru'),
  (4656, 1, 'ml', 'si'),
  (4657, 1, 'ml', 'sk'),
  (4658, 1, 'ml', 'sl'),
  (4659, 1, 'ml', 'so'),
  (4660, 1, 'ml', 'sq'),
  (4661, 1, 'ml', 'sr'),
  (4662, 1, 'ml', 'st'),
  (4663, 1, 'ml', 'su'),
  (4664, 1, 'ml', 'sv'),
  (4665, 1, 'ml', 'sw'),
  (4666, 1, 'ml', 'ta'),
  (4667, 1, 'ml', 'te'),
  (4668, 1, 'ml', 'tg'),
  (4669, 1, 'ml', 'th'),
  (4670, 1, 'ml', 'tl'),
  (4671, 1, 'ml', 'tr'),
  (4672, 1, 'ml', 'uk'),
  (4673, 1, 'ml', 'ur'),
  (4674, 1, 'ml', 'uz'),
  (4675, 1, 'ml', 'vi'),
  (4676, 1, 'ml', 'yi'),
  (4677, 1, 'ml', 'yo'),
  (4678, 1, 'ml', 'zh'),
  (4679, 1, 'ml', 'zh-TW'),
  (4680, 1, 'ml', 'zu'),
  (4681, 1, 'mn', 'af'),
  (4682, 1, 'mn', 'ar'),
  (4683, 1, 'mn', 'az'),
  (4684, 1, 'mn', 'be'),
  (4685, 1, 'mn', 'bg'),
  (4686, 1, 'mn', 'bn'),
  (4687, 1, 'mn', 'bs'),
  (4688, 1, 'mn', 'ca'),
  (4689, 1, 'mn', 'ceb'),
  (4690, 1, 'mn', 'cs'),
  (4691, 1, 'mn', 'cy'),
  (4692, 1, 'mn', 'da'),
  (4693, 1, 'mn', 'de'),
  (4694, 1, 'mn', 'el'),
  (4695, 1, 'mn', 'en'),
  (4696, 1, 'mn', 'eo'),
  (4697, 1, 'mn', 'es'),
  (4698, 1, 'mn', 'et'),
  (4699, 1, 'mn', 'eu'),
  (4700, 1, 'mn', 'fa'),
  (4701, 1, 'mn', 'fi'),
  (4702, 1, 'mn', 'fr'),
  (4703, 1, 'mn', 'ga'),
  (4704, 1, 'mn', 'gl'),
  (4705, 1, 'mn', 'gu'),
  (4706, 1, 'mn', 'ha'),
  (4707, 1, 'mn', 'hi'),
  (4708, 1, 'mn', 'hmn'),
  (4709, 1, 'mn', 'hr'),
  (4710, 1, 'mn', 'ht'),
  (4711, 1, 'mn', 'hu'),
  (4712, 1, 'mn', 'hy'),
  (4713, 1, 'mn', 'id'),
  (4714, 1, 'mn', 'ig'),
  (4715, 1, 'mn', 'is'),
  (4716, 1, 'mn', 'it'),
  (4717, 1, 'mn', 'iw'),
  (4718, 1, 'mn', 'ja'),
  (4719, 1, 'mn', 'jw'),
  (4720, 1, 'mn', 'ka'),
  (4721, 1, 'mn', 'kk'),
  (4722, 1, 'mn', 'km'),
  (4723, 1, 'mn', 'kn'),
  (4724, 1, 'mn', 'ko'),
  (4725, 1, 'mn', 'la'),
  (4726, 1, 'mn', 'lo'),
  (4727, 1, 'mn', 'lt'),
  (4728, 1, 'mn', 'lv'),
  (4729, 1, 'mn', 'mg'),
  (4730, 1, 'mn', 'mi'),
  (4731, 1, 'mn', 'mk'),
  (4732, 1, 'mn', 'ml'),
  (4733, 1, 'mn', 'mr'),
  (4734, 1, 'mn', 'ms'),
  (4735, 1, 'mn', 'mt'),
  (4736, 1, 'mn', 'my'),
  (4737, 1, 'mn', 'ne'),
  (4738, 1, 'mn', 'nl'),
  (4739, 1, 'mn', 'no'),
  (4740, 1, 'mn', 'ny'),
  (4741, 1, 'mn', 'pa'),
  (4742, 1, 'mn', 'pl'),
  (4743, 1, 'mn', 'pt'),
  (4744, 1, 'mn', 'ro'),
  (4745, 1, 'mn', 'ru'),
  (4746, 1, 'mn', 'si'),
  (4747, 1, 'mn', 'sk'),
  (4748, 1, 'mn', 'sl'),
  (4749, 1, 'mn', 'so'),
  (4750, 1, 'mn', 'sq'),
  (4751, 1, 'mn', 'sr'),
  (4752, 1, 'mn', 'st'),
  (4753, 1, 'mn', 'su'),
  (4754, 1, 'mn', 'sv'),
  (4755, 1, 'mn', 'sw'),
  (4756, 1, 'mn', 'ta'),
  (4757, 1, 'mn', 'te'),
  (4758, 1, 'mn', 'tg'),
  (4759, 1, 'mn', 'th'),
  (4760, 1, 'mn', 'tl'),
  (4761, 1, 'mn', 'tr'),
  (4762, 1, 'mn', 'uk'),
  (4763, 1, 'mn', 'ur'),
  (4764, 1, 'mn', 'uz'),
  (4765, 1, 'mn', 'vi'),
  (4766, 1, 'mn', 'yi'),
  (4767, 1, 'mn', 'yo'),
  (4768, 1, 'mn', 'zh'),
  (4769, 1, 'mn', 'zh-TW'),
  (4770, 1, 'mn', 'zu'),
  (4771, 1, 'mr', 'af'),
  (4772, 1, 'mr', 'ar'),
  (4773, 1, 'mr', 'az'),
  (4774, 1, 'mr', 'be'),
  (4775, 1, 'mr', 'bg'),
  (4776, 1, 'mr', 'bn'),
  (4777, 1, 'mr', 'bs'),
  (4778, 1, 'mr', 'ca'),
  (4779, 1, 'mr', 'ceb');
INSERT INTO `#__neno_translation_methods_language_pairs` (`id`, `translation_method_id`, `source_language`, `destination_language`) VALUES
  (4780, 1, 'mr', 'cs'),
  (4781, 1, 'mr', 'cy'),
  (4782, 1, 'mr', 'da'),
  (4783, 1, 'mr', 'de'),
  (4784, 1, 'mr', 'el'),
  (4785, 1, 'mr', 'en'),
  (4786, 1, 'mr', 'eo'),
  (4787, 1, 'mr', 'es'),
  (4788, 1, 'mr', 'et'),
  (4789, 1, 'mr', 'eu'),
  (4790, 1, 'mr', 'fa'),
  (4791, 1, 'mr', 'fi'),
  (4792, 1, 'mr', 'fr'),
  (4793, 1, 'mr', 'ga'),
  (4794, 1, 'mr', 'gl'),
  (4795, 1, 'mr', 'gu'),
  (4796, 1, 'mr', 'ha'),
  (4797, 1, 'mr', 'hi'),
  (4798, 1, 'mr', 'hmn'),
  (4799, 1, 'mr', 'hr'),
  (4800, 1, 'mr', 'ht'),
  (4801, 1, 'mr', 'hu'),
  (4802, 1, 'mr', 'hy'),
  (4803, 1, 'mr', 'id'),
  (4804, 1, 'mr', 'ig'),
  (4805, 1, 'mr', 'is'),
  (4806, 1, 'mr', 'it'),
  (4807, 1, 'mr', 'iw'),
  (4808, 1, 'mr', 'ja'),
  (4809, 1, 'mr', 'jw'),
  (4810, 1, 'mr', 'ka'),
  (4811, 1, 'mr', 'kk'),
  (4812, 1, 'mr', 'km'),
  (4813, 1, 'mr', 'kn'),
  (4814, 1, 'mr', 'ko'),
  (4815, 1, 'mr', 'la'),
  (4816, 1, 'mr', 'lo'),
  (4817, 1, 'mr', 'lt'),
  (4818, 1, 'mr', 'lv'),
  (4819, 1, 'mr', 'mg'),
  (4820, 1, 'mr', 'mi'),
  (4821, 1, 'mr', 'mk'),
  (4822, 1, 'mr', 'ml'),
  (4823, 1, 'mr', 'mn'),
  (4824, 1, 'mr', 'ms'),
  (4825, 1, 'mr', 'mt'),
  (4826, 1, 'mr', 'my'),
  (4827, 1, 'mr', 'ne'),
  (4828, 1, 'mr', 'nl'),
  (4829, 1, 'mr', 'no'),
  (4830, 1, 'mr', 'ny'),
  (4831, 1, 'mr', 'pa'),
  (4832, 1, 'mr', 'pl'),
  (4833, 1, 'mr', 'pt'),
  (4834, 1, 'mr', 'ro'),
  (4835, 1, 'mr', 'ru'),
  (4836, 1, 'mr', 'si'),
  (4837, 1, 'mr', 'sk'),
  (4838, 1, 'mr', 'sl'),
  (4839, 1, 'mr', 'so'),
  (4840, 1, 'mr', 'sq'),
  (4841, 1, 'mr', 'sr'),
  (4842, 1, 'mr', 'st'),
  (4843, 1, 'mr', 'su'),
  (4844, 1, 'mr', 'sv'),
  (4845, 1, 'mr', 'sw'),
  (4846, 1, 'mr', 'ta'),
  (4847, 1, 'mr', 'te'),
  (4848, 1, 'mr', 'tg'),
  (4849, 1, 'mr', 'th'),
  (4850, 1, 'mr', 'tl'),
  (4851, 1, 'mr', 'tr'),
  (4852, 1, 'mr', 'uk'),
  (4853, 1, 'mr', 'ur'),
  (4854, 1, 'mr', 'uz'),
  (4855, 1, 'mr', 'vi'),
  (4856, 1, 'mr', 'yi'),
  (4857, 1, 'mr', 'yo'),
  (4858, 1, 'mr', 'zh'),
  (4859, 1, 'mr', 'zh-TW'),
  (4860, 1, 'mr', 'zu'),
  (4861, 1, 'ms', 'af'),
  (4862, 1, 'ms', 'ar'),
  (4863, 1, 'ms', 'az'),
  (4864, 1, 'ms', 'be'),
  (4865, 1, 'ms', 'bg'),
  (4866, 1, 'ms', 'bn'),
  (4867, 1, 'ms', 'bs'),
  (4868, 1, 'ms', 'ca'),
  (4869, 1, 'ms', 'ceb'),
  (4870, 1, 'ms', 'cs'),
  (4871, 1, 'ms', 'cy'),
  (4872, 1, 'ms', 'da'),
  (4873, 1, 'ms', 'de'),
  (4874, 1, 'ms', 'el'),
  (4875, 1, 'ms', 'en'),
  (4876, 1, 'ms', 'eo'),
  (4877, 1, 'ms', 'es'),
  (4878, 1, 'ms', 'et'),
  (4879, 1, 'ms', 'eu'),
  (4880, 1, 'ms', 'fa'),
  (4881, 1, 'ms', 'fi'),
  (4882, 1, 'ms', 'fr'),
  (4883, 1, 'ms', 'ga'),
  (4884, 1, 'ms', 'gl'),
  (4885, 1, 'ms', 'gu'),
  (4886, 1, 'ms', 'ha'),
  (4887, 1, 'ms', 'hi'),
  (4888, 1, 'ms', 'hmn'),
  (4889, 1, 'ms', 'hr'),
  (4890, 1, 'ms', 'ht'),
  (4891, 1, 'ms', 'hu'),
  (4892, 1, 'ms', 'hy'),
  (4893, 1, 'ms', 'id'),
  (4894, 1, 'ms', 'ig'),
  (4895, 1, 'ms', 'is'),
  (4896, 1, 'ms', 'it'),
  (4897, 1, 'ms', 'iw'),
  (4898, 1, 'ms', 'ja'),
  (4899, 1, 'ms', 'jw'),
  (4900, 1, 'ms', 'ka'),
  (4901, 1, 'ms', 'kk'),
  (4902, 1, 'ms', 'km'),
  (4903, 1, 'ms', 'kn'),
  (4904, 1, 'ms', 'ko'),
  (4905, 1, 'ms', 'la'),
  (4906, 1, 'ms', 'lo'),
  (4907, 1, 'ms', 'lt'),
  (4908, 1, 'ms', 'lv'),
  (4909, 1, 'ms', 'mg'),
  (4910, 1, 'ms', 'mi'),
  (4911, 1, 'ms', 'mk'),
  (4912, 1, 'ms', 'ml'),
  (4913, 1, 'ms', 'mn'),
  (4914, 1, 'ms', 'mr'),
  (4915, 1, 'ms', 'mt'),
  (4916, 1, 'ms', 'my'),
  (4917, 1, 'ms', 'ne'),
  (4918, 1, 'ms', 'nl'),
  (4919, 1, 'ms', 'no'),
  (4920, 1, 'ms', 'ny'),
  (4921, 1, 'ms', 'pa'),
  (4922, 1, 'ms', 'pl'),
  (4923, 1, 'ms', 'pt'),
  (4924, 1, 'ms', 'ro'),
  (4925, 1, 'ms', 'ru'),
  (4926, 1, 'ms', 'si'),
  (4927, 1, 'ms', 'sk'),
  (4928, 1, 'ms', 'sl'),
  (4929, 1, 'ms', 'so'),
  (4930, 1, 'ms', 'sq'),
  (4931, 1, 'ms', 'sr'),
  (4932, 1, 'ms', 'st'),
  (4933, 1, 'ms', 'su'),
  (4934, 1, 'ms', 'sv'),
  (4935, 1, 'ms', 'sw'),
  (4936, 1, 'ms', 'ta'),
  (4937, 1, 'ms', 'te'),
  (4938, 1, 'ms', 'tg'),
  (4939, 1, 'ms', 'th'),
  (4940, 1, 'ms', 'tl'),
  (4941, 1, 'ms', 'tr'),
  (4942, 1, 'ms', 'uk'),
  (4943, 1, 'ms', 'ur'),
  (4944, 1, 'ms', 'uz'),
  (4945, 1, 'ms', 'vi'),
  (4946, 1, 'ms', 'yi'),
  (4947, 1, 'ms', 'yo'),
  (4948, 1, 'ms', 'zh'),
  (4949, 1, 'ms', 'zh-TW'),
  (4950, 1, 'ms', 'zu'),
  (4951, 1, 'mt', 'af'),
  (4952, 1, 'mt', 'ar'),
  (4953, 1, 'mt', 'az'),
  (4954, 1, 'mt', 'be'),
  (4955, 1, 'mt', 'bg'),
  (4956, 1, 'mt', 'bn'),
  (4957, 1, 'mt', 'bs'),
  (4958, 1, 'mt', 'ca'),
  (4959, 1, 'mt', 'ceb'),
  (4960, 1, 'mt', 'cs'),
  (4961, 1, 'mt', 'cy'),
  (4962, 1, 'mt', 'da'),
  (4963, 1, 'mt', 'de'),
  (4964, 1, 'mt', 'el'),
  (4965, 1, 'mt', 'en'),
  (4966, 1, 'mt', 'eo'),
  (4967, 1, 'mt', 'es'),
  (4968, 1, 'mt', 'et'),
  (4969, 1, 'mt', 'eu'),
  (4970, 1, 'mt', 'fa'),
  (4971, 1, 'mt', 'fi'),
  (4972, 1, 'mt', 'fr'),
  (4973, 1, 'mt', 'ga'),
  (4974, 1, 'mt', 'gl'),
  (4975, 1, 'mt', 'gu'),
  (4976, 1, 'mt', 'ha'),
  (4977, 1, 'mt', 'hi'),
  (4978, 1, 'mt', 'hmn'),
  (4979, 1, 'mt', 'hr'),
  (4980, 1, 'mt', 'ht'),
  (4981, 1, 'mt', 'hu'),
  (4982, 1, 'mt', 'hy'),
  (4983, 1, 'mt', 'id'),
  (4984, 1, 'mt', 'ig'),
  (4985, 1, 'mt', 'is'),
  (4986, 1, 'mt', 'it'),
  (4987, 1, 'mt', 'iw'),
  (4988, 1, 'mt', 'ja'),
  (4989, 1, 'mt', 'jw'),
  (4990, 1, 'mt', 'ka'),
  (4991, 1, 'mt', 'kk'),
  (4992, 1, 'mt', 'km'),
  (4993, 1, 'mt', 'kn'),
  (4994, 1, 'mt', 'ko'),
  (4995, 1, 'mt', 'la'),
  (4996, 1, 'mt', 'lo'),
  (4997, 1, 'mt', 'lt'),
  (4998, 1, 'mt', 'lv'),
  (4999, 1, 'mt', 'mg'),
  (5000, 1, 'mt', 'mi'),
  (5001, 1, 'mt', 'mk'),
  (5002, 1, 'mt', 'ml'),
  (5003, 1, 'mt', 'mn'),
  (5004, 1, 'mt', 'mr'),
  (5005, 1, 'mt', 'ms'),
  (5006, 1, 'mt', 'my'),
  (5007, 1, 'mt', 'ne'),
  (5008, 1, 'mt', 'nl'),
  (5009, 1, 'mt', 'no'),
  (5010, 1, 'mt', 'ny'),
  (5011, 1, 'mt', 'pa'),
  (5012, 1, 'mt', 'pl'),
  (5013, 1, 'mt', 'pt'),
  (5014, 1, 'mt', 'ro'),
  (5015, 1, 'mt', 'ru'),
  (5016, 1, 'mt', 'si'),
  (5017, 1, 'mt', 'sk'),
  (5018, 1, 'mt', 'sl'),
  (5019, 1, 'mt', 'so'),
  (5020, 1, 'mt', 'sq'),
  (5021, 1, 'mt', 'sr'),
  (5022, 1, 'mt', 'st'),
  (5023, 1, 'mt', 'su'),
  (5024, 1, 'mt', 'sv'),
  (5025, 1, 'mt', 'sw'),
  (5026, 1, 'mt', 'ta'),
  (5027, 1, 'mt', 'te'),
  (5028, 1, 'mt', 'tg'),
  (5029, 1, 'mt', 'th'),
  (5030, 1, 'mt', 'tl'),
  (5031, 1, 'mt', 'tr'),
  (5032, 1, 'mt', 'uk'),
  (5033, 1, 'mt', 'ur'),
  (5034, 1, 'mt', 'uz'),
  (5035, 1, 'mt', 'vi'),
  (5036, 1, 'mt', 'yi'),
  (5037, 1, 'mt', 'yo'),
  (5038, 1, 'mt', 'zh'),
  (5039, 1, 'mt', 'zh-TW'),
  (5040, 1, 'mt', 'zu'),
  (5041, 1, 'my', 'af'),
  (5042, 1, 'my', 'ar'),
  (5043, 1, 'my', 'az'),
  (5044, 1, 'my', 'be'),
  (5045, 1, 'my', 'bg'),
  (5046, 1, 'my', 'bn'),
  (5047, 1, 'my', 'bs'),
  (5048, 1, 'my', 'ca'),
  (5049, 1, 'my', 'ceb'),
  (5050, 1, 'my', 'cs'),
  (5051, 1, 'my', 'cy'),
  (5052, 1, 'my', 'da'),
  (5053, 1, 'my', 'de'),
  (5054, 1, 'my', 'el'),
  (5055, 1, 'my', 'en'),
  (5056, 1, 'my', 'eo'),
  (5057, 1, 'my', 'es'),
  (5058, 1, 'my', 'et'),
  (5059, 1, 'my', 'eu'),
  (5060, 1, 'my', 'fa'),
  (5061, 1, 'my', 'fi'),
  (5062, 1, 'my', 'fr'),
  (5063, 1, 'my', 'ga'),
  (5064, 1, 'my', 'gl'),
  (5065, 1, 'my', 'gu'),
  (5066, 1, 'my', 'ha'),
  (5067, 1, 'my', 'hi'),
  (5068, 1, 'my', 'hmn'),
  (5069, 1, 'my', 'hr'),
  (5070, 1, 'my', 'ht'),
  (5071, 1, 'my', 'hu'),
  (5072, 1, 'my', 'hy'),
  (5073, 1, 'my', 'id'),
  (5074, 1, 'my', 'ig'),
  (5075, 1, 'my', 'is'),
  (5076, 1, 'my', 'it'),
  (5077, 1, 'my', 'iw'),
  (5078, 1, 'my', 'ja'),
  (5079, 1, 'my', 'jw'),
  (5080, 1, 'my', 'ka'),
  (5081, 1, 'my', 'kk'),
  (5082, 1, 'my', 'km'),
  (5083, 1, 'my', 'kn'),
  (5084, 1, 'my', 'ko'),
  (5085, 1, 'my', 'la'),
  (5086, 1, 'my', 'lo'),
  (5087, 1, 'my', 'lt'),
  (5088, 1, 'my', 'lv'),
  (5089, 1, 'my', 'mg'),
  (5090, 1, 'my', 'mi'),
  (5091, 1, 'my', 'mk'),
  (5092, 1, 'my', 'ml'),
  (5093, 1, 'my', 'mn'),
  (5094, 1, 'my', 'mr'),
  (5095, 1, 'my', 'ms'),
  (5096, 1, 'my', 'mt'),
  (5097, 1, 'my', 'ne'),
  (5098, 1, 'my', 'nl'),
  (5099, 1, 'my', 'no'),
  (5100, 1, 'my', 'ny'),
  (5101, 1, 'my', 'pa'),
  (5102, 1, 'my', 'pl'),
  (5103, 1, 'my', 'pt'),
  (5104, 1, 'my', 'ro'),
  (5105, 1, 'my', 'ru'),
  (5106, 1, 'my', 'si'),
  (5107, 1, 'my', 'sk'),
  (5108, 1, 'my', 'sl'),
  (5109, 1, 'my', 'so'),
  (5110, 1, 'my', 'sq'),
  (5111, 1, 'my', 'sr'),
  (5112, 1, 'my', 'st'),
  (5113, 1, 'my', 'su'),
  (5114, 1, 'my', 'sv'),
  (5115, 1, 'my', 'sw'),
  (5116, 1, 'my', 'ta'),
  (5117, 1, 'my', 'te'),
  (5118, 1, 'my', 'tg'),
  (5119, 1, 'my', 'th'),
  (5120, 1, 'my', 'tl'),
  (5121, 1, 'my', 'tr'),
  (5122, 1, 'my', 'uk'),
  (5123, 1, 'my', 'ur'),
  (5124, 1, 'my', 'uz'),
  (5125, 1, 'my', 'vi'),
  (5126, 1, 'my', 'yi'),
  (5127, 1, 'my', 'yo'),
  (5128, 1, 'my', 'zh'),
  (5129, 1, 'my', 'zh-TW'),
  (5130, 1, 'my', 'zu'),
  (5131, 1, 'ne', 'af'),
  (5132, 1, 'ne', 'ar'),
  (5133, 1, 'ne', 'az'),
  (5134, 1, 'ne', 'be'),
  (5135, 1, 'ne', 'bg'),
  (5136, 1, 'ne', 'bn'),
  (5137, 1, 'ne', 'bs'),
  (5138, 1, 'ne', 'ca'),
  (5139, 1, 'ne', 'ceb'),
  (5140, 1, 'ne', 'cs'),
  (5141, 1, 'ne', 'cy'),
  (5142, 1, 'ne', 'da'),
  (5143, 1, 'ne', 'de'),
  (5144, 1, 'ne', 'el'),
  (5145, 1, 'ne', 'en'),
  (5146, 1, 'ne', 'eo'),
  (5147, 1, 'ne', 'es'),
  (5148, 1, 'ne', 'et'),
  (5149, 1, 'ne', 'eu'),
  (5150, 1, 'ne', 'fa'),
  (5151, 1, 'ne', 'fi'),
  (5152, 1, 'ne', 'fr'),
  (5153, 1, 'ne', 'ga'),
  (5154, 1, 'ne', 'gl'),
  (5155, 1, 'ne', 'gu'),
  (5156, 1, 'ne', 'ha'),
  (5157, 1, 'ne', 'hi'),
  (5158, 1, 'ne', 'hmn'),
  (5159, 1, 'ne', 'hr'),
  (5160, 1, 'ne', 'ht'),
  (5161, 1, 'ne', 'hu'),
  (5162, 1, 'ne', 'hy'),
  (5163, 1, 'ne', 'id'),
  (5164, 1, 'ne', 'ig'),
  (5165, 1, 'ne', 'is'),
  (5166, 1, 'ne', 'it'),
  (5167, 1, 'ne', 'iw'),
  (5168, 1, 'ne', 'ja'),
  (5169, 1, 'ne', 'jw'),
  (5170, 1, 'ne', 'ka'),
  (5171, 1, 'ne', 'kk'),
  (5172, 1, 'ne', 'km'),
  (5173, 1, 'ne', 'kn'),
  (5174, 1, 'ne', 'ko'),
  (5175, 1, 'ne', 'la'),
  (5176, 1, 'ne', 'lo'),
  (5177, 1, 'ne', 'lt'),
  (5178, 1, 'ne', 'lv'),
  (5179, 1, 'ne', 'mg'),
  (5180, 1, 'ne', 'mi'),
  (5181, 1, 'ne', 'mk'),
  (5182, 1, 'ne', 'ml'),
  (5183, 1, 'ne', 'mn'),
  (5184, 1, 'ne', 'mr'),
  (5185, 1, 'ne', 'ms'),
  (5186, 1, 'ne', 'mt'),
  (5187, 1, 'ne', 'my'),
  (5188, 1, 'ne', 'nl'),
  (5189, 1, 'ne', 'no'),
  (5190, 1, 'ne', 'ny'),
  (5191, 1, 'ne', 'pa'),
  (5192, 1, 'ne', 'pl'),
  (5193, 1, 'ne', 'pt'),
  (5194, 1, 'ne', 'ro'),
  (5195, 1, 'ne', 'ru'),
  (5196, 1, 'ne', 'si'),
  (5197, 1, 'ne', 'sk'),
  (5198, 1, 'ne', 'sl'),
  (5199, 1, 'ne', 'so'),
  (5200, 1, 'ne', 'sq'),
  (5201, 1, 'ne', 'sr'),
  (5202, 1, 'ne', 'st'),
  (5203, 1, 'ne', 'su'),
  (5204, 1, 'ne', 'sv'),
  (5205, 1, 'ne', 'sw'),
  (5206, 1, 'ne', 'ta'),
  (5207, 1, 'ne', 'te'),
  (5208, 1, 'ne', 'tg'),
  (5209, 1, 'ne', 'th'),
  (5210, 1, 'ne', 'tl'),
  (5211, 1, 'ne', 'tr'),
  (5212, 1, 'ne', 'uk'),
  (5213, 1, 'ne', 'ur'),
  (5214, 1, 'ne', 'uz'),
  (5215, 1, 'ne', 'vi'),
  (5216, 1, 'ne', 'yi'),
  (5217, 1, 'ne', 'yo'),
  (5218, 1, 'ne', 'zh'),
  (5219, 1, 'ne', 'zh-TW'),
  (5220, 1, 'ne', 'zu'),
  (5221, 1, 'nl', 'af'),
  (5222, 1, 'nl', 'ar'),
  (5223, 1, 'nl', 'az'),
  (5224, 1, 'nl', 'be'),
  (5225, 1, 'nl', 'bg'),
  (5226, 1, 'nl', 'bn'),
  (5227, 1, 'nl', 'bs'),
  (5228, 1, 'nl', 'ca'),
  (5229, 1, 'nl', 'ceb'),
  (5230, 1, 'nl', 'cs'),
  (5231, 1, 'nl', 'cy'),
  (5232, 1, 'nl', 'da'),
  (5233, 1, 'nl', 'de'),
  (5234, 1, 'nl', 'el'),
  (5235, 1, 'nl', 'en'),
  (5236, 1, 'nl', 'eo'),
  (5237, 1, 'nl', 'es'),
  (5238, 1, 'nl', 'et'),
  (5239, 1, 'nl', 'eu'),
  (5240, 1, 'nl', 'fa'),
  (5241, 1, 'nl', 'fi'),
  (5242, 1, 'nl', 'fr'),
  (5243, 1, 'nl', 'ga'),
  (5244, 1, 'nl', 'gl'),
  (5245, 1, 'nl', 'gu'),
  (5246, 1, 'nl', 'ha'),
  (5247, 1, 'nl', 'hi'),
  (5248, 1, 'nl', 'hmn'),
  (5249, 1, 'nl', 'hr'),
  (5250, 1, 'nl', 'ht'),
  (5251, 1, 'nl', 'hu'),
  (5252, 1, 'nl', 'hy'),
  (5253, 1, 'nl', 'id'),
  (5254, 1, 'nl', 'ig'),
  (5255, 1, 'nl', 'is'),
  (5256, 1, 'nl', 'it'),
  (5257, 1, 'nl', 'iw'),
  (5258, 1, 'nl', 'ja'),
  (5259, 1, 'nl', 'jw'),
  (5260, 1, 'nl', 'ka'),
  (5261, 1, 'nl', 'kk'),
  (5262, 1, 'nl', 'km'),
  (5263, 1, 'nl', 'kn'),
  (5264, 1, 'nl', 'ko'),
  (5265, 1, 'nl', 'la'),
  (5266, 1, 'nl', 'lo'),
  (5267, 1, 'nl', 'lt'),
  (5268, 1, 'nl', 'lv'),
  (5269, 1, 'nl', 'mg'),
  (5270, 1, 'nl', 'mi'),
  (5271, 1, 'nl', 'mk'),
  (5272, 1, 'nl', 'ml'),
  (5273, 1, 'nl', 'mn'),
  (5274, 1, 'nl', 'mr'),
  (5275, 1, 'nl', 'ms'),
  (5276, 1, 'nl', 'mt'),
  (5277, 1, 'nl', 'my'),
  (5278, 1, 'nl', 'ne'),
  (5279, 1, 'nl', 'no'),
  (5280, 1, 'nl', 'ny'),
  (5281, 1, 'nl', 'pa'),
  (5282, 1, 'nl', 'pl'),
  (5283, 1, 'nl', 'pt'),
  (5284, 1, 'nl', 'ro'),
  (5285, 1, 'nl', 'ru'),
  (5286, 1, 'nl', 'si'),
  (5287, 1, 'nl', 'sk'),
  (5288, 1, 'nl', 'sl'),
  (5289, 1, 'nl', 'so'),
  (5290, 1, 'nl', 'sq'),
  (5291, 1, 'nl', 'sr'),
  (5292, 1, 'nl', 'st'),
  (5293, 1, 'nl', 'su'),
  (5294, 1, 'nl', 'sv'),
  (5295, 1, 'nl', 'sw'),
  (5296, 1, 'nl', 'ta'),
  (5297, 1, 'nl', 'te'),
  (5298, 1, 'nl', 'tg'),
  (5299, 1, 'nl', 'th'),
  (5300, 1, 'nl', 'tl'),
  (5301, 1, 'nl', 'tr'),
  (5302, 1, 'nl', 'uk'),
  (5303, 1, 'nl', 'ur'),
  (5304, 1, 'nl', 'uz'),
  (5305, 1, 'nl', 'vi'),
  (5306, 1, 'nl', 'yi'),
  (5307, 1, 'nl', 'yo'),
  (5308, 1, 'nl', 'zh'),
  (5309, 1, 'nl', 'zh-TW'),
  (5310, 1, 'nl', 'zu'),
  (5311, 1, 'no', 'af'),
  (5312, 1, 'no', 'ar'),
  (5313, 1, 'no', 'az'),
  (5314, 1, 'no', 'be'),
  (5315, 1, 'no', 'bg'),
  (5316, 1, 'no', 'bn'),
  (5317, 1, 'no', 'bs'),
  (5318, 1, 'no', 'ca'),
  (5319, 1, 'no', 'ceb'),
  (5320, 1, 'no', 'cs'),
  (5321, 1, 'no', 'cy'),
  (5322, 1, 'no', 'da'),
  (5323, 1, 'no', 'de'),
  (5324, 1, 'no', 'el'),
  (5325, 1, 'no', 'en'),
  (5326, 1, 'no', 'eo'),
  (5327, 1, 'no', 'es'),
  (5328, 1, 'no', 'et'),
  (5329, 1, 'no', 'eu'),
  (5330, 1, 'no', 'fa'),
  (5331, 1, 'no', 'fi'),
  (5332, 1, 'no', 'fr'),
  (5333, 1, 'no', 'ga'),
  (5334, 1, 'no', 'gl'),
  (5335, 1, 'no', 'gu'),
  (5336, 1, 'no', 'ha'),
  (5337, 1, 'no', 'hi'),
  (5338, 1, 'no', 'hmn'),
  (5339, 1, 'no', 'hr'),
  (5340, 1, 'no', 'ht'),
  (5341, 1, 'no', 'hu'),
  (5342, 1, 'no', 'hy'),
  (5343, 1, 'no', 'id'),
  (5344, 1, 'no', 'ig'),
  (5345, 1, 'no', 'is'),
  (5346, 1, 'no', 'it'),
  (5347, 1, 'no', 'iw'),
  (5348, 1, 'no', 'ja'),
  (5349, 1, 'no', 'jw'),
  (5350, 1, 'no', 'ka'),
  (5351, 1, 'no', 'kk'),
  (5352, 1, 'no', 'km'),
  (5353, 1, 'no', 'kn'),
  (5354, 1, 'no', 'ko'),
  (5355, 1, 'no', 'la'),
  (5356, 1, 'no', 'lo'),
  (5357, 1, 'no', 'lt'),
  (5358, 1, 'no', 'lv'),
  (5359, 1, 'no', 'mg'),
  (5360, 1, 'no', 'mi'),
  (5361, 1, 'no', 'mk'),
  (5362, 1, 'no', 'ml'),
  (5363, 1, 'no', 'mn'),
  (5364, 1, 'no', 'mr'),
  (5365, 1, 'no', 'ms'),
  (5366, 1, 'no', 'mt'),
  (5367, 1, 'no', 'my'),
  (5368, 1, 'no', 'ne'),
  (5369, 1, 'no', 'nl'),
  (5370, 1, 'no', 'ny'),
  (5371, 1, 'no', 'pa'),
  (5372, 1, 'no', 'pl'),
  (5373, 1, 'no', 'pt'),
  (5374, 1, 'no', 'ro'),
  (5375, 1, 'no', 'ru'),
  (5376, 1, 'no', 'si'),
  (5377, 1, 'no', 'sk'),
  (5378, 1, 'no', 'sl'),
  (5379, 1, 'no', 'so'),
  (5380, 1, 'no', 'sq'),
  (5381, 1, 'no', 'sr'),
  (5382, 1, 'no', 'st'),
  (5383, 1, 'no', 'su'),
  (5384, 1, 'no', 'sv'),
  (5385, 1, 'no', 'sw'),
  (5386, 1, 'no', 'ta'),
  (5387, 1, 'no', 'te'),
  (5388, 1, 'no', 'tg'),
  (5389, 1, 'no', 'th'),
  (5390, 1, 'no', 'tl'),
  (5391, 1, 'no', 'tr'),
  (5392, 1, 'no', 'uk'),
  (5393, 1, 'no', 'ur'),
  (5394, 1, 'no', 'uz'),
  (5395, 1, 'no', 'vi'),
  (5396, 1, 'no', 'yi'),
  (5397, 1, 'no', 'yo'),
  (5398, 1, 'no', 'zh'),
  (5399, 1, 'no', 'zh-TW'),
  (5400, 1, 'no', 'zu'),
  (5401, 1, 'ny', 'af'),
  (5402, 1, 'ny', 'ar'),
  (5403, 1, 'ny', 'az'),
  (5404, 1, 'ny', 'be'),
  (5405, 1, 'ny', 'bg'),
  (5406, 1, 'ny', 'bn'),
  (5407, 1, 'ny', 'bs'),
  (5408, 1, 'ny', 'ca'),
  (5409, 1, 'ny', 'ceb'),
  (5410, 1, 'ny', 'cs'),
  (5411, 1, 'ny', 'cy'),
  (5412, 1, 'ny', 'da'),
  (5413, 1, 'ny', 'de'),
  (5414, 1, 'ny', 'el'),
  (5415, 1, 'ny', 'en'),
  (5416, 1, 'ny', 'eo'),
  (5417, 1, 'ny', 'es'),
  (5418, 1, 'ny', 'et'),
  (5419, 1, 'ny', 'eu'),
  (5420, 1, 'ny', 'fa'),
  (5421, 1, 'ny', 'fi'),
  (5422, 1, 'ny', 'fr'),
  (5423, 1, 'ny', 'ga'),
  (5424, 1, 'ny', 'gl'),
  (5425, 1, 'ny', 'gu'),
  (5426, 1, 'ny', 'ha'),
  (5427, 1, 'ny', 'hi'),
  (5428, 1, 'ny', 'hmn'),
  (5429, 1, 'ny', 'hr'),
  (5430, 1, 'ny', 'ht'),
  (5431, 1, 'ny', 'hu'),
  (5432, 1, 'ny', 'hy'),
  (5433, 1, 'ny', 'id'),
  (5434, 1, 'ny', 'ig'),
  (5435, 1, 'ny', 'is'),
  (5436, 1, 'ny', 'it'),
  (5437, 1, 'ny', 'iw'),
  (5438, 1, 'ny', 'ja'),
  (5439, 1, 'ny', 'jw'),
  (5440, 1, 'ny', 'ka'),
  (5441, 1, 'ny', 'kk'),
  (5442, 1, 'ny', 'km'),
  (5443, 1, 'ny', 'kn'),
  (5444, 1, 'ny', 'ko'),
  (5445, 1, 'ny', 'la'),
  (5446, 1, 'ny', 'lo'),
  (5447, 1, 'ny', 'lt'),
  (5448, 1, 'ny', 'lv'),
  (5449, 1, 'ny', 'mg'),
  (5450, 1, 'ny', 'mi'),
  (5451, 1, 'ny', 'mk'),
  (5452, 1, 'ny', 'ml'),
  (5453, 1, 'ny', 'mn'),
  (5454, 1, 'ny', 'mr'),
  (5455, 1, 'ny', 'ms'),
  (5456, 1, 'ny', 'mt'),
  (5457, 1, 'ny', 'my'),
  (5458, 1, 'ny', 'ne'),
  (5459, 1, 'ny', 'nl'),
  (5460, 1, 'ny', 'no'),
  (5461, 1, 'ny', 'pa'),
  (5462, 1, 'ny', 'pl'),
  (5463, 1, 'ny', 'pt'),
  (5464, 1, 'ny', 'ro'),
  (5465, 1, 'ny', 'ru'),
  (5466, 1, 'ny', 'si'),
  (5467, 1, 'ny', 'sk'),
  (5468, 1, 'ny', 'sl'),
  (5469, 1, 'ny', 'so'),
  (5470, 1, 'ny', 'sq'),
  (5471, 1, 'ny', 'sr'),
  (5472, 1, 'ny', 'st'),
  (5473, 1, 'ny', 'su'),
  (5474, 1, 'ny', 'sv'),
  (5475, 1, 'ny', 'sw'),
  (5476, 1, 'ny', 'ta'),
  (5477, 1, 'ny', 'te'),
  (5478, 1, 'ny', 'tg'),
  (5479, 1, 'ny', 'th'),
  (5480, 1, 'ny', 'tl'),
  (5481, 1, 'ny', 'tr'),
  (5482, 1, 'ny', 'uk'),
  (5483, 1, 'ny', 'ur'),
  (5484, 1, 'ny', 'uz'),
  (5485, 1, 'ny', 'vi'),
  (5486, 1, 'ny', 'yi'),
  (5487, 1, 'ny', 'yo'),
  (5488, 1, 'ny', 'zh'),
  (5489, 1, 'ny', 'zh-TW'),
  (5490, 1, 'ny', 'zu'),
  (5491, 1, 'pa', 'af'),
  (5492, 1, 'pa', 'ar'),
  (5493, 1, 'pa', 'az'),
  (5494, 1, 'pa', 'be'),
  (5495, 1, 'pa', 'bg'),
  (5496, 1, 'pa', 'bn'),
  (5497, 1, 'pa', 'bs'),
  (5498, 1, 'pa', 'ca'),
  (5499, 1, 'pa', 'ceb'),
  (5500, 1, 'pa', 'cs'),
  (5501, 1, 'pa', 'cy'),
  (5502, 1, 'pa', 'da'),
  (5503, 1, 'pa', 'de'),
  (5504, 1, 'pa', 'el'),
  (5505, 1, 'pa', 'en'),
  (5506, 1, 'pa', 'eo'),
  (5507, 1, 'pa', 'es'),
  (5508, 1, 'pa', 'et'),
  (5509, 1, 'pa', 'eu'),
  (5510, 1, 'pa', 'fa'),
  (5511, 1, 'pa', 'fi'),
  (5512, 1, 'pa', 'fr'),
  (5513, 1, 'pa', 'ga'),
  (5514, 1, 'pa', 'gl'),
  (5515, 1, 'pa', 'gu'),
  (5516, 1, 'pa', 'ha'),
  (5517, 1, 'pa', 'hi'),
  (5518, 1, 'pa', 'hmn'),
  (5519, 1, 'pa', 'hr'),
  (5520, 1, 'pa', 'ht'),
  (5521, 1, 'pa', 'hu'),
  (5522, 1, 'pa', 'hy'),
  (5523, 1, 'pa', 'id'),
  (5524, 1, 'pa', 'ig'),
  (5525, 1, 'pa', 'is'),
  (5526, 1, 'pa', 'it'),
  (5527, 1, 'pa', 'iw'),
  (5528, 1, 'pa', 'ja'),
  (5529, 1, 'pa', 'jw'),
  (5530, 1, 'pa', 'ka'),
  (5531, 1, 'pa', 'kk'),
  (5532, 1, 'pa', 'km'),
  (5533, 1, 'pa', 'kn'),
  (5534, 1, 'pa', 'ko'),
  (5535, 1, 'pa', 'la'),
  (5536, 1, 'pa', 'lo'),
  (5537, 1, 'pa', 'lt'),
  (5538, 1, 'pa', 'lv'),
  (5539, 1, 'pa', 'mg'),
  (5540, 1, 'pa', 'mi'),
  (5541, 1, 'pa', 'mk'),
  (5542, 1, 'pa', 'ml'),
  (5543, 1, 'pa', 'mn'),
  (5544, 1, 'pa', 'mr'),
  (5545, 1, 'pa', 'ms'),
  (5546, 1, 'pa', 'mt'),
  (5547, 1, 'pa', 'my'),
  (5548, 1, 'pa', 'ne'),
  (5549, 1, 'pa', 'nl'),
  (5550, 1, 'pa', 'no'),
  (5551, 1, 'pa', 'ny'),
  (5552, 1, 'pa', 'pl'),
  (5553, 1, 'pa', 'pt'),
  (5554, 1, 'pa', 'ro'),
  (5555, 1, 'pa', 'ru'),
  (5556, 1, 'pa', 'si'),
  (5557, 1, 'pa', 'sk'),
  (5558, 1, 'pa', 'sl'),
  (5559, 1, 'pa', 'so'),
  (5560, 1, 'pa', 'sq'),
  (5561, 1, 'pa', 'sr'),
  (5562, 1, 'pa', 'st'),
  (5563, 1, 'pa', 'su'),
  (5564, 1, 'pa', 'sv'),
  (5565, 1, 'pa', 'sw'),
  (5566, 1, 'pa', 'ta'),
  (5567, 1, 'pa', 'te'),
  (5568, 1, 'pa', 'tg'),
  (5569, 1, 'pa', 'th'),
  (5570, 1, 'pa', 'tl'),
  (5571, 1, 'pa', 'tr'),
  (5572, 1, 'pa', 'uk'),
  (5573, 1, 'pa', 'ur'),
  (5574, 1, 'pa', 'uz'),
  (5575, 1, 'pa', 'vi'),
  (5576, 1, 'pa', 'yi'),
  (5577, 1, 'pa', 'yo'),
  (5578, 1, 'pa', 'zh'),
  (5579, 1, 'pa', 'zh-TW'),
  (5580, 1, 'pa', 'zu'),
  (5581, 1, 'pl', 'af'),
  (5582, 1, 'pl', 'ar'),
  (5583, 1, 'pl', 'az'),
  (5584, 1, 'pl', 'be'),
  (5585, 1, 'pl', 'bg'),
  (5586, 1, 'pl', 'bn'),
  (5587, 1, 'pl', 'bs'),
  (5588, 1, 'pl', 'ca'),
  (5589, 1, 'pl', 'ceb'),
  (5590, 1, 'pl', 'cs'),
  (5591, 1, 'pl', 'cy'),
  (5592, 1, 'pl', 'da'),
  (5593, 1, 'pl', 'de'),
  (5594, 1, 'pl', 'el'),
  (5595, 1, 'pl', 'en'),
  (5596, 1, 'pl', 'eo'),
  (5597, 1, 'pl', 'es'),
  (5598, 1, 'pl', 'et'),
  (5599, 1, 'pl', 'eu'),
  (5600, 1, 'pl', 'fa'),
  (5601, 1, 'pl', 'fi'),
  (5602, 1, 'pl', 'fr'),
  (5603, 1, 'pl', 'ga'),
  (5604, 1, 'pl', 'gl'),
  (5605, 1, 'pl', 'gu'),
  (5606, 1, 'pl', 'ha'),
  (5607, 1, 'pl', 'hi'),
  (5608, 1, 'pl', 'hmn'),
  (5609, 1, 'pl', 'hr'),
  (5610, 1, 'pl', 'ht'),
  (5611, 1, 'pl', 'hu'),
  (5612, 1, 'pl', 'hy'),
  (5613, 1, 'pl', 'id'),
  (5614, 1, 'pl', 'ig'),
  (5615, 1, 'pl', 'is'),
  (5616, 1, 'pl', 'it'),
  (5617, 1, 'pl', 'iw'),
  (5618, 1, 'pl', 'ja'),
  (5619, 1, 'pl', 'jw'),
  (5620, 1, 'pl', 'ka'),
  (5621, 1, 'pl', 'kk'),
  (5622, 1, 'pl', 'km'),
  (5623, 1, 'pl', 'kn'),
  (5624, 1, 'pl', 'ko'),
  (5625, 1, 'pl', 'la'),
  (5626, 1, 'pl', 'lo'),
  (5627, 1, 'pl', 'lt'),
  (5628, 1, 'pl', 'lv'),
  (5629, 1, 'pl', 'mg'),
  (5630, 1, 'pl', 'mi'),
  (5631, 1, 'pl', 'mk'),
  (5632, 1, 'pl', 'ml'),
  (5633, 1, 'pl', 'mn'),
  (5634, 1, 'pl', 'mr'),
  (5635, 1, 'pl', 'ms'),
  (5636, 1, 'pl', 'mt'),
  (5637, 1, 'pl', 'my'),
  (5638, 1, 'pl', 'ne'),
  (5639, 1, 'pl', 'nl'),
  (5640, 1, 'pl', 'no'),
  (5641, 1, 'pl', 'ny'),
  (5642, 1, 'pl', 'pa'),
  (5643, 1, 'pl', 'pt'),
  (5644, 1, 'pl', 'ro'),
  (5645, 1, 'pl', 'ru'),
  (5646, 1, 'pl', 'si'),
  (5647, 1, 'pl', 'sk'),
  (5648, 1, 'pl', 'sl'),
  (5649, 1, 'pl', 'so'),
  (5650, 1, 'pl', 'sq'),
  (5651, 1, 'pl', 'sr'),
  (5652, 1, 'pl', 'st'),
  (5653, 1, 'pl', 'su'),
  (5654, 1, 'pl', 'sv'),
  (5655, 1, 'pl', 'sw'),
  (5656, 1, 'pl', 'ta'),
  (5657, 1, 'pl', 'te'),
  (5658, 1, 'pl', 'tg'),
  (5659, 1, 'pl', 'th'),
  (5660, 1, 'pl', 'tl'),
  (5661, 1, 'pl', 'tr'),
  (5662, 1, 'pl', 'uk'),
  (5663, 1, 'pl', 'ur'),
  (5664, 1, 'pl', 'uz'),
  (5665, 1, 'pl', 'vi'),
  (5666, 1, 'pl', 'yi'),
  (5667, 1, 'pl', 'yo'),
  (5668, 1, 'pl', 'zh'),
  (5669, 1, 'pl', 'zh-TW'),
  (5670, 1, 'pl', 'zu'),
  (5671, 1, 'pt', 'af'),
  (5672, 1, 'pt', 'ar'),
  (5673, 1, 'pt', 'az'),
  (5674, 1, 'pt', 'be'),
  (5675, 1, 'pt', 'bg'),
  (5676, 1, 'pt', 'bn'),
  (5677, 1, 'pt', 'bs'),
  (5678, 1, 'pt', 'ca'),
  (5679, 1, 'pt', 'ceb'),
  (5680, 1, 'pt', 'cs'),
  (5681, 1, 'pt', 'cy'),
  (5682, 1, 'pt', 'da'),
  (5683, 1, 'pt', 'de'),
  (5684, 1, 'pt', 'el'),
  (5685, 1, 'pt', 'en'),
  (5686, 1, 'pt', 'eo'),
  (5687, 1, 'pt', 'es'),
  (5688, 1, 'pt', 'et'),
  (5689, 1, 'pt', 'eu'),
  (5690, 1, 'pt', 'fa'),
  (5691, 1, 'pt', 'fi'),
  (5692, 1, 'pt', 'fr'),
  (5693, 1, 'pt', 'ga'),
  (5694, 1, 'pt', 'gl'),
  (5695, 1, 'pt', 'gu'),
  (5696, 1, 'pt', 'ha'),
  (5697, 1, 'pt', 'hi'),
  (5698, 1, 'pt', 'hmn'),
  (5699, 1, 'pt', 'hr'),
  (5700, 1, 'pt', 'ht'),
  (5701, 1, 'pt', 'hu'),
  (5702, 1, 'pt', 'hy'),
  (5703, 1, 'pt', 'id'),
  (5704, 1, 'pt', 'ig'),
  (5705, 1, 'pt', 'is'),
  (5706, 1, 'pt', 'it'),
  (5707, 1, 'pt', 'iw'),
  (5708, 1, 'pt', 'ja'),
  (5709, 1, 'pt', 'jw'),
  (5710, 1, 'pt', 'ka'),
  (5711, 1, 'pt', 'kk'),
  (5712, 1, 'pt', 'km'),
  (5713, 1, 'pt', 'kn'),
  (5714, 1, 'pt', 'ko'),
  (5715, 1, 'pt', 'la'),
  (5716, 1, 'pt', 'lo'),
  (5717, 1, 'pt', 'lt'),
  (5718, 1, 'pt', 'lv'),
  (5719, 1, 'pt', 'mg'),
  (5720, 1, 'pt', 'mi'),
  (5721, 1, 'pt', 'mk'),
  (5722, 1, 'pt', 'ml'),
  (5723, 1, 'pt', 'mn'),
  (5724, 1, 'pt', 'mr'),
  (5725, 1, 'pt', 'ms'),
  (5726, 1, 'pt', 'mt'),
  (5727, 1, 'pt', 'my'),
  (5728, 1, 'pt', 'ne'),
  (5729, 1, 'pt', 'nl'),
  (5730, 1, 'pt', 'no'),
  (5731, 1, 'pt', 'ny'),
  (5732, 1, 'pt', 'pa'),
  (5733, 1, 'pt', 'pl'),
  (5734, 1, 'pt', 'ro'),
  (5735, 1, 'pt', 'ru'),
  (5736, 1, 'pt', 'si'),
  (5737, 1, 'pt', 'sk'),
  (5738, 1, 'pt', 'sl'),
  (5739, 1, 'pt', 'so'),
  (5740, 1, 'pt', 'sq'),
  (5741, 1, 'pt', 'sr'),
  (5742, 1, 'pt', 'st'),
  (5743, 1, 'pt', 'su'),
  (5744, 1, 'pt', 'sv'),
  (5745, 1, 'pt', 'sw'),
  (5746, 1, 'pt', 'ta'),
  (5747, 1, 'pt', 'te'),
  (5748, 1, 'pt', 'tg'),
  (5749, 1, 'pt', 'th'),
  (5750, 1, 'pt', 'tl'),
  (5751, 1, 'pt', 'tr'),
  (5752, 1, 'pt', 'uk'),
  (5753, 1, 'pt', 'ur'),
  (5754, 1, 'pt', 'uz'),
  (5755, 1, 'pt', 'vi'),
  (5756, 1, 'pt', 'yi'),
  (5757, 1, 'pt', 'yo'),
  (5758, 1, 'pt', 'zh'),
  (5759, 1, 'pt', 'zh-TW'),
  (5760, 1, 'pt', 'zu'),
  (5761, 1, 'ro', 'af'),
  (5762, 1, 'ro', 'ar'),
  (5763, 1, 'ro', 'az'),
  (5764, 1, 'ro', 'be'),
  (5765, 1, 'ro', 'bg'),
  (5766, 1, 'ro', 'bn'),
  (5767, 1, 'ro', 'bs'),
  (5768, 1, 'ro', 'ca'),
  (5769, 1, 'ro', 'ceb'),
  (5770, 1, 'ro', 'cs'),
  (5771, 1, 'ro', 'cy'),
  (5772, 1, 'ro', 'da'),
  (5773, 1, 'ro', 'de'),
  (5774, 1, 'ro', 'el'),
  (5775, 1, 'ro', 'en'),
  (5776, 1, 'ro', 'eo'),
  (5777, 1, 'ro', 'es'),
  (5778, 1, 'ro', 'et'),
  (5779, 1, 'ro', 'eu'),
  (5780, 1, 'ro', 'fa'),
  (5781, 1, 'ro', 'fi'),
  (5782, 1, 'ro', 'fr'),
  (5783, 1, 'ro', 'ga'),
  (5784, 1, 'ro', 'gl'),
  (5785, 1, 'ro', 'gu'),
  (5786, 1, 'ro', 'ha'),
  (5787, 1, 'ro', 'hi'),
  (5788, 1, 'ro', 'hmn'),
  (5789, 1, 'ro', 'hr'),
  (5790, 1, 'ro', 'ht'),
  (5791, 1, 'ro', 'hu'),
  (5792, 1, 'ro', 'hy'),
  (5793, 1, 'ro', 'id'),
  (5794, 1, 'ro', 'ig'),
  (5795, 1, 'ro', 'is'),
  (5796, 1, 'ro', 'it'),
  (5797, 1, 'ro', 'iw'),
  (5798, 1, 'ro', 'ja'),
  (5799, 1, 'ro', 'jw'),
  (5800, 1, 'ro', 'ka'),
  (5801, 1, 'ro', 'kk'),
  (5802, 1, 'ro', 'km'),
  (5803, 1, 'ro', 'kn'),
  (5804, 1, 'ro', 'ko'),
  (5805, 1, 'ro', 'la'),
  (5806, 1, 'ro', 'lo'),
  (5807, 1, 'ro', 'lt'),
  (5808, 1, 'ro', 'lv'),
  (5809, 1, 'ro', 'mg'),
  (5810, 1, 'ro', 'mi'),
  (5811, 1, 'ro', 'mk'),
  (5812, 1, 'ro', 'ml'),
  (5813, 1, 'ro', 'mn'),
  (5814, 1, 'ro', 'mr'),
  (5815, 1, 'ro', 'ms'),
  (5816, 1, 'ro', 'mt'),
  (5817, 1, 'ro', 'my'),
  (5818, 1, 'ro', 'ne'),
  (5819, 1, 'ro', 'nl'),
  (5820, 1, 'ro', 'no'),
  (5821, 1, 'ro', 'ny'),
  (5822, 1, 'ro', 'pa'),
  (5823, 1, 'ro', 'pl'),
  (5824, 1, 'ro', 'pt'),
  (5825, 1, 'ro', 'ru'),
  (5826, 1, 'ro', 'si'),
  (5827, 1, 'ro', 'sk'),
  (5828, 1, 'ro', 'sl'),
  (5829, 1, 'ro', 'so'),
  (5830, 1, 'ro', 'sq'),
  (5831, 1, 'ro', 'sr'),
  (5832, 1, 'ro', 'st'),
  (5833, 1, 'ro', 'su'),
  (5834, 1, 'ro', 'sv'),
  (5835, 1, 'ro', 'sw'),
  (5836, 1, 'ro', 'ta'),
  (5837, 1, 'ro', 'te'),
  (5838, 1, 'ro', 'tg'),
  (5839, 1, 'ro', 'th'),
  (5840, 1, 'ro', 'tl'),
  (5841, 1, 'ro', 'tr'),
  (5842, 1, 'ro', 'uk'),
  (5843, 1, 'ro', 'ur'),
  (5844, 1, 'ro', 'uz'),
  (5845, 1, 'ro', 'vi'),
  (5846, 1, 'ro', 'yi'),
  (5847, 1, 'ro', 'yo'),
  (5848, 1, 'ro', 'zh'),
  (5849, 1, 'ro', 'zh-TW'),
  (5850, 1, 'ro', 'zu'),
  (5851, 1, 'ru', 'af'),
  (5852, 1, 'ru', 'ar'),
  (5853, 1, 'ru', 'az'),
  (5854, 1, 'ru', 'be'),
  (5855, 1, 'ru', 'bg'),
  (5856, 1, 'ru', 'bn'),
  (5857, 1, 'ru', 'bs'),
  (5858, 1, 'ru', 'ca'),
  (5859, 1, 'ru', 'ceb'),
  (5860, 1, 'ru', 'cs'),
  (5861, 1, 'ru', 'cy'),
  (5862, 1, 'ru', 'da'),
  (5863, 1, 'ru', 'de'),
  (5864, 1, 'ru', 'el'),
  (5865, 1, 'ru', 'en'),
  (5866, 1, 'ru', 'eo'),
  (5867, 1, 'ru', 'es'),
  (5868, 1, 'ru', 'et'),
  (5869, 1, 'ru', 'eu'),
  (5870, 1, 'ru', 'fa'),
  (5871, 1, 'ru', 'fi'),
  (5872, 1, 'ru', 'fr'),
  (5873, 1, 'ru', 'ga'),
  (5874, 1, 'ru', 'gl'),
  (5875, 1, 'ru', 'gu'),
  (5876, 1, 'ru', 'ha'),
  (5877, 1, 'ru', 'hi'),
  (5878, 1, 'ru', 'hmn'),
  (5879, 1, 'ru', 'hr'),
  (5880, 1, 'ru', 'ht'),
  (5881, 1, 'ru', 'hu'),
  (5882, 1, 'ru', 'hy'),
  (5883, 1, 'ru', 'id'),
  (5884, 1, 'ru', 'ig'),
  (5885, 1, 'ru', 'is'),
  (5886, 1, 'ru', 'it'),
  (5887, 1, 'ru', 'iw'),
  (5888, 1, 'ru', 'ja'),
  (5889, 1, 'ru', 'jw'),
  (5890, 1, 'ru', 'ka'),
  (5891, 1, 'ru', 'kk'),
  (5892, 1, 'ru', 'km'),
  (5893, 1, 'ru', 'kn'),
  (5894, 1, 'ru', 'ko'),
  (5895, 1, 'ru', 'la'),
  (5896, 1, 'ru', 'lo'),
  (5897, 1, 'ru', 'lt'),
  (5898, 1, 'ru', 'lv'),
  (5899, 1, 'ru', 'mg'),
  (5900, 1, 'ru', 'mi'),
  (5901, 1, 'ru', 'mk'),
  (5902, 1, 'ru', 'ml'),
  (5903, 1, 'ru', 'mn'),
  (5904, 1, 'ru', 'mr'),
  (5905, 1, 'ru', 'ms'),
  (5906, 1, 'ru', 'mt'),
  (5907, 1, 'ru', 'my'),
  (5908, 1, 'ru', 'ne'),
  (5909, 1, 'ru', 'nl'),
  (5910, 1, 'ru', 'no'),
  (5911, 1, 'ru', 'ny'),
  (5912, 1, 'ru', 'pa'),
  (5913, 1, 'ru', 'pl'),
  (5914, 1, 'ru', 'pt'),
  (5915, 1, 'ru', 'ro'),
  (5916, 1, 'ru', 'si'),
  (5917, 1, 'ru', 'sk'),
  (5918, 1, 'ru', 'sl'),
  (5919, 1, 'ru', 'so'),
  (5920, 1, 'ru', 'sq'),
  (5921, 1, 'ru', 'sr'),
  (5922, 1, 'ru', 'st'),
  (5923, 1, 'ru', 'su'),
  (5924, 1, 'ru', 'sv'),
  (5925, 1, 'ru', 'sw'),
  (5926, 1, 'ru', 'ta'),
  (5927, 1, 'ru', 'te'),
  (5928, 1, 'ru', 'tg'),
  (5929, 1, 'ru', 'th'),
  (5930, 1, 'ru', 'tl'),
  (5931, 1, 'ru', 'tr'),
  (5932, 1, 'ru', 'uk'),
  (5933, 1, 'ru', 'ur'),
  (5934, 1, 'ru', 'uz'),
  (5935, 1, 'ru', 'vi'),
  (5936, 1, 'ru', 'yi'),
  (5937, 1, 'ru', 'yo'),
  (5938, 1, 'ru', 'zh'),
  (5939, 1, 'ru', 'zh-TW'),
  (5940, 1, 'ru', 'zu'),
  (5941, 1, 'si', 'af'),
  (5942, 1, 'si', 'ar'),
  (5943, 1, 'si', 'az'),
  (5944, 1, 'si', 'be'),
  (5945, 1, 'si', 'bg'),
  (5946, 1, 'si', 'bn'),
  (5947, 1, 'si', 'bs'),
  (5948, 1, 'si', 'ca'),
  (5949, 1, 'si', 'ceb'),
  (5950, 1, 'si', 'cs'),
  (5951, 1, 'si', 'cy'),
  (5952, 1, 'si', 'da'),
  (5953, 1, 'si', 'de'),
  (5954, 1, 'si', 'el'),
  (5955, 1, 'si', 'en'),
  (5956, 1, 'si', 'eo'),
  (5957, 1, 'si', 'es'),
  (5958, 1, 'si', 'et'),
  (5959, 1, 'si', 'eu'),
  (5960, 1, 'si', 'fa'),
  (5961, 1, 'si', 'fi'),
  (5962, 1, 'si', 'fr'),
  (5963, 1, 'si', 'ga'),
  (5964, 1, 'si', 'gl'),
  (5965, 1, 'si', 'gu'),
  (5966, 1, 'si', 'ha'),
  (5967, 1, 'si', 'hi'),
  (5968, 1, 'si', 'hmn'),
  (5969, 1, 'si', 'hr'),
  (5970, 1, 'si', 'ht'),
  (5971, 1, 'si', 'hu'),
  (5972, 1, 'si', 'hy'),
  (5973, 1, 'si', 'id'),
  (5974, 1, 'si', 'ig'),
  (5975, 1, 'si', 'is'),
  (5976, 1, 'si', 'it'),
  (5977, 1, 'si', 'iw'),
  (5978, 1, 'si', 'ja'),
  (5979, 1, 'si', 'jw'),
  (5980, 1, 'si', 'ka'),
  (5981, 1, 'si', 'kk'),
  (5982, 1, 'si', 'km'),
  (5983, 1, 'si', 'kn'),
  (5984, 1, 'si', 'ko'),
  (5985, 1, 'si', 'la'),
  (5986, 1, 'si', 'lo'),
  (5987, 1, 'si', 'lt'),
  (5988, 1, 'si', 'lv'),
  (5989, 1, 'si', 'mg'),
  (5990, 1, 'si', 'mi'),
  (5991, 1, 'si', 'mk'),
  (5992, 1, 'si', 'ml'),
  (5993, 1, 'si', 'mn'),
  (5994, 1, 'si', 'mr'),
  (5995, 1, 'si', 'ms'),
  (5996, 1, 'si', 'mt'),
  (5997, 1, 'si', 'my'),
  (5998, 1, 'si', 'ne'),
  (5999, 1, 'si', 'nl'),
  (6000, 1, 'si', 'no'),
  (6001, 1, 'si', 'ny'),
  (6002, 1, 'si', 'pa'),
  (6003, 1, 'si', 'pl'),
  (6004, 1, 'si', 'pt'),
  (6005, 1, 'si', 'ro'),
  (6006, 1, 'si', 'ru'),
  (6007, 1, 'si', 'sk'),
  (6008, 1, 'si', 'sl'),
  (6009, 1, 'si', 'so'),
  (6010, 1, 'si', 'sq'),
  (6011, 1, 'si', 'sr'),
  (6012, 1, 'si', 'st'),
  (6013, 1, 'si', 'su'),
  (6014, 1, 'si', 'sv'),
  (6015, 1, 'si', 'sw'),
  (6016, 1, 'si', 'ta'),
  (6017, 1, 'si', 'te'),
  (6018, 1, 'si', 'tg'),
  (6019, 1, 'si', 'th'),
  (6020, 1, 'si', 'tl'),
  (6021, 1, 'si', 'tr'),
  (6022, 1, 'si', 'uk'),
  (6023, 1, 'si', 'ur'),
  (6024, 1, 'si', 'uz'),
  (6025, 1, 'si', 'vi'),
  (6026, 1, 'si', 'yi'),
  (6027, 1, 'si', 'yo'),
  (6028, 1, 'si', 'zh'),
  (6029, 1, 'si', 'zh-TW'),
  (6030, 1, 'si', 'zu'),
  (6031, 1, 'sk', 'af'),
  (6032, 1, 'sk', 'ar'),
  (6033, 1, 'sk', 'az'),
  (6034, 1, 'sk', 'be'),
  (6035, 1, 'sk', 'bg'),
  (6036, 1, 'sk', 'bn'),
  (6037, 1, 'sk', 'bs'),
  (6038, 1, 'sk', 'ca'),
  (6039, 1, 'sk', 'ceb'),
  (6040, 1, 'sk', 'cs'),
  (6041, 1, 'sk', 'cy'),
  (6042, 1, 'sk', 'da'),
  (6043, 1, 'sk', 'de'),
  (6044, 1, 'sk', 'el'),
  (6045, 1, 'sk', 'en'),
  (6046, 1, 'sk', 'eo'),
  (6047, 1, 'sk', 'es'),
  (6048, 1, 'sk', 'et'),
  (6049, 1, 'sk', 'eu'),
  (6050, 1, 'sk', 'fa'),
  (6051, 1, 'sk', 'fi'),
  (6052, 1, 'sk', 'fr'),
  (6053, 1, 'sk', 'ga'),
  (6054, 1, 'sk', 'gl'),
  (6055, 1, 'sk', 'gu'),
  (6056, 1, 'sk', 'ha'),
  (6057, 1, 'sk', 'hi'),
  (6058, 1, 'sk', 'hmn'),
  (6059, 1, 'sk', 'hr'),
  (6060, 1, 'sk', 'ht'),
  (6061, 1, 'sk', 'hu'),
  (6062, 1, 'sk', 'hy'),
  (6063, 1, 'sk', 'id'),
  (6064, 1, 'sk', 'ig'),
  (6065, 1, 'sk', 'is'),
  (6066, 1, 'sk', 'it'),
  (6067, 1, 'sk', 'iw'),
  (6068, 1, 'sk', 'ja'),
  (6069, 1, 'sk', 'jw'),
  (6070, 1, 'sk', 'ka'),
  (6071, 1, 'sk', 'kk'),
  (6072, 1, 'sk', 'km'),
  (6073, 1, 'sk', 'kn'),
  (6074, 1, 'sk', 'ko'),
  (6075, 1, 'sk', 'la'),
  (6076, 1, 'sk', 'lo'),
  (6077, 1, 'sk', 'lt'),
  (6078, 1, 'sk', 'lv'),
  (6079, 1, 'sk', 'mg'),
  (6080, 1, 'sk', 'mi'),
  (6081, 1, 'sk', 'mk'),
  (6082, 1, 'sk', 'ml'),
  (6083, 1, 'sk', 'mn'),
  (6084, 1, 'sk', 'mr'),
  (6085, 1, 'sk', 'ms'),
  (6086, 1, 'sk', 'mt'),
  (6087, 1, 'sk', 'my'),
  (6088, 1, 'sk', 'ne'),
  (6089, 1, 'sk', 'nl'),
  (6090, 1, 'sk', 'no'),
  (6091, 1, 'sk', 'ny'),
  (6092, 1, 'sk', 'pa'),
  (6093, 1, 'sk', 'pl'),
  (6094, 1, 'sk', 'pt'),
  (6095, 1, 'sk', 'ro'),
  (6096, 1, 'sk', 'ru'),
  (6097, 1, 'sk', 'si'),
  (6098, 1, 'sk', 'sl'),
  (6099, 1, 'sk', 'so'),
  (6100, 1, 'sk', 'sq'),
  (6101, 1, 'sk', 'sr'),
  (6102, 1, 'sk', 'st'),
  (6103, 1, 'sk', 'su'),
  (6104, 1, 'sk', 'sv'),
  (6105, 1, 'sk', 'sw'),
  (6106, 1, 'sk', 'ta'),
  (6107, 1, 'sk', 'te'),
  (6108, 1, 'sk', 'tg'),
  (6109, 1, 'sk', 'th'),
  (6110, 1, 'sk', 'tl'),
  (6111, 1, 'sk', 'tr'),
  (6112, 1, 'sk', 'uk'),
  (6113, 1, 'sk', 'ur'),
  (6114, 1, 'sk', 'uz'),
  (6115, 1, 'sk', 'vi'),
  (6116, 1, 'sk', 'yi'),
  (6117, 1, 'sk', 'yo'),
  (6118, 1, 'sk', 'zh'),
  (6119, 1, 'sk', 'zh-TW'),
  (6120, 1, 'sk', 'zu'),
  (6121, 1, 'sl', 'af'),
  (6122, 1, 'sl', 'ar'),
  (6123, 1, 'sl', 'az'),
  (6124, 1, 'sl', 'be'),
  (6125, 1, 'sl', 'bg'),
  (6126, 1, 'sl', 'bn'),
  (6127, 1, 'sl', 'bs'),
  (6128, 1, 'sl', 'ca'),
  (6129, 1, 'sl', 'ceb'),
  (6130, 1, 'sl', 'cs'),
  (6131, 1, 'sl', 'cy'),
  (6132, 1, 'sl', 'da'),
  (6133, 1, 'sl', 'de'),
  (6134, 1, 'sl', 'el'),
  (6135, 1, 'sl', 'en'),
  (6136, 1, 'sl', 'eo'),
  (6137, 1, 'sl', 'es'),
  (6138, 1, 'sl', 'et'),
  (6139, 1, 'sl', 'eu'),
  (6140, 1, 'sl', 'fa'),
  (6141, 1, 'sl', 'fi'),
  (6142, 1, 'sl', 'fr'),
  (6143, 1, 'sl', 'ga'),
  (6144, 1, 'sl', 'gl'),
  (6145, 1, 'sl', 'gu'),
  (6146, 1, 'sl', 'ha'),
  (6147, 1, 'sl', 'hi'),
  (6148, 1, 'sl', 'hmn'),
  (6149, 1, 'sl', 'hr'),
  (6150, 1, 'sl', 'ht'),
  (6151, 1, 'sl', 'hu'),
  (6152, 1, 'sl', 'hy'),
  (6153, 1, 'sl', 'id'),
  (6154, 1, 'sl', 'ig'),
  (6155, 1, 'sl', 'is'),
  (6156, 1, 'sl', 'it'),
  (6157, 1, 'sl', 'iw'),
  (6158, 1, 'sl', 'ja'),
  (6159, 1, 'sl', 'jw'),
  (6160, 1, 'sl', 'ka'),
  (6161, 1, 'sl', 'kk'),
  (6162, 1, 'sl', 'km'),
  (6163, 1, 'sl', 'kn'),
  (6164, 1, 'sl', 'ko'),
  (6165, 1, 'sl', 'la'),
  (6166, 1, 'sl', 'lo'),
  (6167, 1, 'sl', 'lt'),
  (6168, 1, 'sl', 'lv'),
  (6169, 1, 'sl', 'mg'),
  (6170, 1, 'sl', 'mi'),
  (6171, 1, 'sl', 'mk'),
  (6172, 1, 'sl', 'ml'),
  (6173, 1, 'sl', 'mn'),
  (6174, 1, 'sl', 'mr'),
  (6175, 1, 'sl', 'ms'),
  (6176, 1, 'sl', 'mt'),
  (6177, 1, 'sl', 'my'),
  (6178, 1, 'sl', 'ne'),
  (6179, 1, 'sl', 'nl'),
  (6180, 1, 'sl', 'no'),
  (6181, 1, 'sl', 'ny'),
  (6182, 1, 'sl', 'pa'),
  (6183, 1, 'sl', 'pl'),
  (6184, 1, 'sl', 'pt'),
  (6185, 1, 'sl', 'ro'),
  (6186, 1, 'sl', 'ru'),
  (6187, 1, 'sl', 'si'),
  (6188, 1, 'sl', 'sk'),
  (6189, 1, 'sl', 'so'),
  (6190, 1, 'sl', 'sq'),
  (6191, 1, 'sl', 'sr'),
  (6192, 1, 'sl', 'st'),
  (6193, 1, 'sl', 'su'),
  (6194, 1, 'sl', 'sv'),
  (6195, 1, 'sl', 'sw'),
  (6196, 1, 'sl', 'ta'),
  (6197, 1, 'sl', 'te'),
  (6198, 1, 'sl', 'tg'),
  (6199, 1, 'sl', 'th'),
  (6200, 1, 'sl', 'tl'),
  (6201, 1, 'sl', 'tr'),
  (6202, 1, 'sl', 'uk'),
  (6203, 1, 'sl', 'ur'),
  (6204, 1, 'sl', 'uz'),
  (6205, 1, 'sl', 'vi'),
  (6206, 1, 'sl', 'yi'),
  (6207, 1, 'sl', 'yo'),
  (6208, 1, 'sl', 'zh'),
  (6209, 1, 'sl', 'zh-TW'),
  (6210, 1, 'sl', 'zu'),
  (6211, 1, 'so', 'af'),
  (6212, 1, 'so', 'ar'),
  (6213, 1, 'so', 'az'),
  (6214, 1, 'so', 'be'),
  (6215, 1, 'so', 'bg'),
  (6216, 1, 'so', 'bn'),
  (6217, 1, 'so', 'bs'),
  (6218, 1, 'so', 'ca'),
  (6219, 1, 'so', 'ceb'),
  (6220, 1, 'so', 'cs'),
  (6221, 1, 'so', 'cy'),
  (6222, 1, 'so', 'da'),
  (6223, 1, 'so', 'de'),
  (6224, 1, 'so', 'el'),
  (6225, 1, 'so', 'en'),
  (6226, 1, 'so', 'eo'),
  (6227, 1, 'so', 'es'),
  (6228, 1, 'so', 'et'),
  (6229, 1, 'so', 'eu'),
  (6230, 1, 'so', 'fa'),
  (6231, 1, 'so', 'fi'),
  (6232, 1, 'so', 'fr'),
  (6233, 1, 'so', 'ga'),
  (6234, 1, 'so', 'gl'),
  (6235, 1, 'so', 'gu'),
  (6236, 1, 'so', 'ha'),
  (6237, 1, 'so', 'hi'),
  (6238, 1, 'so', 'hmn'),
  (6239, 1, 'so', 'hr'),
  (6240, 1, 'so', 'ht'),
  (6241, 1, 'so', 'hu'),
  (6242, 1, 'so', 'hy'),
  (6243, 1, 'so', 'id'),
  (6244, 1, 'so', 'ig'),
  (6245, 1, 'so', 'is'),
  (6246, 1, 'so', 'it'),
  (6247, 1, 'so', 'iw'),
  (6248, 1, 'so', 'ja'),
  (6249, 1, 'so', 'jw'),
  (6250, 1, 'so', 'ka'),
  (6251, 1, 'so', 'kk'),
  (6252, 1, 'so', 'km'),
  (6253, 1, 'so', 'kn'),
  (6254, 1, 'so', 'ko'),
  (6255, 1, 'so', 'la'),
  (6256, 1, 'so', 'lo'),
  (6257, 1, 'so', 'lt'),
  (6258, 1, 'so', 'lv'),
  (6259, 1, 'so', 'mg'),
  (6260, 1, 'so', 'mi'),
  (6261, 1, 'so', 'mk'),
  (6262, 1, 'so', 'ml'),
  (6263, 1, 'so', 'mn'),
  (6264, 1, 'so', 'mr'),
  (6265, 1, 'so', 'ms'),
  (6266, 1, 'so', 'mt'),
  (6267, 1, 'so', 'my'),
  (6268, 1, 'so', 'ne'),
  (6269, 1, 'so', 'nl'),
  (6270, 1, 'so', 'no'),
  (6271, 1, 'so', 'ny'),
  (6272, 1, 'so', 'pa'),
  (6273, 1, 'so', 'pl'),
  (6274, 1, 'so', 'pt'),
  (6275, 1, 'so', 'ro'),
  (6276, 1, 'so', 'ru'),
  (6277, 1, 'so', 'si'),
  (6278, 1, 'so', 'sk'),
  (6279, 1, 'so', 'sl'),
  (6280, 1, 'so', 'sq'),
  (6281, 1, 'so', 'sr'),
  (6282, 1, 'so', 'st'),
  (6283, 1, 'so', 'su'),
  (6284, 1, 'so', 'sv'),
  (6285, 1, 'so', 'sw'),
  (6286, 1, 'so', 'ta'),
  (6287, 1, 'so', 'te'),
  (6288, 1, 'so', 'tg'),
  (6289, 1, 'so', 'th'),
  (6290, 1, 'so', 'tl'),
  (6291, 1, 'so', 'tr'),
  (6292, 1, 'so', 'uk'),
  (6293, 1, 'so', 'ur'),
  (6294, 1, 'so', 'uz'),
  (6295, 1, 'so', 'vi'),
  (6296, 1, 'so', 'yi'),
  (6297, 1, 'so', 'yo'),
  (6298, 1, 'so', 'zh'),
  (6299, 1, 'so', 'zh-TW'),
  (6300, 1, 'so', 'zu'),
  (6301, 1, 'sq', 'af'),
  (6302, 1, 'sq', 'ar'),
  (6303, 1, 'sq', 'az'),
  (6304, 1, 'sq', 'be'),
  (6305, 1, 'sq', 'bg'),
  (6306, 1, 'sq', 'bn'),
  (6307, 1, 'sq', 'bs'),
  (6308, 1, 'sq', 'ca'),
  (6309, 1, 'sq', 'ceb'),
  (6310, 1, 'sq', 'cs'),
  (6311, 1, 'sq', 'cy'),
  (6312, 1, 'sq', 'da'),
  (6313, 1, 'sq', 'de'),
  (6314, 1, 'sq', 'el'),
  (6315, 1, 'sq', 'en'),
  (6316, 1, 'sq', 'eo'),
  (6317, 1, 'sq', 'es'),
  (6318, 1, 'sq', 'et'),
  (6319, 1, 'sq', 'eu'),
  (6320, 1, 'sq', 'fa'),
  (6321, 1, 'sq', 'fi'),
  (6322, 1, 'sq', 'fr'),
  (6323, 1, 'sq', 'ga'),
  (6324, 1, 'sq', 'gl'),
  (6325, 1, 'sq', 'gu'),
  (6326, 1, 'sq', 'ha'),
  (6327, 1, 'sq', 'hi'),
  (6328, 1, 'sq', 'hmn'),
  (6329, 1, 'sq', 'hr'),
  (6330, 1, 'sq', 'ht'),
  (6331, 1, 'sq', 'hu'),
  (6332, 1, 'sq', 'hy'),
  (6333, 1, 'sq', 'id'),
  (6334, 1, 'sq', 'ig'),
  (6335, 1, 'sq', 'is'),
  (6336, 1, 'sq', 'it'),
  (6337, 1, 'sq', 'iw'),
  (6338, 1, 'sq', 'ja'),
  (6339, 1, 'sq', 'jw'),
  (6340, 1, 'sq', 'ka'),
  (6341, 1, 'sq', 'kk'),
  (6342, 1, 'sq', 'km'),
  (6343, 1, 'sq', 'kn'),
  (6344, 1, 'sq', 'ko'),
  (6345, 1, 'sq', 'la'),
  (6346, 1, 'sq', 'lo'),
  (6347, 1, 'sq', 'lt'),
  (6348, 1, 'sq', 'lv'),
  (6349, 1, 'sq', 'mg'),
  (6350, 1, 'sq', 'mi'),
  (6351, 1, 'sq', 'mk'),
  (6352, 1, 'sq', 'ml'),
  (6353, 1, 'sq', 'mn'),
  (6354, 1, 'sq', 'mr'),
  (6355, 1, 'sq', 'ms'),
  (6356, 1, 'sq', 'mt'),
  (6357, 1, 'sq', 'my'),
  (6358, 1, 'sq', 'ne'),
  (6359, 1, 'sq', 'nl'),
  (6360, 1, 'sq', 'no'),
  (6361, 1, 'sq', 'ny'),
  (6362, 1, 'sq', 'pa'),
  (6363, 1, 'sq', 'pl'),
  (6364, 1, 'sq', 'pt'),
  (6365, 1, 'sq', 'ro'),
  (6366, 1, 'sq', 'ru'),
  (6367, 1, 'sq', 'si'),
  (6368, 1, 'sq', 'sk'),
  (6369, 1, 'sq', 'sl'),
  (6370, 1, 'sq', 'so'),
  (6371, 1, 'sq', 'sr'),
  (6372, 1, 'sq', 'st'),
  (6373, 1, 'sq', 'su'),
  (6374, 1, 'sq', 'sv'),
  (6375, 1, 'sq', 'sw'),
  (6376, 1, 'sq', 'ta'),
  (6377, 1, 'sq', 'te'),
  (6378, 1, 'sq', 'tg'),
  (6379, 1, 'sq', 'th'),
  (6380, 1, 'sq', 'tl'),
  (6381, 1, 'sq', 'tr'),
  (6382, 1, 'sq', 'uk'),
  (6383, 1, 'sq', 'ur'),
  (6384, 1, 'sq', 'uz'),
  (6385, 1, 'sq', 'vi'),
  (6386, 1, 'sq', 'yi'),
  (6387, 1, 'sq', 'yo'),
  (6388, 1, 'sq', 'zh'),
  (6389, 1, 'sq', 'zh-TW'),
  (6390, 1, 'sq', 'zu'),
  (6391, 1, 'sr', 'af'),
  (6392, 1, 'sr', 'ar'),
  (6393, 1, 'sr', 'az'),
  (6394, 1, 'sr', 'be'),
  (6395, 1, 'sr', 'bg'),
  (6396, 1, 'sr', 'bn'),
  (6397, 1, 'sr', 'bs'),
  (6398, 1, 'sr', 'ca'),
  (6399, 1, 'sr', 'ceb'),
  (6400, 1, 'sr', 'cs'),
  (6401, 1, 'sr', 'cy'),
  (6402, 1, 'sr', 'da'),
  (6403, 1, 'sr', 'de'),
  (6404, 1, 'sr', 'el'),
  (6405, 1, 'sr', 'en'),
  (6406, 1, 'sr', 'eo'),
  (6407, 1, 'sr', 'es'),
  (6408, 1, 'sr', 'et'),
  (6409, 1, 'sr', 'eu'),
  (6410, 1, 'sr', 'fa'),
  (6411, 1, 'sr', 'fi'),
  (6412, 1, 'sr', 'fr'),
  (6413, 1, 'sr', 'ga'),
  (6414, 1, 'sr', 'gl'),
  (6415, 1, 'sr', 'gu'),
  (6416, 1, 'sr', 'ha'),
  (6417, 1, 'sr', 'hi'),
  (6418, 1, 'sr', 'hmn'),
  (6419, 1, 'sr', 'hr'),
  (6420, 1, 'sr', 'ht'),
  (6421, 1, 'sr', 'hu'),
  (6422, 1, 'sr', 'hy'),
  (6423, 1, 'sr', 'id'),
  (6424, 1, 'sr', 'ig'),
  (6425, 1, 'sr', 'is'),
  (6426, 1, 'sr', 'it'),
  (6427, 1, 'sr', 'iw'),
  (6428, 1, 'sr', 'ja'),
  (6429, 1, 'sr', 'jw'),
  (6430, 1, 'sr', 'ka'),
  (6431, 1, 'sr', 'kk'),
  (6432, 1, 'sr', 'km'),
  (6433, 1, 'sr', 'kn'),
  (6434, 1, 'sr', 'ko'),
  (6435, 1, 'sr', 'la'),
  (6436, 1, 'sr', 'lo'),
  (6437, 1, 'sr', 'lt'),
  (6438, 1, 'sr', 'lv'),
  (6439, 1, 'sr', 'mg'),
  (6440, 1, 'sr', 'mi'),
  (6441, 1, 'sr', 'mk'),
  (6442, 1, 'sr', 'ml'),
  (6443, 1, 'sr', 'mn'),
  (6444, 1, 'sr', 'mr'),
  (6445, 1, 'sr', 'ms'),
  (6446, 1, 'sr', 'mt'),
  (6447, 1, 'sr', 'my'),
  (6448, 1, 'sr', 'ne'),
  (6449, 1, 'sr', 'nl'),
  (6450, 1, 'sr', 'no'),
  (6451, 1, 'sr', 'ny'),
  (6452, 1, 'sr', 'pa'),
  (6453, 1, 'sr', 'pl'),
  (6454, 1, 'sr', 'pt'),
  (6455, 1, 'sr', 'ro'),
  (6456, 1, 'sr', 'ru'),
  (6457, 1, 'sr', 'si'),
  (6458, 1, 'sr', 'sk'),
  (6459, 1, 'sr', 'sl'),
  (6460, 1, 'sr', 'so'),
  (6461, 1, 'sr', 'sq'),
  (6462, 1, 'sr', 'st'),
  (6463, 1, 'sr', 'su'),
  (6464, 1, 'sr', 'sv'),
  (6465, 1, 'sr', 'sw'),
  (6466, 1, 'sr', 'ta'),
  (6467, 1, 'sr', 'te'),
  (6468, 1, 'sr', 'tg'),
  (6469, 1, 'sr', 'th'),
  (6470, 1, 'sr', 'tl'),
  (6471, 1, 'sr', 'tr'),
  (6472, 1, 'sr', 'uk'),
  (6473, 1, 'sr', 'ur'),
  (6474, 1, 'sr', 'uz'),
  (6475, 1, 'sr', 'vi'),
  (6476, 1, 'sr', 'yi'),
  (6477, 1, 'sr', 'yo'),
  (6478, 1, 'sr', 'zh'),
  (6479, 1, 'sr', 'zh-TW'),
  (6480, 1, 'sr', 'zu'),
  (6481, 1, 'st', 'af'),
  (6482, 1, 'st', 'ar'),
  (6483, 1, 'st', 'az'),
  (6484, 1, 'st', 'be'),
  (6485, 1, 'st', 'bg'),
  (6486, 1, 'st', 'bn'),
  (6487, 1, 'st', 'bs'),
  (6488, 1, 'st', 'ca'),
  (6489, 1, 'st', 'ceb'),
  (6490, 1, 'st', 'cs'),
  (6491, 1, 'st', 'cy'),
  (6492, 1, 'st', 'da'),
  (6493, 1, 'st', 'de'),
  (6494, 1, 'st', 'el'),
  (6495, 1, 'st', 'en'),
  (6496, 1, 'st', 'eo'),
  (6497, 1, 'st', 'es'),
  (6498, 1, 'st', 'et'),
  (6499, 1, 'st', 'eu'),
  (6500, 1, 'st', 'fa'),
  (6501, 1, 'st', 'fi'),
  (6502, 1, 'st', 'fr'),
  (6503, 1, 'st', 'ga'),
  (6504, 1, 'st', 'gl'),
  (6505, 1, 'st', 'gu'),
  (6506, 1, 'st', 'ha'),
  (6507, 1, 'st', 'hi'),
  (6508, 1, 'st', 'hmn'),
  (6509, 1, 'st', 'hr'),
  (6510, 1, 'st', 'ht'),
  (6511, 1, 'st', 'hu'),
  (6512, 1, 'st', 'hy'),
  (6513, 1, 'st', 'id'),
  (6514, 1, 'st', 'ig'),
  (6515, 1, 'st', 'is'),
  (6516, 1, 'st', 'it'),
  (6517, 1, 'st', 'iw'),
  (6518, 1, 'st', 'ja'),
  (6519, 1, 'st', 'jw'),
  (6520, 1, 'st', 'ka'),
  (6521, 1, 'st', 'kk'),
  (6522, 1, 'st', 'km'),
  (6523, 1, 'st', 'kn'),
  (6524, 1, 'st', 'ko'),
  (6525, 1, 'st', 'la'),
  (6526, 1, 'st', 'lo'),
  (6527, 1, 'st', 'lt'),
  (6528, 1, 'st', 'lv'),
  (6529, 1, 'st', 'mg'),
  (6530, 1, 'st', 'mi'),
  (6531, 1, 'st', 'mk'),
  (6532, 1, 'st', 'ml'),
  (6533, 1, 'st', 'mn'),
  (6534, 1, 'st', 'mr'),
  (6535, 1, 'st', 'ms'),
  (6536, 1, 'st', 'mt'),
  (6537, 1, 'st', 'my'),
  (6538, 1, 'st', 'ne'),
  (6539, 1, 'st', 'nl'),
  (6540, 1, 'st', 'no'),
  (6541, 1, 'st', 'ny'),
  (6542, 1, 'st', 'pa'),
  (6543, 1, 'st', 'pl'),
  (6544, 1, 'st', 'pt'),
  (6545, 1, 'st', 'ro'),
  (6546, 1, 'st', 'ru'),
  (6547, 1, 'st', 'si'),
  (6548, 1, 'st', 'sk'),
  (6549, 1, 'st', 'sl'),
  (6550, 1, 'st', 'so'),
  (6551, 1, 'st', 'sq'),
  (6552, 1, 'st', 'sr'),
  (6553, 1, 'st', 'su'),
  (6554, 1, 'st', 'sv'),
  (6555, 1, 'st', 'sw'),
  (6556, 1, 'st', 'ta'),
  (6557, 1, 'st', 'te'),
  (6558, 1, 'st', 'tg'),
  (6559, 1, 'st', 'th'),
  (6560, 1, 'st', 'tl'),
  (6561, 1, 'st', 'tr'),
  (6562, 1, 'st', 'uk'),
  (6563, 1, 'st', 'ur'),
  (6564, 1, 'st', 'uz'),
  (6565, 1, 'st', 'vi'),
  (6566, 1, 'st', 'yi'),
  (6567, 1, 'st', 'yo'),
  (6568, 1, 'st', 'zh'),
  (6569, 1, 'st', 'zh-TW'),
  (6570, 1, 'st', 'zu'),
  (6571, 1, 'su', 'af'),
  (6572, 1, 'su', 'ar'),
  (6573, 1, 'su', 'az'),
  (6574, 1, 'su', 'be'),
  (6575, 1, 'su', 'bg'),
  (6576, 1, 'su', 'bn'),
  (6577, 1, 'su', 'bs'),
  (6578, 1, 'su', 'ca'),
  (6579, 1, 'su', 'ceb'),
  (6580, 1, 'su', 'cs'),
  (6581, 1, 'su', 'cy'),
  (6582, 1, 'su', 'da'),
  (6583, 1, 'su', 'de'),
  (6584, 1, 'su', 'el'),
  (6585, 1, 'su', 'en'),
  (6586, 1, 'su', 'eo'),
  (6587, 1, 'su', 'es'),
  (6588, 1, 'su', 'et'),
  (6589, 1, 'su', 'eu'),
  (6590, 1, 'su', 'fa'),
  (6591, 1, 'su', 'fi'),
  (6592, 1, 'su', 'fr'),
  (6593, 1, 'su', 'ga'),
  (6594, 1, 'su', 'gl'),
  (6595, 1, 'su', 'gu'),
  (6596, 1, 'su', 'ha'),
  (6597, 1, 'su', 'hi'),
  (6598, 1, 'su', 'hmn'),
  (6599, 1, 'su', 'hr'),
  (6600, 1, 'su', 'ht'),
  (6601, 1, 'su', 'hu'),
  (6602, 1, 'su', 'hy'),
  (6603, 1, 'su', 'id'),
  (6604, 1, 'su', 'ig'),
  (6605, 1, 'su', 'is'),
  (6606, 1, 'su', 'it'),
  (6607, 1, 'su', 'iw'),
  (6608, 1, 'su', 'ja'),
  (6609, 1, 'su', 'jw'),
  (6610, 1, 'su', 'ka'),
  (6611, 1, 'su', 'kk'),
  (6612, 1, 'su', 'km'),
  (6613, 1, 'su', 'kn'),
  (6614, 1, 'su', 'ko'),
  (6615, 1, 'su', 'la'),
  (6616, 1, 'su', 'lo'),
  (6617, 1, 'su', 'lt'),
  (6618, 1, 'su', 'lv'),
  (6619, 1, 'su', 'mg'),
  (6620, 1, 'su', 'mi'),
  (6621, 1, 'su', 'mk'),
  (6622, 1, 'su', 'ml'),
  (6623, 1, 'su', 'mn'),
  (6624, 1, 'su', 'mr'),
  (6625, 1, 'su', 'ms'),
  (6626, 1, 'su', 'mt'),
  (6627, 1, 'su', 'my'),
  (6628, 1, 'su', 'ne'),
  (6629, 1, 'su', 'nl'),
  (6630, 1, 'su', 'no'),
  (6631, 1, 'su', 'ny'),
  (6632, 1, 'su', 'pa'),
  (6633, 1, 'su', 'pl'),
  (6634, 1, 'su', 'pt'),
  (6635, 1, 'su', 'ro'),
  (6636, 1, 'su', 'ru'),
  (6637, 1, 'su', 'si'),
  (6638, 1, 'su', 'sk'),
  (6639, 1, 'su', 'sl'),
  (6640, 1, 'su', 'so'),
  (6641, 1, 'su', 'sq'),
  (6642, 1, 'su', 'sr'),
  (6643, 1, 'su', 'st'),
  (6644, 1, 'su', 'sv'),
  (6645, 1, 'su', 'sw'),
  (6646, 1, 'su', 'ta'),
  (6647, 1, 'su', 'te'),
  (6648, 1, 'su', 'tg'),
  (6649, 1, 'su', 'th'),
  (6650, 1, 'su', 'tl'),
  (6651, 1, 'su', 'tr'),
  (6652, 1, 'su', 'uk'),
  (6653, 1, 'su', 'ur'),
  (6654, 1, 'su', 'uz'),
  (6655, 1, 'su', 'vi'),
  (6656, 1, 'su', 'yi'),
  (6657, 1, 'su', 'yo'),
  (6658, 1, 'su', 'zh'),
  (6659, 1, 'su', 'zh-TW'),
  (6660, 1, 'su', 'zu'),
  (6661, 1, 'sv', 'af'),
  (6662, 1, 'sv', 'ar'),
  (6663, 1, 'sv', 'az'),
  (6664, 1, 'sv', 'be'),
  (6665, 1, 'sv', 'bg'),
  (6666, 1, 'sv', 'bn'),
  (6667, 1, 'sv', 'bs'),
  (6668, 1, 'sv', 'ca'),
  (6669, 1, 'sv', 'ceb'),
  (6670, 1, 'sv', 'cs'),
  (6671, 1, 'sv', 'cy'),
  (6672, 1, 'sv', 'da'),
  (6673, 1, 'sv', 'de'),
  (6674, 1, 'sv', 'el'),
  (6675, 1, 'sv', 'en'),
  (6676, 1, 'sv', 'eo'),
  (6677, 1, 'sv', 'es'),
  (6678, 1, 'sv', 'et'),
  (6679, 1, 'sv', 'eu'),
  (6680, 1, 'sv', 'fa'),
  (6681, 1, 'sv', 'fi'),
  (6682, 1, 'sv', 'fr'),
  (6683, 1, 'sv', 'ga'),
  (6684, 1, 'sv', 'gl'),
  (6685, 1, 'sv', 'gu'),
  (6686, 1, 'sv', 'ha'),
  (6687, 1, 'sv', 'hi'),
  (6688, 1, 'sv', 'hmn'),
  (6689, 1, 'sv', 'hr'),
  (6690, 1, 'sv', 'ht'),
  (6691, 1, 'sv', 'hu'),
  (6692, 1, 'sv', 'hy'),
  (6693, 1, 'sv', 'id'),
  (6694, 1, 'sv', 'ig'),
  (6695, 1, 'sv', 'is'),
  (6696, 1, 'sv', 'it'),
  (6697, 1, 'sv', 'iw'),
  (6698, 1, 'sv', 'ja'),
  (6699, 1, 'sv', 'jw'),
  (6700, 1, 'sv', 'ka'),
  (6701, 1, 'sv', 'kk'),
  (6702, 1, 'sv', 'km'),
  (6703, 1, 'sv', 'kn'),
  (6704, 1, 'sv', 'ko'),
  (6705, 1, 'sv', 'la'),
  (6706, 1, 'sv', 'lo'),
  (6707, 1, 'sv', 'lt'),
  (6708, 1, 'sv', 'lv'),
  (6709, 1, 'sv', 'mg'),
  (6710, 1, 'sv', 'mi'),
  (6711, 1, 'sv', 'mk'),
  (6712, 1, 'sv', 'ml'),
  (6713, 1, 'sv', 'mn'),
  (6714, 1, 'sv', 'mr'),
  (6715, 1, 'sv', 'ms'),
  (6716, 1, 'sv', 'mt'),
  (6717, 1, 'sv', 'my'),
  (6718, 1, 'sv', 'ne'),
  (6719, 1, 'sv', 'nl'),
  (6720, 1, 'sv', 'no'),
  (6721, 1, 'sv', 'ny'),
  (6722, 1, 'sv', 'pa'),
  (6723, 1, 'sv', 'pl'),
  (6724, 1, 'sv', 'pt'),
  (6725, 1, 'sv', 'ro'),
  (6726, 1, 'sv', 'ru'),
  (6727, 1, 'sv', 'si'),
  (6728, 1, 'sv', 'sk'),
  (6729, 1, 'sv', 'sl'),
  (6730, 1, 'sv', 'so'),
  (6731, 1, 'sv', 'sq'),
  (6732, 1, 'sv', 'sr'),
  (6733, 1, 'sv', 'st'),
  (6734, 1, 'sv', 'su'),
  (6735, 1, 'sv', 'sw'),
  (6736, 1, 'sv', 'ta'),
  (6737, 1, 'sv', 'te'),
  (6738, 1, 'sv', 'tg'),
  (6739, 1, 'sv', 'th'),
  (6740, 1, 'sv', 'tl'),
  (6741, 1, 'sv', 'tr'),
  (6742, 1, 'sv', 'uk'),
  (6743, 1, 'sv', 'ur'),
  (6744, 1, 'sv', 'uz'),
  (6745, 1, 'sv', 'vi'),
  (6746, 1, 'sv', 'yi'),
  (6747, 1, 'sv', 'yo'),
  (6748, 1, 'sv', 'zh'),
  (6749, 1, 'sv', 'zh-TW'),
  (6750, 1, 'sv', 'zu'),
  (6751, 1, 'sw', 'af'),
  (6752, 1, 'sw', 'ar'),
  (6753, 1, 'sw', 'az'),
  (6754, 1, 'sw', 'be'),
  (6755, 1, 'sw', 'bg'),
  (6756, 1, 'sw', 'bn'),
  (6757, 1, 'sw', 'bs'),
  (6758, 1, 'sw', 'ca'),
  (6759, 1, 'sw', 'ceb'),
  (6760, 1, 'sw', 'cs'),
  (6761, 1, 'sw', 'cy'),
  (6762, 1, 'sw', 'da'),
  (6763, 1, 'sw', 'de'),
  (6764, 1, 'sw', 'el'),
  (6765, 1, 'sw', 'en'),
  (6766, 1, 'sw', 'eo'),
  (6767, 1, 'sw', 'es'),
  (6768, 1, 'sw', 'et'),
  (6769, 1, 'sw', 'eu'),
  (6770, 1, 'sw', 'fa'),
  (6771, 1, 'sw', 'fi'),
  (6772, 1, 'sw', 'fr'),
  (6773, 1, 'sw', 'ga'),
  (6774, 1, 'sw', 'gl'),
  (6775, 1, 'sw', 'gu'),
  (6776, 1, 'sw', 'ha'),
  (6777, 1, 'sw', 'hi'),
  (6778, 1, 'sw', 'hmn'),
  (6779, 1, 'sw', 'hr'),
  (6780, 1, 'sw', 'ht'),
  (6781, 1, 'sw', 'hu'),
  (6782, 1, 'sw', 'hy'),
  (6783, 1, 'sw', 'id'),
  (6784, 1, 'sw', 'ig'),
  (6785, 1, 'sw', 'is'),
  (6786, 1, 'sw', 'it'),
  (6787, 1, 'sw', 'iw'),
  (6788, 1, 'sw', 'ja'),
  (6789, 1, 'sw', 'jw'),
  (6790, 1, 'sw', 'ka'),
  (6791, 1, 'sw', 'kk'),
  (6792, 1, 'sw', 'km'),
  (6793, 1, 'sw', 'kn'),
  (6794, 1, 'sw', 'ko'),
  (6795, 1, 'sw', 'la'),
  (6796, 1, 'sw', 'lo'),
  (6797, 1, 'sw', 'lt'),
  (6798, 1, 'sw', 'lv'),
  (6799, 1, 'sw', 'mg'),
  (6800, 1, 'sw', 'mi'),
  (6801, 1, 'sw', 'mk'),
  (6802, 1, 'sw', 'ml'),
  (6803, 1, 'sw', 'mn'),
  (6804, 1, 'sw', 'mr'),
  (6805, 1, 'sw', 'ms'),
  (6806, 1, 'sw', 'mt'),
  (6807, 1, 'sw', 'my'),
  (6808, 1, 'sw', 'ne'),
  (6809, 1, 'sw', 'nl'),
  (6810, 1, 'sw', 'no'),
  (6811, 1, 'sw', 'ny'),
  (6812, 1, 'sw', 'pa'),
  (6813, 1, 'sw', 'pl'),
  (6814, 1, 'sw', 'pt'),
  (6815, 1, 'sw', 'ro'),
  (6816, 1, 'sw', 'ru'),
  (6817, 1, 'sw', 'si'),
  (6818, 1, 'sw', 'sk'),
  (6819, 1, 'sw', 'sl'),
  (6820, 1, 'sw', 'so'),
  (6821, 1, 'sw', 'sq'),
  (6822, 1, 'sw', 'sr'),
  (6823, 1, 'sw', 'st'),
  (6824, 1, 'sw', 'su'),
  (6825, 1, 'sw', 'sv'),
  (6826, 1, 'sw', 'ta'),
  (6827, 1, 'sw', 'te'),
  (6828, 1, 'sw', 'tg'),
  (6829, 1, 'sw', 'th'),
  (6830, 1, 'sw', 'tl'),
  (6831, 1, 'sw', 'tr'),
  (6832, 1, 'sw', 'uk'),
  (6833, 1, 'sw', 'ur'),
  (6834, 1, 'sw', 'uz'),
  (6835, 1, 'sw', 'vi'),
  (6836, 1, 'sw', 'yi'),
  (6837, 1, 'sw', 'yo'),
  (6838, 1, 'sw', 'zh'),
  (6839, 1, 'sw', 'zh-TW'),
  (6840, 1, 'sw', 'zu'),
  (6841, 1, 'ta', 'af'),
  (6842, 1, 'ta', 'ar'),
  (6843, 1, 'ta', 'az'),
  (6844, 1, 'ta', 'be'),
  (6845, 1, 'ta', 'bg'),
  (6846, 1, 'ta', 'bn'),
  (6847, 1, 'ta', 'bs'),
  (6848, 1, 'ta', 'ca'),
  (6849, 1, 'ta', 'ceb'),
  (6850, 1, 'ta', 'cs'),
  (6851, 1, 'ta', 'cy'),
  (6852, 1, 'ta', 'da'),
  (6853, 1, 'ta', 'de'),
  (6854, 1, 'ta', 'el'),
  (6855, 1, 'ta', 'en'),
  (6856, 1, 'ta', 'eo'),
  (6857, 1, 'ta', 'es'),
  (6858, 1, 'ta', 'et'),
  (6859, 1, 'ta', 'eu'),
  (6860, 1, 'ta', 'fa'),
  (6861, 1, 'ta', 'fi'),
  (6862, 1, 'ta', 'fr'),
  (6863, 1, 'ta', 'ga'),
  (6864, 1, 'ta', 'gl'),
  (6865, 1, 'ta', 'gu'),
  (6866, 1, 'ta', 'ha'),
  (6867, 1, 'ta', 'hi'),
  (6868, 1, 'ta', 'hmn'),
  (6869, 1, 'ta', 'hr'),
  (6870, 1, 'ta', 'ht'),
  (6871, 1, 'ta', 'hu'),
  (6872, 1, 'ta', 'hy'),
  (6873, 1, 'ta', 'id'),
  (6874, 1, 'ta', 'ig'),
  (6875, 1, 'ta', 'is'),
  (6876, 1, 'ta', 'it'),
  (6877, 1, 'ta', 'iw'),
  (6878, 1, 'ta', 'ja'),
  (6879, 1, 'ta', 'jw'),
  (6880, 1, 'ta', 'ka'),
  (6881, 1, 'ta', 'kk'),
  (6882, 1, 'ta', 'km'),
  (6883, 1, 'ta', 'kn'),
  (6884, 1, 'ta', 'ko'),
  (6885, 1, 'ta', 'la'),
  (6886, 1, 'ta', 'lo'),
  (6887, 1, 'ta', 'lt'),
  (6888, 1, 'ta', 'lv'),
  (6889, 1, 'ta', 'mg'),
  (6890, 1, 'ta', 'mi'),
  (6891, 1, 'ta', 'mk'),
  (6892, 1, 'ta', 'ml'),
  (6893, 1, 'ta', 'mn'),
  (6894, 1, 'ta', 'mr'),
  (6895, 1, 'ta', 'ms'),
  (6896, 1, 'ta', 'mt'),
  (6897, 1, 'ta', 'my'),
  (6898, 1, 'ta', 'ne'),
  (6899, 1, 'ta', 'nl'),
  (6900, 1, 'ta', 'no'),
  (6901, 1, 'ta', 'ny'),
  (6902, 1, 'ta', 'pa'),
  (6903, 1, 'ta', 'pl'),
  (6904, 1, 'ta', 'pt'),
  (6905, 1, 'ta', 'ro'),
  (6906, 1, 'ta', 'ru'),
  (6907, 1, 'ta', 'si'),
  (6908, 1, 'ta', 'sk'),
  (6909, 1, 'ta', 'sl'),
  (6910, 1, 'ta', 'so'),
  (6911, 1, 'ta', 'sq'),
  (6912, 1, 'ta', 'sr'),
  (6913, 1, 'ta', 'st'),
  (6914, 1, 'ta', 'su'),
  (6915, 1, 'ta', 'sv'),
  (6916, 1, 'ta', 'sw'),
  (6917, 1, 'ta', 'te'),
  (6918, 1, 'ta', 'tg'),
  (6919, 1, 'ta', 'th'),
  (6920, 1, 'ta', 'tl'),
  (6921, 1, 'ta', 'tr'),
  (6922, 1, 'ta', 'uk'),
  (6923, 1, 'ta', 'ur'),
  (6924, 1, 'ta', 'uz'),
  (6925, 1, 'ta', 'vi'),
  (6926, 1, 'ta', 'yi'),
  (6927, 1, 'ta', 'yo'),
  (6928, 1, 'ta', 'zh'),
  (6929, 1, 'ta', 'zh-TW'),
  (6930, 1, 'ta', 'zu'),
  (6931, 1, 'te', 'af'),
  (6932, 1, 'te', 'ar'),
  (6933, 1, 'te', 'az'),
  (6934, 1, 'te', 'be'),
  (6935, 1, 'te', 'bg'),
  (6936, 1, 'te', 'bn'),
  (6937, 1, 'te', 'bs'),
  (6938, 1, 'te', 'ca'),
  (6939, 1, 'te', 'ceb'),
  (6940, 1, 'te', 'cs'),
  (6941, 1, 'te', 'cy'),
  (6942, 1, 'te', 'da'),
  (6943, 1, 'te', 'de'),
  (6944, 1, 'te', 'el'),
  (6945, 1, 'te', 'en'),
  (6946, 1, 'te', 'eo'),
  (6947, 1, 'te', 'es'),
  (6948, 1, 'te', 'et'),
  (6949, 1, 'te', 'eu'),
  (6950, 1, 'te', 'fa'),
  (6951, 1, 'te', 'fi'),
  (6952, 1, 'te', 'fr'),
  (6953, 1, 'te', 'ga'),
  (6954, 1, 'te', 'gl'),
  (6955, 1, 'te', 'gu'),
  (6956, 1, 'te', 'ha'),
  (6957, 1, 'te', 'hi'),
  (6958, 1, 'te', 'hmn'),
  (6959, 1, 'te', 'hr'),
  (6960, 1, 'te', 'ht'),
  (6961, 1, 'te', 'hu'),
  (6962, 1, 'te', 'hy'),
  (6963, 1, 'te', 'id'),
  (6964, 1, 'te', 'ig'),
  (6965, 1, 'te', 'is'),
  (6966, 1, 'te', 'it'),
  (6967, 1, 'te', 'iw'),
  (6968, 1, 'te', 'ja'),
  (6969, 1, 'te', 'jw'),
  (6970, 1, 'te', 'ka'),
  (6971, 1, 'te', 'kk'),
  (6972, 1, 'te', 'km'),
  (6973, 1, 'te', 'kn'),
  (6974, 1, 'te', 'ko'),
  (6975, 1, 'te', 'la'),
  (6976, 1, 'te', 'lo'),
  (6977, 1, 'te', 'lt'),
  (6978, 1, 'te', 'lv'),
  (6979, 1, 'te', 'mg'),
  (6980, 1, 'te', 'mi'),
  (6981, 1, 'te', 'mk'),
  (6982, 1, 'te', 'ml'),
  (6983, 1, 'te', 'mn'),
  (6984, 1, 'te', 'mr'),
  (6985, 1, 'te', 'ms'),
  (6986, 1, 'te', 'mt'),
  (6987, 1, 'te', 'my'),
  (6988, 1, 'te', 'ne'),
  (6989, 1, 'te', 'nl'),
  (6990, 1, 'te', 'no'),
  (6991, 1, 'te', 'ny'),
  (6992, 1, 'te', 'pa'),
  (6993, 1, 'te', 'pl'),
  (6994, 1, 'te', 'pt'),
  (6995, 1, 'te', 'ro'),
  (6996, 1, 'te', 'ru'),
  (6997, 1, 'te', 'si'),
  (6998, 1, 'te', 'sk'),
  (6999, 1, 'te', 'sl'),
  (7000, 1, 'te', 'so'),
  (7001, 1, 'te', 'sq'),
  (7002, 1, 'te', 'sr'),
  (7003, 1, 'te', 'st'),
  (7004, 1, 'te', 'su'),
  (7005, 1, 'te', 'sv'),
  (7006, 1, 'te', 'sw'),
  (7007, 1, 'te', 'ta'),
  (7008, 1, 'te', 'tg'),
  (7009, 1, 'te', 'th'),
  (7010, 1, 'te', 'tl'),
  (7011, 1, 'te', 'tr'),
  (7012, 1, 'te', 'uk'),
  (7013, 1, 'te', 'ur'),
  (7014, 1, 'te', 'uz'),
  (7015, 1, 'te', 'vi'),
  (7016, 1, 'te', 'yi'),
  (7017, 1, 'te', 'yo'),
  (7018, 1, 'te', 'zh'),
  (7019, 1, 'te', 'zh-TW'),
  (7020, 1, 'te', 'zu'),
  (7021, 1, 'tg', 'af'),
  (7022, 1, 'tg', 'ar'),
  (7023, 1, 'tg', 'az'),
  (7024, 1, 'tg', 'be'),
  (7025, 1, 'tg', 'bg'),
  (7026, 1, 'tg', 'bn'),
  (7027, 1, 'tg', 'bs'),
  (7028, 1, 'tg', 'ca'),
  (7029, 1, 'tg', 'ceb'),
  (7030, 1, 'tg', 'cs'),
  (7031, 1, 'tg', 'cy'),
  (7032, 1, 'tg', 'da'),
  (7033, 1, 'tg', 'de'),
  (7034, 1, 'tg', 'el'),
  (7035, 1, 'tg', 'en'),
  (7036, 1, 'tg', 'eo'),
  (7037, 1, 'tg', 'es'),
  (7038, 1, 'tg', 'et'),
  (7039, 1, 'tg', 'eu'),
  (7040, 1, 'tg', 'fa'),
  (7041, 1, 'tg', 'fi'),
  (7042, 1, 'tg', 'fr'),
  (7043, 1, 'tg', 'ga'),
  (7044, 1, 'tg', 'gl'),
  (7045, 1, 'tg', 'gu'),
  (7046, 1, 'tg', 'ha'),
  (7047, 1, 'tg', 'hi'),
  (7048, 1, 'tg', 'hmn'),
  (7049, 1, 'tg', 'hr'),
  (7050, 1, 'tg', 'ht'),
  (7051, 1, 'tg', 'hu'),
  (7052, 1, 'tg', 'hy'),
  (7053, 1, 'tg', 'id'),
  (7054, 1, 'tg', 'ig'),
  (7055, 1, 'tg', 'is'),
  (7056, 1, 'tg', 'it'),
  (7057, 1, 'tg', 'iw'),
  (7058, 1, 'tg', 'ja'),
  (7059, 1, 'tg', 'jw'),
  (7060, 1, 'tg', 'ka'),
  (7061, 1, 'tg', 'kk'),
  (7062, 1, 'tg', 'km'),
  (7063, 1, 'tg', 'kn'),
  (7064, 1, 'tg', 'ko'),
  (7065, 1, 'tg', 'la'),
  (7066, 1, 'tg', 'lo'),
  (7067, 1, 'tg', 'lt'),
  (7068, 1, 'tg', 'lv'),
  (7069, 1, 'tg', 'mg'),
  (7070, 1, 'tg', 'mi'),
  (7071, 1, 'tg', 'mk'),
  (7072, 1, 'tg', 'ml'),
  (7073, 1, 'tg', 'mn'),
  (7074, 1, 'tg', 'mr'),
  (7075, 1, 'tg', 'ms'),
  (7076, 1, 'tg', 'mt'),
  (7077, 1, 'tg', 'my'),
  (7078, 1, 'tg', 'ne'),
  (7079, 1, 'tg', 'nl'),
  (7080, 1, 'tg', 'no'),
  (7081, 1, 'tg', 'ny'),
  (7082, 1, 'tg', 'pa'),
  (7083, 1, 'tg', 'pl'),
  (7084, 1, 'tg', 'pt'),
  (7085, 1, 'tg', 'ro'),
  (7086, 1, 'tg', 'ru'),
  (7087, 1, 'tg', 'si'),
  (7088, 1, 'tg', 'sk'),
  (7089, 1, 'tg', 'sl'),
  (7090, 1, 'tg', 'so'),
  (7091, 1, 'tg', 'sq'),
  (7092, 1, 'tg', 'sr'),
  (7093, 1, 'tg', 'st'),
  (7094, 1, 'tg', 'su'),
  (7095, 1, 'tg', 'sv'),
  (7096, 1, 'tg', 'sw'),
  (7097, 1, 'tg', 'ta'),
  (7098, 1, 'tg', 'te'),
  (7099, 1, 'tg', 'th'),
  (7100, 1, 'tg', 'tl'),
  (7101, 1, 'tg', 'tr'),
  (7102, 1, 'tg', 'uk'),
  (7103, 1, 'tg', 'ur'),
  (7104, 1, 'tg', 'uz'),
  (7105, 1, 'tg', 'vi'),
  (7106, 1, 'tg', 'yi'),
  (7107, 1, 'tg', 'yo'),
  (7108, 1, 'tg', 'zh'),
  (7109, 1, 'tg', 'zh-TW'),
  (7110, 1, 'tg', 'zu'),
  (7111, 1, 'th', 'af'),
  (7112, 1, 'th', 'ar'),
  (7113, 1, 'th', 'az'),
  (7114, 1, 'th', 'be'),
  (7115, 1, 'th', 'bg'),
  (7116, 1, 'th', 'bn'),
  (7117, 1, 'th', 'bs'),
  (7118, 1, 'th', 'ca'),
  (7119, 1, 'th', 'ceb'),
  (7120, 1, 'th', 'cs'),
  (7121, 1, 'th', 'cy'),
  (7122, 1, 'th', 'da'),
  (7123, 1, 'th', 'de'),
  (7124, 1, 'th', 'el'),
  (7125, 1, 'th', 'en'),
  (7126, 1, 'th', 'eo'),
  (7127, 1, 'th', 'es'),
  (7128, 1, 'th', 'et'),
  (7129, 1, 'th', 'eu'),
  (7130, 1, 'th', 'fa'),
  (7131, 1, 'th', 'fi'),
  (7132, 1, 'th', 'fr'),
  (7133, 1, 'th', 'ga'),
  (7134, 1, 'th', 'gl'),
  (7135, 1, 'th', 'gu'),
  (7136, 1, 'th', 'ha'),
  (7137, 1, 'th', 'hi'),
  (7138, 1, 'th', 'hmn'),
  (7139, 1, 'th', 'hr'),
  (7140, 1, 'th', 'ht'),
  (7141, 1, 'th', 'hu'),
  (7142, 1, 'th', 'hy'),
  (7143, 1, 'th', 'id'),
  (7144, 1, 'th', 'ig'),
  (7145, 1, 'th', 'is'),
  (7146, 1, 'th', 'it'),
  (7147, 1, 'th', 'iw');
INSERT INTO `#__neno_translation_methods_language_pairs` (`id`, `translation_method_id`, `source_language`, `destination_language`) VALUES
  (7148, 1, 'th', 'ja'),
  (7149, 1, 'th', 'jw'),
  (7150, 1, 'th', 'ka'),
  (7151, 1, 'th', 'kk'),
  (7152, 1, 'th', 'km'),
  (7153, 1, 'th', 'kn'),
  (7154, 1, 'th', 'ko'),
  (7155, 1, 'th', 'la'),
  (7156, 1, 'th', 'lo'),
  (7157, 1, 'th', 'lt'),
  (7158, 1, 'th', 'lv'),
  (7159, 1, 'th', 'mg'),
  (7160, 1, 'th', 'mi'),
  (7161, 1, 'th', 'mk'),
  (7162, 1, 'th', 'ml'),
  (7163, 1, 'th', 'mn'),
  (7164, 1, 'th', 'mr'),
  (7165, 1, 'th', 'ms'),
  (7166, 1, 'th', 'mt'),
  (7167, 1, 'th', 'my'),
  (7168, 1, 'th', 'ne'),
  (7169, 1, 'th', 'nl'),
  (7170, 1, 'th', 'no'),
  (7171, 1, 'th', 'ny'),
  (7172, 1, 'th', 'pa'),
  (7173, 1, 'th', 'pl'),
  (7174, 1, 'th', 'pt'),
  (7175, 1, 'th', 'ro'),
  (7176, 1, 'th', 'ru'),
  (7177, 1, 'th', 'si'),
  (7178, 1, 'th', 'sk'),
  (7179, 1, 'th', 'sl'),
  (7180, 1, 'th', 'so'),
  (7181, 1, 'th', 'sq'),
  (7182, 1, 'th', 'sr'),
  (7183, 1, 'th', 'st'),
  (7184, 1, 'th', 'su'),
  (7185, 1, 'th', 'sv'),
  (7186, 1, 'th', 'sw'),
  (7187, 1, 'th', 'ta'),
  (7188, 1, 'th', 'te'),
  (7189, 1, 'th', 'tg'),
  (7190, 1, 'th', 'tl'),
  (7191, 1, 'th', 'tr'),
  (7192, 1, 'th', 'uk'),
  (7193, 1, 'th', 'ur'),
  (7194, 1, 'th', 'uz'),
  (7195, 1, 'th', 'vi'),
  (7196, 1, 'th', 'yi'),
  (7197, 1, 'th', 'yo'),
  (7198, 1, 'th', 'zh'),
  (7199, 1, 'th', 'zh-TW'),
  (7200, 1, 'th', 'zu'),
  (7201, 1, 'tl', 'af'),
  (7202, 1, 'tl', 'ar'),
  (7203, 1, 'tl', 'az'),
  (7204, 1, 'tl', 'be'),
  (7205, 1, 'tl', 'bg'),
  (7206, 1, 'tl', 'bn'),
  (7207, 1, 'tl', 'bs'),
  (7208, 1, 'tl', 'ca'),
  (7209, 1, 'tl', 'ceb'),
  (7210, 1, 'tl', 'cs'),
  (7211, 1, 'tl', 'cy'),
  (7212, 1, 'tl', 'da'),
  (7213, 1, 'tl', 'de'),
  (7214, 1, 'tl', 'el'),
  (7215, 1, 'tl', 'en'),
  (7216, 1, 'tl', 'eo'),
  (7217, 1, 'tl', 'es'),
  (7218, 1, 'tl', 'et'),
  (7219, 1, 'tl', 'eu'),
  (7220, 1, 'tl', 'fa'),
  (7221, 1, 'tl', 'fi'),
  (7222, 1, 'tl', 'fr'),
  (7223, 1, 'tl', 'ga'),
  (7224, 1, 'tl', 'gl'),
  (7225, 1, 'tl', 'gu'),
  (7226, 1, 'tl', 'ha'),
  (7227, 1, 'tl', 'hi'),
  (7228, 1, 'tl', 'hmn'),
  (7229, 1, 'tl', 'hr'),
  (7230, 1, 'tl', 'ht'),
  (7231, 1, 'tl', 'hu'),
  (7232, 1, 'tl', 'hy'),
  (7233, 1, 'tl', 'id'),
  (7234, 1, 'tl', 'ig'),
  (7235, 1, 'tl', 'is'),
  (7236, 1, 'tl', 'it'),
  (7237, 1, 'tl', 'iw'),
  (7238, 1, 'tl', 'ja'),
  (7239, 1, 'tl', 'jw'),
  (7240, 1, 'tl', 'ka'),
  (7241, 1, 'tl', 'kk'),
  (7242, 1, 'tl', 'km'),
  (7243, 1, 'tl', 'kn'),
  (7244, 1, 'tl', 'ko'),
  (7245, 1, 'tl', 'la'),
  (7246, 1, 'tl', 'lo'),
  (7247, 1, 'tl', 'lt'),
  (7248, 1, 'tl', 'lv'),
  (7249, 1, 'tl', 'mg'),
  (7250, 1, 'tl', 'mi'),
  (7251, 1, 'tl', 'mk'),
  (7252, 1, 'tl', 'ml'),
  (7253, 1, 'tl', 'mn'),
  (7254, 1, 'tl', 'mr'),
  (7255, 1, 'tl', 'ms'),
  (7256, 1, 'tl', 'mt'),
  (7257, 1, 'tl', 'my'),
  (7258, 1, 'tl', 'ne'),
  (7259, 1, 'tl', 'nl'),
  (7260, 1, 'tl', 'no'),
  (7261, 1, 'tl', 'ny'),
  (7262, 1, 'tl', 'pa'),
  (7263, 1, 'tl', 'pl'),
  (7264, 1, 'tl', 'pt'),
  (7265, 1, 'tl', 'ro'),
  (7266, 1, 'tl', 'ru'),
  (7267, 1, 'tl', 'si'),
  (7268, 1, 'tl', 'sk'),
  (7269, 1, 'tl', 'sl'),
  (7270, 1, 'tl', 'so'),
  (7271, 1, 'tl', 'sq'),
  (7272, 1, 'tl', 'sr'),
  (7273, 1, 'tl', 'st'),
  (7274, 1, 'tl', 'su'),
  (7275, 1, 'tl', 'sv'),
  (7276, 1, 'tl', 'sw'),
  (7277, 1, 'tl', 'ta'),
  (7278, 1, 'tl', 'te'),
  (7279, 1, 'tl', 'tg'),
  (7280, 1, 'tl', 'th'),
  (7281, 1, 'tl', 'tr'),
  (7282, 1, 'tl', 'uk'),
  (7283, 1, 'tl', 'ur'),
  (7284, 1, 'tl', 'uz'),
  (7285, 1, 'tl', 'vi'),
  (7286, 1, 'tl', 'yi'),
  (7287, 1, 'tl', 'yo'),
  (7288, 1, 'tl', 'zh'),
  (7289, 1, 'tl', 'zh-TW'),
  (7290, 1, 'tl', 'zu'),
  (7291, 1, 'tr', 'af'),
  (7292, 1, 'tr', 'ar'),
  (7293, 1, 'tr', 'az'),
  (7294, 1, 'tr', 'be'),
  (7295, 1, 'tr', 'bg'),
  (7296, 1, 'tr', 'bn'),
  (7297, 1, 'tr', 'bs'),
  (7298, 1, 'tr', 'ca'),
  (7299, 1, 'tr', 'ceb'),
  (7300, 1, 'tr', 'cs'),
  (7301, 1, 'tr', 'cy'),
  (7302, 1, 'tr', 'da'),
  (7303, 1, 'tr', 'de'),
  (7304, 1, 'tr', 'el'),
  (7305, 1, 'tr', 'en'),
  (7306, 1, 'tr', 'eo'),
  (7307, 1, 'tr', 'es'),
  (7308, 1, 'tr', 'et'),
  (7309, 1, 'tr', 'eu'),
  (7310, 1, 'tr', 'fa'),
  (7311, 1, 'tr', 'fi'),
  (7312, 1, 'tr', 'fr'),
  (7313, 1, 'tr', 'ga'),
  (7314, 1, 'tr', 'gl'),
  (7315, 1, 'tr', 'gu'),
  (7316, 1, 'tr', 'ha'),
  (7317, 1, 'tr', 'hi'),
  (7318, 1, 'tr', 'hmn'),
  (7319, 1, 'tr', 'hr'),
  (7320, 1, 'tr', 'ht'),
  (7321, 1, 'tr', 'hu'),
  (7322, 1, 'tr', 'hy'),
  (7323, 1, 'tr', 'id'),
  (7324, 1, 'tr', 'ig'),
  (7325, 1, 'tr', 'is'),
  (7326, 1, 'tr', 'it'),
  (7327, 1, 'tr', 'iw'),
  (7328, 1, 'tr', 'ja'),
  (7329, 1, 'tr', 'jw'),
  (7330, 1, 'tr', 'ka'),
  (7331, 1, 'tr', 'kk'),
  (7332, 1, 'tr', 'km'),
  (7333, 1, 'tr', 'kn'),
  (7334, 1, 'tr', 'ko'),
  (7335, 1, 'tr', 'la'),
  (7336, 1, 'tr', 'lo'),
  (7337, 1, 'tr', 'lt'),
  (7338, 1, 'tr', 'lv'),
  (7339, 1, 'tr', 'mg'),
  (7340, 1, 'tr', 'mi'),
  (7341, 1, 'tr', 'mk'),
  (7342, 1, 'tr', 'ml'),
  (7343, 1, 'tr', 'mn'),
  (7344, 1, 'tr', 'mr'),
  (7345, 1, 'tr', 'ms'),
  (7346, 1, 'tr', 'mt'),
  (7347, 1, 'tr', 'my'),
  (7348, 1, 'tr', 'ne'),
  (7349, 1, 'tr', 'nl'),
  (7350, 1, 'tr', 'no'),
  (7351, 1, 'tr', 'ny'),
  (7352, 1, 'tr', 'pa'),
  (7353, 1, 'tr', 'pl'),
  (7354, 1, 'tr', 'pt'),
  (7355, 1, 'tr', 'ro'),
  (7356, 1, 'tr', 'ru'),
  (7357, 1, 'tr', 'si'),
  (7358, 1, 'tr', 'sk'),
  (7359, 1, 'tr', 'sl'),
  (7360, 1, 'tr', 'so'),
  (7361, 1, 'tr', 'sq'),
  (7362, 1, 'tr', 'sr'),
  (7363, 1, 'tr', 'st'),
  (7364, 1, 'tr', 'su'),
  (7365, 1, 'tr', 'sv'),
  (7366, 1, 'tr', 'sw'),
  (7367, 1, 'tr', 'ta'),
  (7368, 1, 'tr', 'te'),
  (7369, 1, 'tr', 'tg'),
  (7370, 1, 'tr', 'th'),
  (7371, 1, 'tr', 'tl'),
  (7372, 1, 'tr', 'uk'),
  (7373, 1, 'tr', 'ur'),
  (7374, 1, 'tr', 'uz'),
  (7375, 1, 'tr', 'vi'),
  (7376, 1, 'tr', 'yi'),
  (7377, 1, 'tr', 'yo'),
  (7378, 1, 'tr', 'zh'),
  (7379, 1, 'tr', 'zh-TW'),
  (7380, 1, 'tr', 'zu'),
  (7381, 1, 'uk', 'af'),
  (7382, 1, 'uk', 'ar'),
  (7383, 1, 'uk', 'az'),
  (7384, 1, 'uk', 'be'),
  (7385, 1, 'uk', 'bg'),
  (7386, 1, 'uk', 'bn'),
  (7387, 1, 'uk', 'bs'),
  (7388, 1, 'uk', 'ca'),
  (7389, 1, 'uk', 'ceb'),
  (7390, 1, 'uk', 'cs'),
  (7391, 1, 'uk', 'cy'),
  (7392, 1, 'uk', 'da'),
  (7393, 1, 'uk', 'de'),
  (7394, 1, 'uk', 'el'),
  (7395, 1, 'uk', 'en'),
  (7396, 1, 'uk', 'eo'),
  (7397, 1, 'uk', 'es'),
  (7398, 1, 'uk', 'et'),
  (7399, 1, 'uk', 'eu'),
  (7400, 1, 'uk', 'fa'),
  (7401, 1, 'uk', 'fi'),
  (7402, 1, 'uk', 'fr'),
  (7403, 1, 'uk', 'ga'),
  (7404, 1, 'uk', 'gl'),
  (7405, 1, 'uk', 'gu'),
  (7406, 1, 'uk', 'ha'),
  (7407, 1, 'uk', 'hi'),
  (7408, 1, 'uk', 'hmn'),
  (7409, 1, 'uk', 'hr'),
  (7410, 1, 'uk', 'ht'),
  (7411, 1, 'uk', 'hu'),
  (7412, 1, 'uk', 'hy'),
  (7413, 1, 'uk', 'id'),
  (7414, 1, 'uk', 'ig'),
  (7415, 1, 'uk', 'is'),
  (7416, 1, 'uk', 'it'),
  (7417, 1, 'uk', 'iw'),
  (7418, 1, 'uk', 'ja'),
  (7419, 1, 'uk', 'jw'),
  (7420, 1, 'uk', 'ka'),
  (7421, 1, 'uk', 'kk'),
  (7422, 1, 'uk', 'km'),
  (7423, 1, 'uk', 'kn'),
  (7424, 1, 'uk', 'ko'),
  (7425, 1, 'uk', 'la'),
  (7426, 1, 'uk', 'lo'),
  (7427, 1, 'uk', 'lt'),
  (7428, 1, 'uk', 'lv'),
  (7429, 1, 'uk', 'mg'),
  (7430, 1, 'uk', 'mi'),
  (7431, 1, 'uk', 'mk'),
  (7432, 1, 'uk', 'ml'),
  (7433, 1, 'uk', 'mn'),
  (7434, 1, 'uk', 'mr'),
  (7435, 1, 'uk', 'ms'),
  (7436, 1, 'uk', 'mt'),
  (7437, 1, 'uk', 'my'),
  (7438, 1, 'uk', 'ne'),
  (7439, 1, 'uk', 'nl'),
  (7440, 1, 'uk', 'no'),
  (7441, 1, 'uk', 'ny'),
  (7442, 1, 'uk', 'pa'),
  (7443, 1, 'uk', 'pl'),
  (7444, 1, 'uk', 'pt'),
  (7445, 1, 'uk', 'ro'),
  (7446, 1, 'uk', 'ru'),
  (7447, 1, 'uk', 'si'),
  (7448, 1, 'uk', 'sk'),
  (7449, 1, 'uk', 'sl'),
  (7450, 1, 'uk', 'so'),
  (7451, 1, 'uk', 'sq'),
  (7452, 1, 'uk', 'sr'),
  (7453, 1, 'uk', 'st'),
  (7454, 1, 'uk', 'su'),
  (7455, 1, 'uk', 'sv'),
  (7456, 1, 'uk', 'sw'),
  (7457, 1, 'uk', 'ta'),
  (7458, 1, 'uk', 'te'),
  (7459, 1, 'uk', 'tg'),
  (7460, 1, 'uk', 'th'),
  (7461, 1, 'uk', 'tl'),
  (7462, 1, 'uk', 'tr'),
  (7463, 1, 'uk', 'ur'),
  (7464, 1, 'uk', 'uz'),
  (7465, 1, 'uk', 'vi'),
  (7466, 1, 'uk', 'yi'),
  (7467, 1, 'uk', 'yo'),
  (7468, 1, 'uk', 'zh'),
  (7469, 1, 'uk', 'zh-TW'),
  (7470, 1, 'uk', 'zu'),
  (7471, 1, 'ur', 'af'),
  (7472, 1, 'ur', 'ar'),
  (7473, 1, 'ur', 'az'),
  (7474, 1, 'ur', 'be'),
  (7475, 1, 'ur', 'bg'),
  (7476, 1, 'ur', 'bn'),
  (7477, 1, 'ur', 'bs'),
  (7478, 1, 'ur', 'ca'),
  (7479, 1, 'ur', 'ceb'),
  (7480, 1, 'ur', 'cs'),
  (7481, 1, 'ur', 'cy'),
  (7482, 1, 'ur', 'da'),
  (7483, 1, 'ur', 'de'),
  (7484, 1, 'ur', 'el'),
  (7485, 1, 'ur', 'en'),
  (7486, 1, 'ur', 'eo'),
  (7487, 1, 'ur', 'es'),
  (7488, 1, 'ur', 'et'),
  (7489, 1, 'ur', 'eu'),
  (7490, 1, 'ur', 'fa'),
  (7491, 1, 'ur', 'fi'),
  (7492, 1, 'ur', 'fr'),
  (7493, 1, 'ur', 'ga'),
  (7494, 1, 'ur', 'gl'),
  (7495, 1, 'ur', 'gu'),
  (7496, 1, 'ur', 'ha'),
  (7497, 1, 'ur', 'hi'),
  (7498, 1, 'ur', 'hmn'),
  (7499, 1, 'ur', 'hr'),
  (7500, 1, 'ur', 'ht'),
  (7501, 1, 'ur', 'hu'),
  (7502, 1, 'ur', 'hy'),
  (7503, 1, 'ur', 'id'),
  (7504, 1, 'ur', 'ig'),
  (7505, 1, 'ur', 'is'),
  (7506, 1, 'ur', 'it'),
  (7507, 1, 'ur', 'iw'),
  (7508, 1, 'ur', 'ja'),
  (7509, 1, 'ur', 'jw'),
  (7510, 1, 'ur', 'ka'),
  (7511, 1, 'ur', 'kk'),
  (7512, 1, 'ur', 'km'),
  (7513, 1, 'ur', 'kn'),
  (7514, 1, 'ur', 'ko'),
  (7515, 1, 'ur', 'la'),
  (7516, 1, 'ur', 'lo'),
  (7517, 1, 'ur', 'lt'),
  (7518, 1, 'ur', 'lv'),
  (7519, 1, 'ur', 'mg'),
  (7520, 1, 'ur', 'mi'),
  (7521, 1, 'ur', 'mk'),
  (7522, 1, 'ur', 'ml'),
  (7523, 1, 'ur', 'mn'),
  (7524, 1, 'ur', 'mr'),
  (7525, 1, 'ur', 'ms'),
  (7526, 1, 'ur', 'mt'),
  (7527, 1, 'ur', 'my'),
  (7528, 1, 'ur', 'ne'),
  (7529, 1, 'ur', 'nl'),
  (7530, 1, 'ur', 'no'),
  (7531, 1, 'ur', 'ny'),
  (7532, 1, 'ur', 'pa'),
  (7533, 1, 'ur', 'pl'),
  (7534, 1, 'ur', 'pt'),
  (7535, 1, 'ur', 'ro'),
  (7536, 1, 'ur', 'ru'),
  (7537, 1, 'ur', 'si'),
  (7538, 1, 'ur', 'sk'),
  (7539, 1, 'ur', 'sl'),
  (7540, 1, 'ur', 'so'),
  (7541, 1, 'ur', 'sq'),
  (7542, 1, 'ur', 'sr'),
  (7543, 1, 'ur', 'st'),
  (7544, 1, 'ur', 'su'),
  (7545, 1, 'ur', 'sv'),
  (7546, 1, 'ur', 'sw'),
  (7547, 1, 'ur', 'ta'),
  (7548, 1, 'ur', 'te'),
  (7549, 1, 'ur', 'tg'),
  (7550, 1, 'ur', 'th'),
  (7551, 1, 'ur', 'tl'),
  (7552, 1, 'ur', 'tr'),
  (7553, 1, 'ur', 'uk'),
  (7554, 1, 'ur', 'uz'),
  (7555, 1, 'ur', 'vi'),
  (7556, 1, 'ur', 'yi'),
  (7557, 1, 'ur', 'yo'),
  (7558, 1, 'ur', 'zh'),
  (7559, 1, 'ur', 'zh-TW'),
  (7560, 1, 'ur', 'zu'),
  (7561, 1, 'uz', 'af'),
  (7562, 1, 'uz', 'ar'),
  (7563, 1, 'uz', 'az'),
  (7564, 1, 'uz', 'be'),
  (7565, 1, 'uz', 'bg'),
  (7566, 1, 'uz', 'bn'),
  (7567, 1, 'uz', 'bs'),
  (7568, 1, 'uz', 'ca'),
  (7569, 1, 'uz', 'ceb'),
  (7570, 1, 'uz', 'cs'),
  (7571, 1, 'uz', 'cy'),
  (7572, 1, 'uz', 'da'),
  (7573, 1, 'uz', 'de'),
  (7574, 1, 'uz', 'el'),
  (7575, 1, 'uz', 'en'),
  (7576, 1, 'uz', 'eo'),
  (7577, 1, 'uz', 'es'),
  (7578, 1, 'uz', 'et'),
  (7579, 1, 'uz', 'eu'),
  (7580, 1, 'uz', 'fa'),
  (7581, 1, 'uz', 'fi'),
  (7582, 1, 'uz', 'fr'),
  (7583, 1, 'uz', 'ga'),
  (7584, 1, 'uz', 'gl'),
  (7585, 1, 'uz', 'gu'),
  (7586, 1, 'uz', 'ha'),
  (7587, 1, 'uz', 'hi'),
  (7588, 1, 'uz', 'hmn'),
  (7589, 1, 'uz', 'hr'),
  (7590, 1, 'uz', 'ht'),
  (7591, 1, 'uz', 'hu'),
  (7592, 1, 'uz', 'hy'),
  (7593, 1, 'uz', 'id'),
  (7594, 1, 'uz', 'ig'),
  (7595, 1, 'uz', 'is'),
  (7596, 1, 'uz', 'it'),
  (7597, 1, 'uz', 'iw'),
  (7598, 1, 'uz', 'ja'),
  (7599, 1, 'uz', 'jw'),
  (7600, 1, 'uz', 'ka'),
  (7601, 1, 'uz', 'kk'),
  (7602, 1, 'uz', 'km'),
  (7603, 1, 'uz', 'kn'),
  (7604, 1, 'uz', 'ko'),
  (7605, 1, 'uz', 'la'),
  (7606, 1, 'uz', 'lo'),
  (7607, 1, 'uz', 'lt'),
  (7608, 1, 'uz', 'lv'),
  (7609, 1, 'uz', 'mg'),
  (7610, 1, 'uz', 'mi'),
  (7611, 1, 'uz', 'mk'),
  (7612, 1, 'uz', 'ml'),
  (7613, 1, 'uz', 'mn'),
  (7614, 1, 'uz', 'mr'),
  (7615, 1, 'uz', 'ms'),
  (7616, 1, 'uz', 'mt'),
  (7617, 1, 'uz', 'my'),
  (7618, 1, 'uz', 'ne'),
  (7619, 1, 'uz', 'nl'),
  (7620, 1, 'uz', 'no'),
  (7621, 1, 'uz', 'ny'),
  (7622, 1, 'uz', 'pa'),
  (7623, 1, 'uz', 'pl'),
  (7624, 1, 'uz', 'pt'),
  (7625, 1, 'uz', 'ro'),
  (7626, 1, 'uz', 'ru'),
  (7627, 1, 'uz', 'si'),
  (7628, 1, 'uz', 'sk'),
  (7629, 1, 'uz', 'sl'),
  (7630, 1, 'uz', 'so'),
  (7631, 1, 'uz', 'sq'),
  (7632, 1, 'uz', 'sr'),
  (7633, 1, 'uz', 'st'),
  (7634, 1, 'uz', 'su'),
  (7635, 1, 'uz', 'sv'),
  (7636, 1, 'uz', 'sw'),
  (7637, 1, 'uz', 'ta'),
  (7638, 1, 'uz', 'te'),
  (7639, 1, 'uz', 'tg'),
  (7640, 1, 'uz', 'th'),
  (7641, 1, 'uz', 'tl'),
  (7642, 1, 'uz', 'tr'),
  (7643, 1, 'uz', 'uk'),
  (7644, 1, 'uz', 'ur'),
  (7645, 1, 'uz', 'vi'),
  (7646, 1, 'uz', 'yi'),
  (7647, 1, 'uz', 'yo'),
  (7648, 1, 'uz', 'zh'),
  (7649, 1, 'uz', 'zh-TW'),
  (7650, 1, 'uz', 'zu'),
  (7651, 1, 'vi', 'af'),
  (7652, 1, 'vi', 'ar'),
  (7653, 1, 'vi', 'az'),
  (7654, 1, 'vi', 'be'),
  (7655, 1, 'vi', 'bg'),
  (7656, 1, 'vi', 'bn'),
  (7657, 1, 'vi', 'bs'),
  (7658, 1, 'vi', 'ca'),
  (7659, 1, 'vi', 'ceb'),
  (7660, 1, 'vi', 'cs'),
  (7661, 1, 'vi', 'cy'),
  (7662, 1, 'vi', 'da'),
  (7663, 1, 'vi', 'de'),
  (7664, 1, 'vi', 'el'),
  (7665, 1, 'vi', 'en'),
  (7666, 1, 'vi', 'eo'),
  (7667, 1, 'vi', 'es'),
  (7668, 1, 'vi', 'et'),
  (7669, 1, 'vi', 'eu'),
  (7670, 1, 'vi', 'fa'),
  (7671, 1, 'vi', 'fi'),
  (7672, 1, 'vi', 'fr'),
  (7673, 1, 'vi', 'ga'),
  (7674, 1, 'vi', 'gl'),
  (7675, 1, 'vi', 'gu'),
  (7676, 1, 'vi', 'ha'),
  (7677, 1, 'vi', 'hi'),
  (7678, 1, 'vi', 'hmn'),
  (7679, 1, 'vi', 'hr'),
  (7680, 1, 'vi', 'ht'),
  (7681, 1, 'vi', 'hu'),
  (7682, 1, 'vi', 'hy'),
  (7683, 1, 'vi', 'id'),
  (7684, 1, 'vi', 'ig'),
  (7685, 1, 'vi', 'is'),
  (7686, 1, 'vi', 'it'),
  (7687, 1, 'vi', 'iw'),
  (7688, 1, 'vi', 'ja'),
  (7689, 1, 'vi', 'jw'),
  (7690, 1, 'vi', 'ka'),
  (7691, 1, 'vi', 'kk'),
  (7692, 1, 'vi', 'km'),
  (7693, 1, 'vi', 'kn'),
  (7694, 1, 'vi', 'ko'),
  (7695, 1, 'vi', 'la'),
  (7696, 1, 'vi', 'lo'),
  (7697, 1, 'vi', 'lt'),
  (7698, 1, 'vi', 'lv'),
  (7699, 1, 'vi', 'mg'),
  (7700, 1, 'vi', 'mi'),
  (7701, 1, 'vi', 'mk'),
  (7702, 1, 'vi', 'ml'),
  (7703, 1, 'vi', 'mn'),
  (7704, 1, 'vi', 'mr'),
  (7705, 1, 'vi', 'ms'),
  (7706, 1, 'vi', 'mt'),
  (7707, 1, 'vi', 'my'),
  (7708, 1, 'vi', 'ne'),
  (7709, 1, 'vi', 'nl'),
  (7710, 1, 'vi', 'no'),
  (7711, 1, 'vi', 'ny'),
  (7712, 1, 'vi', 'pa'),
  (7713, 1, 'vi', 'pl'),
  (7714, 1, 'vi', 'pt'),
  (7715, 1, 'vi', 'ro'),
  (7716, 1, 'vi', 'ru'),
  (7717, 1, 'vi', 'si'),
  (7718, 1, 'vi', 'sk'),
  (7719, 1, 'vi', 'sl'),
  (7720, 1, 'vi', 'so'),
  (7721, 1, 'vi', 'sq'),
  (7722, 1, 'vi', 'sr'),
  (7723, 1, 'vi', 'st'),
  (7724, 1, 'vi', 'su'),
  (7725, 1, 'vi', 'sv'),
  (7726, 1, 'vi', 'sw'),
  (7727, 1, 'vi', 'ta'),
  (7728, 1, 'vi', 'te'),
  (7729, 1, 'vi', 'tg'),
  (7730, 1, 'vi', 'th'),
  (7731, 1, 'vi', 'tl'),
  (7732, 1, 'vi', 'tr'),
  (7733, 1, 'vi', 'uk'),
  (7734, 1, 'vi', 'ur'),
  (7735, 1, 'vi', 'uz'),
  (7736, 1, 'vi', 'yi'),
  (7737, 1, 'vi', 'yo'),
  (7738, 1, 'vi', 'zh'),
  (7739, 1, 'vi', 'zh-TW'),
  (7740, 1, 'vi', 'zu'),
  (7741, 1, 'yi', 'af'),
  (7742, 1, 'yi', 'ar'),
  (7743, 1, 'yi', 'az'),
  (7744, 1, 'yi', 'be'),
  (7745, 1, 'yi', 'bg'),
  (7746, 1, 'yi', 'bn'),
  (7747, 1, 'yi', 'bs'),
  (7748, 1, 'yi', 'ca'),
  (7749, 1, 'yi', 'ceb'),
  (7750, 1, 'yi', 'cs'),
  (7751, 1, 'yi', 'cy'),
  (7752, 1, 'yi', 'da'),
  (7753, 1, 'yi', 'de'),
  (7754, 1, 'yi', 'el'),
  (7755, 1, 'yi', 'en'),
  (7756, 1, 'yi', 'eo'),
  (7757, 1, 'yi', 'es'),
  (7758, 1, 'yi', 'et'),
  (7759, 1, 'yi', 'eu'),
  (7760, 1, 'yi', 'fa'),
  (7761, 1, 'yi', 'fi'),
  (7762, 1, 'yi', 'fr'),
  (7763, 1, 'yi', 'ga'),
  (7764, 1, 'yi', 'gl'),
  (7765, 1, 'yi', 'gu'),
  (7766, 1, 'yi', 'ha'),
  (7767, 1, 'yi', 'hi'),
  (7768, 1, 'yi', 'hmn'),
  (7769, 1, 'yi', 'hr'),
  (7770, 1, 'yi', 'ht'),
  (7771, 1, 'yi', 'hu'),
  (7772, 1, 'yi', 'hy'),
  (7773, 1, 'yi', 'id'),
  (7774, 1, 'yi', 'ig'),
  (7775, 1, 'yi', 'is'),
  (7776, 1, 'yi', 'it'),
  (7777, 1, 'yi', 'iw'),
  (7778, 1, 'yi', 'ja'),
  (7779, 1, 'yi', 'jw'),
  (7780, 1, 'yi', 'ka'),
  (7781, 1, 'yi', 'kk'),
  (7782, 1, 'yi', 'km'),
  (7783, 1, 'yi', 'kn'),
  (7784, 1, 'yi', 'ko'),
  (7785, 1, 'yi', 'la'),
  (7786, 1, 'yi', 'lo'),
  (7787, 1, 'yi', 'lt'),
  (7788, 1, 'yi', 'lv'),
  (7789, 1, 'yi', 'mg'),
  (7790, 1, 'yi', 'mi'),
  (7791, 1, 'yi', 'mk'),
  (7792, 1, 'yi', 'ml'),
  (7793, 1, 'yi', 'mn'),
  (7794, 1, 'yi', 'mr'),
  (7795, 1, 'yi', 'ms'),
  (7796, 1, 'yi', 'mt'),
  (7797, 1, 'yi', 'my'),
  (7798, 1, 'yi', 'ne'),
  (7799, 1, 'yi', 'nl'),
  (7800, 1, 'yi', 'no'),
  (7801, 1, 'yi', 'ny'),
  (7802, 1, 'yi', 'pa'),
  (7803, 1, 'yi', 'pl'),
  (7804, 1, 'yi', 'pt'),
  (7805, 1, 'yi', 'ro'),
  (7806, 1, 'yi', 'ru'),
  (7807, 1, 'yi', 'si'),
  (7808, 1, 'yi', 'sk'),
  (7809, 1, 'yi', 'sl'),
  (7810, 1, 'yi', 'so'),
  (7811, 1, 'yi', 'sq'),
  (7812, 1, 'yi', 'sr'),
  (7813, 1, 'yi', 'st'),
  (7814, 1, 'yi', 'su'),
  (7815, 1, 'yi', 'sv'),
  (7816, 1, 'yi', 'sw'),
  (7817, 1, 'yi', 'ta'),
  (7818, 1, 'yi', 'te'),
  (7819, 1, 'yi', 'tg'),
  (7820, 1, 'yi', 'th'),
  (7821, 1, 'yi', 'tl'),
  (7822, 1, 'yi', 'tr'),
  (7823, 1, 'yi', 'uk'),
  (7824, 1, 'yi', 'ur'),
  (7825, 1, 'yi', 'uz'),
  (7826, 1, 'yi', 'vi'),
  (7827, 1, 'yi', 'yo'),
  (7828, 1, 'yi', 'zh'),
  (7829, 1, 'yi', 'zh-TW'),
  (7830, 1, 'yi', 'zu'),
  (7831, 1, 'yo', 'af'),
  (7832, 1, 'yo', 'ar'),
  (7833, 1, 'yo', 'az'),
  (7834, 1, 'yo', 'be'),
  (7835, 1, 'yo', 'bg'),
  (7836, 1, 'yo', 'bn'),
  (7837, 1, 'yo', 'bs'),
  (7838, 1, 'yo', 'ca'),
  (7839, 1, 'yo', 'ceb'),
  (7840, 1, 'yo', 'cs'),
  (7841, 1, 'yo', 'cy'),
  (7842, 1, 'yo', 'da'),
  (7843, 1, 'yo', 'de'),
  (7844, 1, 'yo', 'el'),
  (7845, 1, 'yo', 'en'),
  (7846, 1, 'yo', 'eo'),
  (7847, 1, 'yo', 'es'),
  (7848, 1, 'yo', 'et'),
  (7849, 1, 'yo', 'eu'),
  (7850, 1, 'yo', 'fa'),
  (7851, 1, 'yo', 'fi'),
  (7852, 1, 'yo', 'fr'),
  (7853, 1, 'yo', 'ga'),
  (7854, 1, 'yo', 'gl'),
  (7855, 1, 'yo', 'gu'),
  (7856, 1, 'yo', 'ha'),
  (7857, 1, 'yo', 'hi'),
  (7858, 1, 'yo', 'hmn'),
  (7859, 1, 'yo', 'hr'),
  (7860, 1, 'yo', 'ht'),
  (7861, 1, 'yo', 'hu'),
  (7862, 1, 'yo', 'hy'),
  (7863, 1, 'yo', 'id'),
  (7864, 1, 'yo', 'ig'),
  (7865, 1, 'yo', 'is'),
  (7866, 1, 'yo', 'it'),
  (7867, 1, 'yo', 'iw'),
  (7868, 1, 'yo', 'ja'),
  (7869, 1, 'yo', 'jw'),
  (7870, 1, 'yo', 'ka'),
  (7871, 1, 'yo', 'kk'),
  (7872, 1, 'yo', 'km'),
  (7873, 1, 'yo', 'kn'),
  (7874, 1, 'yo', 'ko'),
  (7875, 1, 'yo', 'la'),
  (7876, 1, 'yo', 'lo'),
  (7877, 1, 'yo', 'lt'),
  (7878, 1, 'yo', 'lv'),
  (7879, 1, 'yo', 'mg'),
  (7880, 1, 'yo', 'mi'),
  (7881, 1, 'yo', 'mk'),
  (7882, 1, 'yo', 'ml'),
  (7883, 1, 'yo', 'mn'),
  (7884, 1, 'yo', 'mr'),
  (7885, 1, 'yo', 'ms'),
  (7886, 1, 'yo', 'mt'),
  (7887, 1, 'yo', 'my'),
  (7888, 1, 'yo', 'ne'),
  (7889, 1, 'yo', 'nl'),
  (7890, 1, 'yo', 'no'),
  (7891, 1, 'yo', 'ny'),
  (7892, 1, 'yo', 'pa'),
  (7893, 1, 'yo', 'pl'),
  (7894, 1, 'yo', 'pt'),
  (7895, 1, 'yo', 'ro'),
  (7896, 1, 'yo', 'ru'),
  (7897, 1, 'yo', 'si'),
  (7898, 1, 'yo', 'sk'),
  (7899, 1, 'yo', 'sl'),
  (7900, 1, 'yo', 'so'),
  (7901, 1, 'yo', 'sq'),
  (7902, 1, 'yo', 'sr'),
  (7903, 1, 'yo', 'st'),
  (7904, 1, 'yo', 'su'),
  (7905, 1, 'yo', 'sv'),
  (7906, 1, 'yo', 'sw'),
  (7907, 1, 'yo', 'ta'),
  (7908, 1, 'yo', 'te'),
  (7909, 1, 'yo', 'tg'),
  (7910, 1, 'yo', 'th'),
  (7911, 1, 'yo', 'tl'),
  (7912, 1, 'yo', 'tr'),
  (7913, 1, 'yo', 'uk'),
  (7914, 1, 'yo', 'ur'),
  (7915, 1, 'yo', 'uz'),
  (7916, 1, 'yo', 'vi'),
  (7917, 1, 'yo', 'yi'),
  (7918, 1, 'yo', 'zh'),
  (7919, 1, 'yo', 'zh-TW'),
  (7920, 1, 'yo', 'zu'),
  (7921, 1, 'zh', 'af'),
  (7922, 1, 'zh', 'ar'),
  (7923, 1, 'zh', 'az'),
  (7924, 1, 'zh', 'be'),
  (7925, 1, 'zh', 'bg'),
  (7926, 1, 'zh', 'bn'),
  (7927, 1, 'zh', 'bs'),
  (7928, 1, 'zh', 'ca'),
  (7929, 1, 'zh', 'ceb'),
  (7930, 1, 'zh', 'cs'),
  (7931, 1, 'zh', 'cy'),
  (7932, 1, 'zh', 'da'),
  (7933, 1, 'zh', 'de'),
  (7934, 1, 'zh', 'el'),
  (7935, 1, 'zh', 'en'),
  (7936, 1, 'zh', 'eo'),
  (7937, 1, 'zh', 'es'),
  (7938, 1, 'zh', 'et'),
  (7939, 1, 'zh', 'eu'),
  (7940, 1, 'zh', 'fa'),
  (7941, 1, 'zh', 'fi'),
  (7942, 1, 'zh', 'fr'),
  (7943, 1, 'zh', 'ga'),
  (7944, 1, 'zh', 'gl'),
  (7945, 1, 'zh', 'gu'),
  (7946, 1, 'zh', 'ha'),
  (7947, 1, 'zh', 'hi'),
  (7948, 1, 'zh', 'hmn'),
  (7949, 1, 'zh', 'hr'),
  (7950, 1, 'zh', 'ht'),
  (7951, 1, 'zh', 'hu'),
  (7952, 1, 'zh', 'hy'),
  (7953, 1, 'zh', 'id'),
  (7954, 1, 'zh', 'ig'),
  (7955, 1, 'zh', 'is'),
  (7956, 1, 'zh', 'it'),
  (7957, 1, 'zh', 'iw'),
  (7958, 1, 'zh', 'ja'),
  (7959, 1, 'zh', 'jw'),
  (7960, 1, 'zh', 'ka'),
  (7961, 1, 'zh', 'kk'),
  (7962, 1, 'zh', 'km'),
  (7963, 1, 'zh', 'kn'),
  (7964, 1, 'zh', 'ko'),
  (7965, 1, 'zh', 'la'),
  (7966, 1, 'zh', 'lo'),
  (7967, 1, 'zh', 'lt'),
  (7968, 1, 'zh', 'lv'),
  (7969, 1, 'zh', 'mg'),
  (7970, 1, 'zh', 'mi'),
  (7971, 1, 'zh', 'mk'),
  (7972, 1, 'zh', 'ml'),
  (7973, 1, 'zh', 'mn'),
  (7974, 1, 'zh', 'mr'),
  (7975, 1, 'zh', 'ms'),
  (7976, 1, 'zh', 'mt'),
  (7977, 1, 'zh', 'my'),
  (7978, 1, 'zh', 'ne'),
  (7979, 1, 'zh', 'nl'),
  (7980, 1, 'zh', 'no'),
  (7981, 1, 'zh', 'ny'),
  (7982, 1, 'zh', 'pa'),
  (7983, 1, 'zh', 'pl'),
  (7984, 1, 'zh', 'pt'),
  (7985, 1, 'zh', 'ro'),
  (7986, 1, 'zh', 'ru'),
  (7987, 1, 'zh', 'si'),
  (7988, 1, 'zh', 'sk'),
  (7989, 1, 'zh', 'sl'),
  (7990, 1, 'zh', 'so'),
  (7991, 1, 'zh', 'sq'),
  (7992, 1, 'zh', 'sr'),
  (7993, 1, 'zh', 'st'),
  (7994, 1, 'zh', 'su'),
  (7995, 1, 'zh', 'sv'),
  (7996, 1, 'zh', 'sw'),
  (7997, 1, 'zh', 'ta'),
  (7998, 1, 'zh', 'te'),
  (7999, 1, 'zh', 'tg'),
  (8000, 1, 'zh', 'th'),
  (8001, 1, 'zh', 'tl'),
  (8002, 1, 'zh', 'tr'),
  (8003, 1, 'zh', 'uk'),
  (8004, 1, 'zh', 'ur'),
  (8005, 1, 'zh', 'uz'),
  (8006, 1, 'zh', 'vi'),
  (8007, 1, 'zh', 'yi'),
  (8008, 1, 'zh', 'yo'),
  (8009, 1, 'zh', 'zh-TW'),
  (8010, 1, 'zh', 'zu'),
  (8011, 1, 'zh-TW', 'af'),
  (8012, 1, 'zh-TW', 'ar'),
  (8013, 1, 'zh-TW', 'az'),
  (8014, 1, 'zh-TW', 'be'),
  (8015, 1, 'zh-TW', 'bg'),
  (8016, 1, 'zh-TW', 'bn'),
  (8017, 1, 'zh-TW', 'bs'),
  (8018, 1, 'zh-TW', 'ca'),
  (8019, 1, 'zh-TW', 'ceb'),
  (8020, 1, 'zh-TW', 'cs'),
  (8021, 1, 'zh-TW', 'cy'),
  (8022, 1, 'zh-TW', 'da'),
  (8023, 1, 'zh-TW', 'de'),
  (8024, 1, 'zh-TW', 'el'),
  (8025, 1, 'zh-TW', 'en'),
  (8026, 1, 'zh-TW', 'eo'),
  (8027, 1, 'zh-TW', 'es'),
  (8028, 1, 'zh-TW', 'et'),
  (8029, 1, 'zh-TW', 'eu'),
  (8030, 1, 'zh-TW', 'fa'),
  (8031, 1, 'zh-TW', 'fi'),
  (8032, 1, 'zh-TW', 'fr'),
  (8033, 1, 'zh-TW', 'ga'),
  (8034, 1, 'zh-TW', 'gl'),
  (8035, 1, 'zh-TW', 'gu'),
  (8036, 1, 'zh-TW', 'ha'),
  (8037, 1, 'zh-TW', 'hi'),
  (8038, 1, 'zh-TW', 'hmn'),
  (8039, 1, 'zh-TW', 'hr'),
  (8040, 1, 'zh-TW', 'ht'),
  (8041, 1, 'zh-TW', 'hu'),
  (8042, 1, 'zh-TW', 'hy'),
  (8043, 1, 'zh-TW', 'id'),
  (8044, 1, 'zh-TW', 'ig'),
  (8045, 1, 'zh-TW', 'is'),
  (8046, 1, 'zh-TW', 'it'),
  (8047, 1, 'zh-TW', 'iw'),
  (8048, 1, 'zh-TW', 'ja'),
  (8049, 1, 'zh-TW', 'jw'),
  (8050, 1, 'zh-TW', 'ka'),
  (8051, 1, 'zh-TW', 'kk'),
  (8052, 1, 'zh-TW', 'km'),
  (8053, 1, 'zh-TW', 'kn'),
  (8054, 1, 'zh-TW', 'ko'),
  (8055, 1, 'zh-TW', 'la'),
  (8056, 1, 'zh-TW', 'lo'),
  (8057, 1, 'zh-TW', 'lt'),
  (8058, 1, 'zh-TW', 'lv'),
  (8059, 1, 'zh-TW', 'mg'),
  (8060, 1, 'zh-TW', 'mi'),
  (8061, 1, 'zh-TW', 'mk'),
  (8062, 1, 'zh-TW', 'ml'),
  (8063, 1, 'zh-TW', 'mn'),
  (8064, 1, 'zh-TW', 'mr'),
  (8065, 1, 'zh-TW', 'ms'),
  (8066, 1, 'zh-TW', 'mt'),
  (8067, 1, 'zh-TW', 'my'),
  (8068, 1, 'zh-TW', 'ne'),
  (8069, 1, 'zh-TW', 'nl'),
  (8070, 1, 'zh-TW', 'no'),
  (8071, 1, 'zh-TW', 'ny'),
  (8072, 1, 'zh-TW', 'pa'),
  (8073, 1, 'zh-TW', 'pl'),
  (8074, 1, 'zh-TW', 'pt'),
  (8075, 1, 'zh-TW', 'ro'),
  (8076, 1, 'zh-TW', 'ru'),
  (8077, 1, 'zh-TW', 'si'),
  (8078, 1, 'zh-TW', 'sk'),
  (8079, 1, 'zh-TW', 'sl'),
  (8080, 1, 'zh-TW', 'so'),
  (8081, 1, 'zh-TW', 'sq'),
  (8082, 1, 'zh-TW', 'sr'),
  (8083, 1, 'zh-TW', 'st'),
  (8084, 1, 'zh-TW', 'su'),
  (8085, 1, 'zh-TW', 'sv'),
  (8086, 1, 'zh-TW', 'sw'),
  (8087, 1, 'zh-TW', 'ta'),
  (8088, 1, 'zh-TW', 'te'),
  (8089, 1, 'zh-TW', 'tg'),
  (8090, 1, 'zh-TW', 'th'),
  (8091, 1, 'zh-TW', 'tl'),
  (8092, 1, 'zh-TW', 'tr'),
  (8093, 1, 'zh-TW', 'uk'),
  (8094, 1, 'zh-TW', 'ur'),
  (8095, 1, 'zh-TW', 'uz'),
  (8096, 1, 'zh-TW', 'vi'),
  (8097, 1, 'zh-TW', 'yi'),
  (8098, 1, 'zh-TW', 'yo'),
  (8099, 1, 'zh-TW', 'zh'),
  (8100, 1, 'zh-TW', 'zu'),
  (8101, 1, 'zu', 'af'),
  (8102, 1, 'zu', 'ar'),
  (8103, 1, 'zu', 'az'),
  (8104, 1, 'zu', 'be'),
  (8105, 1, 'zu', 'bg'),
  (8106, 1, 'zu', 'bn'),
  (8107, 1, 'zu', 'bs'),
  (8108, 1, 'zu', 'ca'),
  (8109, 1, 'zu', 'ceb'),
  (8110, 1, 'zu', 'cs'),
  (8111, 1, 'zu', 'cy'),
  (8112, 1, 'zu', 'da'),
  (8113, 1, 'zu', 'de'),
  (8114, 1, 'zu', 'el'),
  (8115, 1, 'zu', 'en'),
  (8116, 1, 'zu', 'eo'),
  (8117, 1, 'zu', 'es'),
  (8118, 1, 'zu', 'et'),
  (8119, 1, 'zu', 'eu'),
  (8120, 1, 'zu', 'fa'),
  (8121, 1, 'zu', 'fi'),
  (8122, 1, 'zu', 'fr'),
  (8123, 1, 'zu', 'ga'),
  (8124, 1, 'zu', 'gl'),
  (8125, 1, 'zu', 'gu'),
  (8126, 1, 'zu', 'ha'),
  (8127, 1, 'zu', 'hi'),
  (8128, 1, 'zu', 'hmn'),
  (8129, 1, 'zu', 'hr'),
  (8130, 1, 'zu', 'ht'),
  (8131, 1, 'zu', 'hu'),
  (8132, 1, 'zu', 'hy'),
  (8133, 1, 'zu', 'id'),
  (8134, 1, 'zu', 'ig'),
  (8135, 1, 'zu', 'is'),
  (8136, 1, 'zu', 'it'),
  (8137, 1, 'zu', 'iw'),
  (8138, 1, 'zu', 'ja'),
  (8139, 1, 'zu', 'jw'),
  (8140, 1, 'zu', 'ka'),
  (8141, 1, 'zu', 'kk'),
  (8142, 1, 'zu', 'km'),
  (8143, 1, 'zu', 'kn'),
  (8144, 1, 'zu', 'ko'),
  (8145, 1, 'zu', 'la'),
  (8146, 1, 'zu', 'lo'),
  (8147, 1, 'zu', 'lt'),
  (8148, 1, 'zu', 'lv'),
  (8149, 1, 'zu', 'mg'),
  (8150, 1, 'zu', 'mi'),
  (8151, 1, 'zu', 'mk'),
  (8152, 1, 'zu', 'ml'),
  (8153, 1, 'zu', 'mn'),
  (8154, 1, 'zu', 'mr'),
  (8155, 1, 'zu', 'ms'),
  (8156, 1, 'zu', 'mt'),
  (8157, 1, 'zu', 'my'),
  (8158, 1, 'zu', 'ne'),
  (8159, 1, 'zu', 'nl'),
  (8160, 1, 'zu', 'no'),
  (8161, 1, 'zu', 'ny'),
  (8162, 1, 'zu', 'pa'),
  (8163, 1, 'zu', 'pl'),
  (8164, 1, 'zu', 'pt'),
  (8165, 1, 'zu', 'ro'),
  (8166, 1, 'zu', 'ru'),
  (8167, 1, 'zu', 'si'),
  (8168, 1, 'zu', 'sk'),
  (8169, 1, 'zu', 'sl'),
  (8170, 1, 'zu', 'so'),
  (8171, 1, 'zu', 'sq'),
  (8172, 1, 'zu', 'sr'),
  (8173, 1, 'zu', 'st'),
  (8174, 1, 'zu', 'su'),
  (8175, 1, 'zu', 'sv'),
  (8176, 1, 'zu', 'sw'),
  (8177, 1, 'zu', 'ta'),
  (8178, 1, 'zu', 'te'),
  (8179, 1, 'zu', 'tg'),
  (8180, 1, 'zu', 'th'),
  (8181, 1, 'zu', 'tl'),
  (8182, 1, 'zu', 'tr'),
  (8183, 1, 'zu', 'uk'),
  (8184, 1, 'zu', 'ur'),
  (8185, 1, 'zu', 'uz'),
  (8186, 1, 'zu', 'vi'),
  (8187, 1, 'zu', 'yi'),
  (8188, 1, 'zu', 'yo'),
  (8189, 1, 'zu', 'zh'),
  (8190, 1, 'zu', 'zh-TW');